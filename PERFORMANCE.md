...
| | measured |
| --- | --- |
| Mutation harness | **593 mutants, 824 s wall · 701 s user · 74 s sys** |
| Source | 25 460 lines TypeScript · 19 797 lines of tests |
| Test suite | **1 125 tests, 11.5 s** (`npm test`) |

Latest full run: **585 killed · 0 survived · 8 equivalent · 0 missing ·
0 ambiguous · 0 invalid · 0 relabel.**

### Where the 824 seconds actually go
...
| baseline full suite | 12.0 s | 1.5 % | 1 | 12.0 s |
| per-mutant typecheck | 125.4 s | 15.4 % | 593 | 0.21 s |
| **targeted test runs** | **573.1 s** | **70.2 %** | 592 | **0.97 s** |
| full-suite fallbacks | 105.9 s | 13.0 % | 9 | 11.77 s |
| total | 816.7 s | | | |
...
- **`sys` is 73.8 s — 9.0 % of 824.3 s real.** `user` is 701.3 s, **85 %**.
- The harness now measures its own **process-startup floor** directly, by timing
  one bare invocation of each subprocess: `tsc` 77 ms, `tsx` 67 ms. Against the
  observed counts that is `77 ms × 593 + 67 ms × 601` = **85.9 s, 10.5 %** of the
...
The cost centre is the **targeted test runs: 70 % of the run**, at 0.97 s each
across 592 invocations — of which only ~67 ms is startup. The other ~0.90 s is
...
sources 592 times": precompile once and run plain JS, or keep a warm worker.

Two smaller levers, for scale: the per-mutant typecheck is 15.4 % (36 % of which
*is* startup, so an incremental/`--watch` tsc server would help there), and the
full-suite fallbacks are 13.0 % from only **9** runs at 11.8 s each. Fallbacks
