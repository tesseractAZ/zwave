...
- **Measured at:** each figure section states when it was measured — a
  version, a date or both; the newest is §3, v0.66.0 (2026-09-19), and §8
...
The tables and run figures below are from the v0.66.0 release run, 2026-09-19.

| | measured |
| --- | --- |
| Mutation harness | **690 mutants, 1783 s wall · 804 s user · 99 s sys** |
| Source | 26 213 lines TypeScript · 21 399 lines of tests |
| Test suite | **1 190 tests, 12.6 s** (`npm test`) |

Latest full run: **681 killed · 0 survived · 9 equivalent · 0 missing ·
0 ambiguous · 0 invalid · 0 relabel.**

### Where the 1783 seconds actually go
...
| baseline full suite | 12.3 s | 0.7 % | 1 | 12.3 s |
| per-mutant typecheck | 141.0 s | 7.9 % | 690 | 0.20 s |
| **targeted test runs** | **1495.1 s** | **84.3 %** | 689 | **2.17 s** |
| full-suite fallbacks | 124.9 s | 7.0 % | 10 | 12.49 s |
| total | 1773.6 s | | | |
...
- **`sys` is 99.0 s — 5.6 % of 1782.8 s real.** `user` is 804.4 s, **45 %**.
- The harness now measures its own **process-startup floor** directly, by timing
  one bare invocation of each subprocess: `tsc` 60 ms, `tsx` 71 ms. Against the
  observed counts that is `60 ms × 690 + 71 ms × 699` = **91.0 s, 5.1 %** of the
...
across 689 invocations — of which only ~71 ms is startup. The harness times
...
Two smaller levers, for scale: the per-mutant typecheck is 7.9 % (29 % of which
*is* startup, so an incremental/`--watch` tsc server would help there), and the
full-suite fallbacks are 7.0 % from only **10** runs at 12.5 s each. A mutant
...
baseline (~12 s) rather than a full run (~30 minutes; 1783 s in the v0.66.0
