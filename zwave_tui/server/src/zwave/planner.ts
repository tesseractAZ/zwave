...
export function planFor(symptom: Symptom, node: NodeSnapshot | undefined, ctx: PlanContext): Plan {
  ...
  switch (symptom.kind) {
    ...
    case 'node-down': {
      headline = 'Node is DOWN — the driver proved it, a ping often revives it';
      candidates.push({ action: 'ping', title: 'Ping the node', rationale: 'Dead is set REACTIVELY: a transmission to this node failed and none has since succeeded. That is a measured verdict, not an inference from silence. A ping is the cheapest thing that can disprove it — a node whose radio is still present often answers and returns to Alive immediately. With auto-ping enabled this is attempted automatically after its dwell, up to its attempt cap; past that cap the engine says so and stops, and a manual ping is worth trying even then, because the cap expiring says nothing about whether the node is reachable NOW.', basis: 'source', cost: 'safe', blocked: gateExecutable(node, ctx, { probes: true }) });
      candidates.push({ action: null, title: 'Power-cycle the device, then exclude/re-include if it persists', rationale: 'If pings do not bring it back, the runbook is physical: power-cycle the device, and if it stays dead, exclude and re-include it. A route rebuild cannot repair a node that cannot be reached — the rebuild\u2019s first step is to query the node, which a dead node cannot answer.', basis: 'lore', cost: 'physical', blocked: null });
      break;
    }
