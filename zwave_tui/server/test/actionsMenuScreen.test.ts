    ...
    for (const rebuilding of [false, true]) {
      for (const hasNode of [false, true]) {
        for (const locked of [false, true]) {
          ...
          const items = buildMenu({ scope, hasNode, rebuilding, identityPending: false, identityResumable: false, autonomyPausedByTui: false });
...
test('the menu header always states its blast radius, at every width', () => {
  ...
  for (const [cols, rows] of SIZES) {
    const dev = renderActionsMenu(view(cols, rows), {
      scope: 'device', items: buildMenu({ scope: 'device', hasNode: true, rebuilding: false, identityPending: false, identityResumable: false, autonomyPausedByTui: false }),
      index: 0, targetLabel: '#16 Kitchen', locked: false,
    }).map((l) => l.replace(/\x1b\[[0-9;]*m/g, '')).join('\n');
    const net = renderActionsMenu(view(cols, rows), {
      scope: 'network', items: buildMenu({ scope: 'network', hasNode: false, rebuilding: false, identityPending: false, identityResumable: false, autonomyPausedByTui: false }),
    ...
    const noNode = renderActionsMenu(view(cols, rows), {
      scope: 'device', items: buildMenu({ scope: 'device', hasNode: false, rebuilding: false, identityPending: false, identityResumable: false, autonomyPausedByTui: false }),
      index: 0, targetLabel: null, locked: false,
    }).map((l) => l.replace(/\x1b\[[0-9;]*m/g, '')).join('\n');
    if (cols >= 40) {
...
test('renderActionsMenu holds the contract with a LONG menu (control + config rows) at every size + cursor', () => {
  const items = [...buildMenu({ scope: 'device', hasNode: true, rebuilding: false, identityPending: false, identityResumable: false, autonomyPausedByTui: false }), ...buildEntityRows(manyEntities), ...buildConfigRows(manyParams)];
  for (const [cols, rows] of SIZES) {
