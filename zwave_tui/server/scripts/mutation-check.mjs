...
const MUTANTS = [
  ...
  { id: 'between-tick-death-not-double-counted', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (state.deadSince.has(d.nodeId)) continue; // Dead at this tick: trackEpisodes owns it",
    repl: "",
    what: "a death the tick also saw is counted once",
    equivalent: true,
    why: "a death the tick saw is settled first: settleProbeDeath removes the unanswered probe it books, and any probe the node answered before dying fails the deaths loop's own answered-at-death test, so the loop finds nothing left to blame; and the feed hands over a death only once it cleared. The guard is defence in depth against a feed that ever loses that ordering." },
  ...
  { id: 'measurement-reroute-window', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (sinceMs >= 0 && sinceMs <= ANSWER_GRACE_MS) ourFrame = { lane: sent.lane, frame: sent.frame, sinceMs };",
    repl: "        ourFrame = { lane: sent.lane, frame: sent.frame, sinceMs };",
  ...
  { id: 'stamp-consumed-by-our-frame', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "counters.tx + counters.dropTx > sent.tx0) {\n        this.measurementSent.delete(nodeId);",
    repl: "counters.tx + counters.dropTx > sent.tx0) {",
