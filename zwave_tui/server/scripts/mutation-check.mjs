...
const MUTANTS = [
  ...
  { id: 'hometag-answerable-readonly', file: 'src/telnet/session.ts', tests: ['homeTag'],
    ...
    find: '    if (!isLocal && !this.actions?.enabled) {',
  ...
  { id: 'autoping-probe-death-hygiene', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    // Re-pointed in v0.71.0: probeDeath is a Map now.
    find: "  for (const id of [...state.probeDeath.keys()]) if (!seen.has(id)) state.probeDeath.delete(id);",
    repl: "",
  ...
  { id: 'autoping-probe-death-feeds-reply-rate', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    // Re-pointed in v0.71.0: the frame rides along.
    find: "      if (s.lane === 'sweep') o.onProbeResult?.(s.nodeId, false, s.cls, s.frame);",
    repl: "",
  ...
  { id: 'autoping-probe-death-reply-rate-sweep-only', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    // Re-pointed in v0.71.0: the frame rides along.
    find: "      if (s.lane === 'sweep') o.onProbeResult?.(s.nodeId, false, s.cls, s.frame);",
    repl: "      o.onProbeResult?.(s.nodeId, false, s.cls, s.frame);",
  ...
  /* ── v0.64.4 review ───────────────────────────────────────────────────── */
  // `autoping-probe-death-sweep-only` RETIRED in v0.71.0: a verification kill
  // skips the dwell too, and the kill–revive loop is bounded by the post-kill
  // hold instead — see `probe-death-covers-verify` and the hold mutants.
  { id: 'autoping-probe-death-newest-decides', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    // Re-pointed in v0.71.0: an older pending sweep must lend nothing to a MANUAL kill.
    find: "  const newest = killed[killed.length - 1];",
    repl: "  const newest = killed.find((p) => p.lane === 'sweep' || p.lane === 'verify') ?? killed[killed.length - 1];",
  ...
  /* ── v0.64.6: the transient/undersampled split measures the LIVE span ──── */
  { id: 'undersampled-measures-live-span', file: 'src/zwave/outcomes.ts', tests: ['outcomes', 'zwaveDataChurn'],
    // Restores open-to-resolve: the 10-min confirmation window counts as live
    // time, every blink closes "undersampled", and `transient` is unreachable.
    find: '        const liveMs = (resolveOpts?.absentSinceMs ?? resolvedMs) - liveStart;',
    repl: '        const liveMs = resolvedMs - liveStart;',
    what: 'a blink is measured to its first absence, not to the end of the confirmation window' },
  { id: 'live-span-anchored-at-dwell-start', file: 'src/zwave/outcomes.ts', tests: ['outcomes', 'zwaveDataChurn'],
    // Anchors at the open tick: an episode that opened late carries a reading
    // that simply aged out back under the boundary, relabelling it transient.
    find: '        const liveStart = ep.dwellStartMs ?? ep.onsetMs - DWELL_MS;',
  ...
  { id: 'open-stores-dwell-start', file: 'src/zwave/outcomes.ts', tests: ['outcomes', 'zwaveDataChurn'],
    ...
    repl: '',
    what: 'an episode keeps the dwell start it was opened with' },
  { id: 'lifecycle-carries-first-absence', file: 'src/zwave/outcomes.ts', tests: ['outcomes', 'zwaveDataChurn'],
    find: '      toResolve.push({ nodeId: ep.nodeId, kind: ep.kind, key: ep.key, absentSinceMs: since });',
    repl: '      toResolve.push({ nodeId: ep.nodeId, kind: ep.kind, key: ep.key, absentSinceMs: now });',
    what: 'a resolution carries the tick the symptom was FIRST seen absent, not the resolve tick' },
  { id: 'resolve-receives-absent-since', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      const ep = oc.resolve(r.nodeId, r.kind, now, this.nodeWindow(r.nodeId, now), { unprobeable, absentSinceMs: r.absentSinceMs });',
    repl: '      const ep = oc.resolve(r.nodeId, r.kind, now, this.nodeWindow(r.nodeId, now), { unprobeable });',
    what: 'the production caller hands the ledger the first absence; without it every blink closes undersampled' },
  { id: 'open-receives-dwell-start', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      oc.open(s.nodeId, s.kind, now, this.degradedWindow(s.nodeId, since, now), { dwellStartMs: since });',
    repl: '      oc.open(s.nodeId, s.kind, now, this.degradedWindow(s.nodeId, since, now));',
    what: 'the production caller hands the ledger the dwell start' },
  { id: 'split-marker-written', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    find: "        splitRule: 'live-span',",
    repl: '        splitRule: undefined,',
    what: 'a saved ledger marks its split tallies, so they survive the next load' },
  { id: 'legacy-split-tallies-not-restored', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    find: "      if (o.splitRule === 'live-span') {",
    repl: "      if (o.splitRule !== 'never-written') {",
    what: 'a pre-v0.64.6 ledger does not restore split tallies counted on open-to-resolve time' },
  { id: 'legacy-discard-reaches-disk', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    find: '          dirty = true; // the marker must reach disk, or the next load discards again',
    repl: '',
    what: 'the marker is written after a discard, so the next load does not discard again' },
  { id: 'legacy-discard-announced', file: 'src/zwave/outcomes.ts', tests: ['outcomes', 'zwaveDataChurn'],
    find: '          loadNotice = `${Math.round(t + u)} transient/undersampled tallies reset — split rule changed (v0.64.6)`;',
    repl: '          void 0;',
    what: 'discarding pre-v0.64.6 tallies is announced, never silent' },
  { id: 'load-notice-is-one-shot', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    find: '      const n = loadNotice;\n      loadNotice = null;\n      return n;',
    repl: '      const n = loadNotice;\n      return n;',
    what: 'the load notice is handed over once' },
  { id: 'legacy-discard-reaches-the-log-ring', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: "    if (ocNotice) this.pushEvent('engine', 'info', 'system', null, ocNotice);",
    repl: '    void ocNotice;',
    what: 'the discard is visible in the TUI, not only on stdout' },
  /* ── v0.64.6: only a counter that proves the node was HEARD stamps lastSeen ─ */
  { id: 'lastseen-timeout-not-heard', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '        prev.commandsDroppedRX !== counters.dropRx);',
    repl: '        prev.commandsDroppedRX !== counters.dropRx ||\n        prev.timeoutResponse !== counters.timeout);',
    what: 'a response timeout (the node NOT answering) does not stamp lastSeen' },
  { id: 'lastseen-droptx-not-heard', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      (prev.commandsTX !== counters.tx ||',
    repl: '      (prev.commandsTX !== counters.tx || prev.commandsDroppedTX !== counters.dropTx ||',
    what: 'the dropped-TX counter moving alone does not stamp lastSeen (defensive)' },
  { id: 'lastseen-droprx-is-heard', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '        prev.commandsRX !== counters.rx ||\n        prev.commandsDroppedRX !== counters.dropRx);',
    repl: '        prev.commandsRX !== counters.rx);',
    what: 'an undecodable frame from the node still stamps lastSeen' },
  { id: 'lastseen-rx-is-heard', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      (prev.commandsTX !== counters.tx ||\n        prev.commandsRX !== counters.rx ||',
    repl: '      (prev.commandsTX !== counters.tx ||',
    what: 'a frame received from the node stamps lastSeen' },
  /* ── v0.64.6: rate-fallback needs a PERSISTENT regression ─────────────────── */
  { id: 'rate-fallback-rejects-one-retried-frame', file: 'src/zwave/symptoms.ts', tests: ['symptoms'],
    find: 'const RATE_FALLBACK_MIN_TX = 2;',
    repl: 'const RATE_FALLBACK_MIN_TX = 1;',
    what: 'one retried frame (a single-exchange retry) cannot arm rate-fallback' },
  { id: 'rate-fallback-lost-route-de-asserts', file: 'src/zwave/symptoms.ts', tests: ['symptoms'],
    find: '      const b = !node.isLongRange && run != null && last?.routeKey === run.routeKey &&',
    repl: '      const b = !node.isLongRange && run != null &&',
    what: 'a route the driver no longer reports is not asserted as regressed' },
  { id: 'rate-fallback-reports-the-counted-reading', file: 'src/zwave/symptoms.ts', tests: ['symptoms'],
    find: '        const rate = run?.rateKbps ?? null;',
    repl: '        const rate = last?.rateKbps ?? null;',
    what: "the card reports the run's newest reading, not an aborted transmission's rewritten rate" },
  { id: 'rate-run-copies-are-not-transmissions', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms', 'evidenceStore'],
    find: '  const transmitted = s.fresh && s.dTx != null && s.dTx > 0;',
    repl: '  const transmitted = true;',
    what: 'a copied cached rate is not another transmission' },
  { id: 'rate-run-rx-or-failure-is-not-a-reading', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms', 'evidenceStore'],
    find: '  const transmitted = s.fresh && s.dTx != null && s.dTx > 0;',
    repl: '  const transmitted = s.fresh;',
    what: 'a fresh tick with no acknowledged send (received traffic, an aborted transmission) is not a reading' },
  { id: 'rate-run-copies-do-not-move-it', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms', 'evidenceStore'],
    // Repointed in the v0.64.6 review: the comment on this line was corrected.
    find: '  if (!transmitted || s.rateKbps == null) return prev; // a copied or aborted-transmission rate is not a reading',
    repl: '  if (s.rateKbps == null) return prev;',
    what: 'only reading ticks move the rate run' },
  { id: 'rate-run-newest-reading-decides', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms'],
    find: '  if (s.rateKbps >= 100) return { routeKey: s.routeKey, sawHundred: true, belowTx: 0, lastBelowTxAt: null, rateKbps: s.rateKbps };',
    repl: '  if (s.rateKbps >= 100) return { routeKey: s.routeKey, sawHundred: true, belowTx: onRoute?.belowTx ?? 0, lastBelowTxAt: onRoute?.lastBelowTxAt ?? null, rateKbps: s.rateKbps };',
    what: 'a 100k reading clears rate-fallback on that tick, not 30 min later' },
  { id: 'rate-run-new-route-has-no-capability', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms'],
    find: '  const onRoute = prev != null && prev.routeKey === s.routeKey ? prev : null;',
    repl: '  const onRoute = prev;',
    what: '100k capability does not carry across a reroute' },
  { id: 'rate-run-separate-exchanges', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms', 'evidenceStore'],
    find: '  const separate = onRoute?.lastBelowTxAt == null || s.t - onRoute.lastBelowTxAt >= RATE_FALLBACK_MIN_GAP_MS;',
    repl: '  const separate = true;',
    what: 'transmit reports seconds apart are one exchange, not two readings' },
  { id: 'rate-run-capability-is-observed', file: 'src/zwave/evidenceStore.ts', tests: ['symptoms'],
    find: '    sawHundred: onRoute?.sawHundred === true,',
    repl: '    sawHundred: true,',
    what: 'a route never seen at 100k does not fire (capability cap, fail-closed)' },
  { id: 'rate-run-survives-an-lwr-blink', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '  if (s.routeKey == null) return prev; // a route we cannot see has not moved',
    repl: '  if (s.routeKey == null) return null;',
    what: 'a momentary missing route does not erase the rate memory' },
  { id: 'rate-run-folded-on-record', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '      const nextRun = foldRateRun(rateRuns.get(nodeId) ?? null, sample);',
    repl: '      const nextRun = rateRuns.get(nodeId) ?? null;',
    what: 'every recorded sample updates the rate run' },
  { id: 'rate-run-evicted-with-the-node', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '      routeFails.delete(nodeId);\n      rateRuns.delete(nodeId);',
    repl: '      routeFails.delete(nodeId);',
    what: 'a reused node id does not inherit 100k capability' },
  { id: 'rate-run-reset-with-the-store', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '      rateRuns.clear();\n      since = null;',
    repl: '      since = null;',
    what: 'a reset discards the rate memory' },
  { id: 'rate-run-cleared-on-load', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '      lastCtrl = null;\n      rateRuns.clear();\n      try {',
    repl: '      lastCtrl = null;\n      try {',
    what: 'a load replaces the rate memory with the restored file\'s' },
  { id: 'rate-run-seeded-on-load', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '            const seeded = bounded.reduce<RateRun | null>(foldRateRun, null);',
    repl: '            const seeded = null as RateRun | null;',
    what: 'a restart re-folds the rate run from the restored ring' },
  { id: 'rate-fallback-reads-the-store', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '        rateRun: (id) => ev.rateRun(id),',
    repl: '        rateRun: () => null,',
    what: 'the production engine reads the store rate run; type-checking alone cannot catch a stub' },
  /* ── v0.64.6 review ─────────────────────────────────────────────────────── */
  { id: 'quiet-node-starved-is-undersampled', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    find: "          if (kind === 'quiet-node' || liveMs >= DWELL_MS + UNDERSAMPLED_AFTER_MS) {",
    repl: '          if (liveMs >= DWELL_MS + UNDERSAMPLED_AFTER_MS) {',
    what: 'a silent node is never filed as a blink — its before-window cannot hold a transmission' },
  /* ── v0.65.0: the 2026-09-15 live audit ─────────────────────────────────── */
  { id: 'rf-off-suppresses-every-lane', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: "  if (input.rfOffSince != null) return { ...base, suppressed: gate('controller-rf-off') };",
    repl: '',
    what: 'a switched-off receiver suppresses every lane, the dead ladder included' },
  { id: 'rf-off-reported-through-the-boot-gate', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: "  if (input.rfOffSince != null) return { ...base, suppressed: gate('controller-rf-off') };",
    repl: "  if (input.rfOffSince != null) return { ...base, suppressed: 'controller-rf-off' };",
    what: 'the blackout still reports boot-window while the mesh is settling' },
  { id: 'blackout-drops-unanswered-probes', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '      const unjudgeable = !answered && p.at <= rfOffSince;',
    repl: '      const unjudgeable = false;',
    what: 'a probe unanswered when the radio went off is dropped, never booked as a miss' },
  { id: 'blackout-keeps-answered-probes', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '      const unjudgeable = !answered && p.at <= rfOffSince;',
    repl: '      const unjudgeable = p.at <= rfOffSince;',
    what: 'a probe ANSWERED before the blackout keeps its credit' },
  { id: 'blackout-only-drops-what-was-in-flight', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '      const unjudgeable = !answered && p.at <= rfOffSince;',
    repl: '      const unjudgeable = !answered;',
    what: 'a probe sent after the blackout is left to the ordinary judgment' },
  { id: 'runner-drops-blackout-probes', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '    const rfOff = o.rfOffSince?.() ?? null;',
    repl: '    const rfOff = null;',
    what: 'the runner reads the RF-off reading and drops what was in flight' },
  { id: 'decision-sees-the-rf-off-reading', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '      rfOffSince: o.rfOffSince?.() ?? null,',
    repl: '      rfOffSince: null,',
    what: 'the suppression is wired to the live reading, not merely implemented' },
  { id: 'reconnect-burst-not-self-proven', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: '      const selfProven = heardRecently && spokeOnItsOwn && attributed != null && !inReconnectBurst;',
    repl: '      const selfProven = heardRecently && spokeOnItsOwn && attributed != null;',
    what: "the driver's own restart burst is never credited as the node's voice",
    // EQUIVALENT, for exactly the reason `boot-attribution-not-credited` above
    // is. `attributionUnknown` covers the ONLY state this clause can change —
    // heardRecently && spokeOnItsOwn && attributed != null && inReconnectBurst
    // makes it true — and it is tested FIRST in both consumers of `selfProven`
    // (the `cls` ladder and the log line), so `cls` is 'attribution-unknown'
    // either way. It stays because it keeps the local correct on its OWN terms
    // rather than correct only by its callers' ordering. The rule itself is
    // pinned by `reconnect-burst-is-attribution-unknown`, which IS killed.
    equivalent: true,
  },
  ...
  { id: 'reconnect-burst-has-a-deadline', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    repl: '        && seenAt >= reconnAt - DRIVER_BURST_LEAD_MS;',
    what: 'the restart-burst doubt EXPIRES — a reconnect cannot suppress self-proof for the life of the process' },
  /* ── v0.65.0 review: the guards the audit fixes needed of their own ────── */
  { id: 'rearm-defers-to-a-rebuild-in-flight', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    if (this.rebuildInFlight) return;',
    repl: '',
    what: 'a re-arm never tears the handles out from under a rebuild already running' },
  { id: 'rebuild-announces-itself', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      this.rebuildInFlight = true;',
    repl: '',
    what: 'a rebuild in flight is VISIBLE to the re-arm — the epoch cannot separate two runs on one connection' },
  { id: 'reload-latch-survives-a-declined-rearm', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    this.entryReloadSeen = false;\n    this.log(`live statistics: re-subscribing (${why})`);',
    repl: '    this.log(`live statistics: re-subscribing (${why})`);',
    what: 'the reload is consumed by the re-arm that RUNS, not by one the throttle turned away' },
  { id: 'activity-state-feed-is-retained', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      owned.push(sub);',
    repl: '',
    what: 'a re-arm RELEASES the whole-house state feed instead of leaving a zombie fanout behind' },
  { id: 'activity-notification-feed-is-retained', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      if (notif) owned.push(notif);',
    repl: '',
    what: 'a re-arm releases the notification feed too — its handle is captured, not discarded' },
  { id: 'counter-reset-is-not-a-transmission', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      !counterReset &&\n',
    repl: '',
    what: "a NEW driver's zeroed counters do not stamp \"heard just now\" on the whole roster" },
  { id: 'reload-anchors-the-restart-burst', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    if (ws == null) return reload;',
    repl: '    if (ws == null) return null;',
    what: 'a driver restart the driver-WS never reconnects to still anchors the burst window' },
  { id: 'driver-handshake-is-stamped', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '              this.driverWsConnAt = Date.now();',
    repl: '',
    what: 'the driver handshake records WHEN the restart burst began, not merely that it happened' },
  { id: 'rf-off-reading-crosses-the-data-layer', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    return this.driverWs?.controllerRfOffSince() ?? null;',
    repl: '    return null;',
    what: 'the blackout reading reaches the engine, not only the driver-WS client that computes it' },
  { id: 'index-wires-the-rf-off-reading', file: 'src/index.ts', tests: ['configContract'],
    find: '      rfOffSince: () => zwaveData.controllerRfOffSince(),',
    repl: '      rfOffSince: () => null,',
    what: 'index.ts hands auto-ping the live RF-off reading (source assertion — both hooks are optional, so tsc cannot)' },
  { id: 'index-wires-the-restart-stamp', file: 'src/index.ts', tests: ['configContract'],
    find: '      driverReconnectedAt: () => zwaveData.driverReconnectedAt(),',
    repl: '      driverReconnectedAt: () => null,',
    what: 'index.ts hands auto-ping the driver-restart stamp (source assertion)' },
  { id: 'reconnect-burst-window-is-small', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    find: 'export const DRIVER_BURST_MS = 120_000;',
    repl: 'export const DRIVER_BURST_MS = 30 * 60_000;',
    what: 'the burst window is bounded at a size the measured ~9 s restart justifies, not merely bounded' },
  { id: 'rf-event-ignores-node-lines', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: "  if (ctx && typeof ctx === 'object' && ctx.type === 'node') return null;",
    repl: '',
    what: "a node-attributed log line can never switch the mesh's write lane off" },
  { id: 'rf-reading-needs-a-live-lane', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: '  if (!link.live || !link.logsAcked || link.stormStopped) return null;',
    repl: '',
    what: 'a reading nobody can refresh is not trusted (the `RF on` arrives on the same lane)' },
  { id: 'rf-reading-expires', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: '  return now - rfOffSince < RF_OFF_MAX_MS ? rfOffSince : null;',
    repl: '  return rfOffSince;',
    what: 'a missed `Turning RF on` cannot suppress the sweep forever' },
  { id: 'rf-off-recorded', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: "    if (rf === 'off') rfOffSince = now;",
    // A body-less `if`, NOT a deletion: deleting the line strands the
    // `else if` on the next one, so the mutant fails to COMPILE and the whole
    // run is void — a mutant that cannot build is scored INVALID, never killed.
    repl: "    if (rf === 'off') { /* v0.65.0 reverted: the blackout is not recorded */ }",
    what: 'the controller taking its radio down is actually recorded' },
  { id: 'rf-on-clears-the-hold', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: "    else if (rf === 'on') rfOffSince = null;",
    repl: '',
    what: 'the radio coming back clears the hold' },
  { id: 'rf-hold-dies-with-the-connection', file: 'src/zwave/driverWsClient.ts', tests: ['driverWsClient'],
    find: `    // A blackout observed on the OLD connection says nothing about the new one,
    // and a \`Turning RF on\` that lands while we are disconnected is lost — so
    // the suppression never survives a reconnect (v0.65.0).
    rfOffSince = null;`,
    repl: `    // A blackout observed on the OLD connection says nothing about the new one,
    // and a \`Turning RF on\` that lands while we are disconnected is lost — so
    // the suppression never survives a reconnect (v0.65.0).`,
    what: 'a blackout seen on the old connection does not survive a reconnect' },
  { id: 'entry-reload-latched', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '        this.entryReloadSeen = true;\n        this.entryReloadAt = Date.now();',
    repl: '        this.entryReloadAt = Date.now();',
    what: 'a config-entry reload is remembered until the roster poll answers again' },
  { id: 'entry-reload-rearms-the-feeds', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: "        void this.rearmStatsFeeds('zwave_js config entry reloaded');",
    repl: '        void 0;',
    what: 'the statistics feeds are re-armed after a reload — the engine must not go blind' },
  { id: 'stats-watchdog-rearms-a-dead-feed', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: `    return this.statsSubscribed
      && this.lastStatsAt != null
      && now - this.lastStatsAt > STATS_FEED_DEAD_MS
      && this.client.ready();`,
    repl: '    return false;',
    what: 'a feed that silently stops delivering is re-armed on its own — absence is checked' },
  { id: 'stats-watchdog-needs-a-prior-event', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: `    return this.statsSubscribed
      && this.lastStatsAt != null
      && now - this.lastStatsAt > STATS_FEED_DEAD_MS
      && this.client.ready();`,
    repl: `    return this.statsSubscribed
      && now - (this.lastStatsAt ?? 0) > STATS_FEED_DEAD_MS
      && this.client.ready();`,
    what: 'a feed that has never delivered anything is not re-armed in a loop' },
  { id: 'rearm-releases-the-orphans', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    for (const sub of stale) await sub.unsubscribe().catch(() => {});',
    repl: '    void stale;',
    what: 'the orphaned subscriptions are RELEASED, not left doubled on the socket' },
  { id: 'rearm-is-throttled', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '    if (now - this.lastFeedRearmAt < STATS_FEED_DEAD_MS) return;',
    repl: '',
    what: 'a failing re-arm does not become a per-tick reconnect storm' },
  { id: 'stats-subs-are-retained', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: '      this.ownedStatsSubs = owned.slice();',
    repl: '      this.ownedStatsSubs = [];',
    what: 'the subscription handles are retained, so a re-arm can release them' },
  { id: 'blind-engine-degrades-the-mesh', file: 'src/haStates.ts', tests: ['haStates'],
    find: '    || statsBlind',
    repl: '',
    what: 'an engine whose evidence has stopped arriving raises the degraded sensor' },
  { id: 'blind-needs-a-reading-first', file: 'src/haStates.ts', tests: ['haStates'],
    find: '  const statsAt = data.lastStatsUpdated?.() ?? null;',
    repl: '  const statsAt = data.lastStatsUpdated?.() ?? 0;',
    what: 'a feed that has never delivered anything is not called silent' },
  { id: 'blind-reason-names-the-silence', file: 'src/haStates.ts', tests: ['haStates'],
    find: '            ? `statistics feed silent ${Math.round((statsSilentMs ?? 0) / 60_000)}m — the engine is blind`',
    repl: '            ? `statistics feed silent ${Math.round(0 / 60_000)}m — the engine is blind`',
    what: 'the degraded reason reports the MEASURED silence, not a constant' },
  { id: 'evidence-sample-waits-for-settled-stats', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
  ...
  { id: 'legacy-discard-on-identity-reaches-the-log-ring', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: "    if (idNotice) this.pushEvent('engine', 'info', 'system', null, idNotice);",
    repl: '    void idNotice;',
    what: 'a ledger reloaded by an identity resume announces its discarded split too' },
  { id: 'rate-run-persisted', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '          rateRuns: runsOut,',
    repl: '',
    what: 'the rate run is saved with the snapshot' },
  { id: 'rate-run-restored-whatever-the-fine-tier', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '            rateRuns.set(id, { routeKey: v.rk, sawHundred: v.sh === 1, belowTx: bt, lastBelowTxAt: numOrNull(v.lb), rateKbps: numOrNull(v.r) });',
    repl: '',
    what: 'a restart does not forget a sustained fallback whose proof is older than the fine ring' },
  { id: 'rate-run-invalid-window-is-not-a-reading', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '  const transmitted = s.fresh && s.dTx != null && s.dTx > 0;',
    repl: '  const transmitted = s.fresh && (s.dTx == null || s.dTx > 0);',
    what: 'a fresh sample whose delta window was nulled is not a reading' },
  { id: 'rate-run-gap-from-counted-reading', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '    lastBelowTxAt: separate ? s.t : (onRoute?.lastBelowTxAt ?? s.t),',
    repl: '    lastBelowTxAt: s.t,',
    what: 'the 30 s gap runs from the previous COUNTED reading, not the previous reading' },
  { id: 'rate-run-gap-inclusive', file: 'src/zwave/evidenceStore.ts', tests: ['evidenceStore'],
    find: '  const separate = onRoute?.lastBelowTxAt == null || s.t - onRoute.lastBelowTxAt >= RATE_FALLBACK_MIN_GAP_MS;',
    repl: '  const separate = onRoute?.lastBelowTxAt == null || s.t - onRoute.lastBelowTxAt > RATE_FALLBACK_MIN_GAP_MS;',
    what: 'a reading exactly 30 s after the counted one is separate' },
  { id: 'rate-fallback-severity-from-the-reading', file: 'src/zwave/symptoms.ts', tests: ['symptoms'],
    find: "          kind: 'rate-fallback', nodeId: id, severity: rate === 9.6 ? 'warn' : 'watch',",
    repl: "          kind: 'rate-fallback', nodeId: id, severity: last?.rateKbps === 9.6 ? 'warn' : 'watch',",
    what: "a 9.6k regression stays a warning after an aborted transmission rewrites the cached rate" },
  ...
  { id: 'epoch-standdown-all', file: 'src/zwave/zwaveData.ts',
    find: '      if (this.superseded(epoch)) { await standDown(); return; }\n      // RETAIN the handles (v0.65.0).',
    repl: '      // RETAIN the handles (v0.65.0).',
  ...
  { id: 'lastseen-replay', file: 'src/zwave/zwaveData.ts',
    // Repointed v0.64.6: the stamp now moves on the three HEARING counters only.
  ...
  { id: 'probe-outcome-is-recorded', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    // Re-pointed in v0.71.0: the frame rides along.
    find: "      if (lane === 'sweep') o.onProbeResult?.(nodeId, false, cls, frame);",
    repl: "      void nodeId;",
  ...
  { id: 'sweep-lane-uses-probe', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    // Re-pointed in v0.71.0: the lane sends measure(frame), whose NoOp arm is probe().
    find: "      settleProbe(o, state, nodeId, t, measure(frame, nodeId), () => {\n        if (priorStale == null) state.lastStaleAt.delete(nodeId);",
  ...
  { id: 'verify-lane-uses-probe', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    // Re-pointed in v0.71.0: the lane sends measure(frame); its refund reads prevVerify.
  ...
  { id: 'closure-shows-its-arithmetic', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    ...
    find: "      log(`episode ${k} ${ep.verdict}${who} live=${liveM}m [before ${win(ep.before)} | after ${win(ep.after)}]${tag}${killed}`);",
    repl: "      log(`episode ${k} ${ep.verdict}${who} live=${liveM}m${tag}${killed}`);",
  ...
  { id: 'probe-flag-rides-the-probe', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    // Re-pointed in v0.71.0: the frame rides along.
    find: "      out.push({ nodeId, answered, misses, cls, lane, frame: frame ?? 'ping' });",
    repl: "      out.push({ nodeId, answered, misses, cls: 'unheard', lane, frame: frame ?? 'ping' });",
  ...
  { id: 'skip-marks-confounded', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    ...
    find: "          // spontaneous-recovery claim is dead, whatever the verdict.\n          ep.confounded = true;\n          continue;",
    repl: "          // spontaneous-recovery claim is dead, whatever the verdict.\n          continue;",
  ...
  { id: 'dead-marks-confounded', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    ...
    find: "        if (flapped || (nd && nd.status === NodeStatus.Dead)) {\n          oc.markConfounded(ep.nodeId, ep.kind);",
    repl: "        if (flapped || (nd && nd.status === NodeStatus.Dead)) {\n          void 0;",
  ...
  { id: 'confounded-tag-renders', file: 'src/zwave/outcomes.ts', tests: ['outcomes'],
    // Re-pointed in v0.71.0 (a re-route by our own read) and v0.72.0 (a pause).
    find: "          ? ' (confounded \u2014 the node died, was remediated, was re-routed by our own probe, or the checks were paused mid-episode; credited to neither arm)'",
    repl: "          ? ''",
  ...
  { id: 'reply-rate-is-sweep-only', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    // Re-pointed in v0.71.0: the frame rides along.
    find: "      if (lane === 'sweep') o.onProbeResult?.(nodeId, false, cls, frame);",
    repl: "      o.onProbeResult?.(nodeId, false, cls, frame);",
  ...
  { id: 'boot-attribution-not-credited', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    find: '      const selfProven = heardRecently && spokeOnItsOwn && attributed != null && !inReconnectBurst;',
    repl: '      const selfProven = heardRecently && spokeOnItsOwn && !inReconnectBurst;',
  ...
  { id: 'answered-path-is-sweep-only', file: 'src/zwave/autoPing.ts', tests: ['autoPing'],
    ...
    // Re-pointed in v0.71.0: the frame rides along.
    find: "        if (lane === 'sweep') o.onProbeResult?.(nodeId, true, cls, frame);",
    repl: "        o.onProbeResult?.(nodeId, true, cls, frame);",
  ...
  { id: 'engine-shows-suppression', file: 'src/telnet/screens/engine.ts', tests: ['engineScreen'],
    ...
    // Re-anchored in v0.72.0, when a pause gained its own wording.
    find: "      : ap.suppressed === 'none'\n        ? c.green('running')",
    repl: "      : (ap.suppressed as string) !== ''\n        ? c.green('running')",
  ...
  { id: 'refusal-is-classified', file: 'src/zwave/zwaveActions.ts', tests: ['zwaveActions'],
    ...
    repl: "        'transport' as ActionRefusal;",
  ...
  { id: 'refusal-reaches-the-ledger', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn'],
    find: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ true, sentAt ?? Date.now(), skip, scope, { origin });",
    repl: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ false, sentAt ?? Date.now(), skip, scope, { origin });",
  ...
  { id: 'refusal-scope-reaches-the-ledger', file: 'src/zwave/zwaveData.ts', tests: ['zwaveDataChurn', 'outcomes'],
    ...
    find: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ true, sentAt ?? Date.now(), skip, scope, { origin });",
    repl: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ true, sentAt ?? Date.now(), skip, undefined, { origin });",
  ...
  { id: 'success-attribution-stays-node-wide', file: 'src/zwave/zwaveData.ts', tests: ['outcomes', 'zwaveDataChurn'],
    ...
    find: "    this.outcomes.recordAction(nodeId, actionKind, false, sentAt ?? Date.now(), skip, undefined,",
    repl: "    this.outcomes.recordAction(nodeId, actionKind, false, sentAt ?? Date.now(), skip, refusalScope(actionKind),",
  ...
  { id: 'manual-stamp-before-call', file: 'src/zwave/zwaveActions.ts', tests: ['zwaveActions'],
    ...
    find: "      const reply = await fn();\n",
    repl: "      const reply = await fn();\n      const sentAt = now();\n",
  ...
  { id: 'manual-stamp-forwarded', file: 'src/zwave/zwaveActions.ts', tests: ['zwaveActions'],
    find: "      if (learn) o.onOutcome?.(kind, nodeId, true, undefined, origin, sentAt, aliveAtLaunch);",
    repl: "      if (learn) o.onOutcome?.(kind, nodeId, true, undefined, origin, undefined, aliveAtLaunch);",
  ...
  { id: 'manual-stamp-index-outcome', file: 'src/index.ts', tests: ['configContract'],
    find: "    onOutcome: (kind, nodeId, ok, refusal, origin, sentAt, aliveAtLaunch) =>\n      zwaveData.recordActionOutcome(kind, nodeId, ok, refusal, origin, sentAt, aliveAtLaunch),",
    repl: "    onOutcome: (kind, nodeId, ok, refusal, origin, _sentAt, aliveAtLaunch) =>\n      zwaveData.recordActionOutcome(kind, nodeId, ok, refusal, origin, undefined, aliveAtLaunch),",
  ...
  { id: 'suppression-chip-only-when-suppressed', file: 'src/telnet/chrome.ts', tests: ['chrome'],
    ...
    find: "  const apChip = shown && shown !== 'none'",
    repl: "  const apChip = shown != null",
  ...
  // ── v0.72.0: the admission rule. Nothing runs automatically; these keep
  //    that a table the type checker and the suite enforce, not a sentence.
  { id: 'admission-admits-refresh', file: 'src/zwave/admission.ts', tests: ['admission'],
    // Deleting a verb's failure rows IS admitting it — the admissible set is
    // derived from the rows, so a row deleted without the set following is red.
    find: "    verb: 'refreshValues',\n    fails: [\n      { p: 'P1', why: 'A refreshed value that corrects a stale Home Assistant state fires state-triggered automations.', source: 'ha/entity.py:106-128' },\n      { p: 'P2', why: 'Home Assistant sends the refresh with wait_for_result=False, so the call returns before anything is on the air.', source: 'zwave-js-server-python model/node/__init__.py:622-626' },\n      { p: 'P5', why: \"On 35 of the 38 nodes it is the routed read's single Get, which the verification lane already sends to both arms.\", source: 'zwave-js Node.ts:2514-2521; live CC inventory' },\n    ],\n",
    repl: "    verb: 'refreshValues',\n    fails: [],\n",
    what: 'refreshValues loses its failure rows, i.e. is admitted without review' },
  { id: 'admission-never-auto-drops-heal', file: 'src/zwave/admission.ts', tests: ['admission'],
    find: "  'healNode', 'rebuildAll', 'stopRebuild', 'removeFailed', 'reInterview', 'controlEntity', 'setConfigParam',",
    repl: "  'rebuildAll', 'stopRebuild', 'removeFailed', 'reInterview', 'controlEntity', 'setConfigParam',",
    what: 'route-deleting heal leaves the never-automatic set' },
  { id: 'admission-manual-empty', file: 'src/zwave/admission.ts', tests: ['admission'],
    find: '  return VERB_ADMISSION[v].manualBecause;',
    repl: "  return '';",
    what: 'a planned action shows no reason a person runs it' },
  { id: 'engine-header-drops-admission', file: 'src/telnet/screens/engine.ts', tests: ['engineScreen'],
    find: '`  — the engine\'s one autonomous write · automatic remediation: ${ADMISSION_SUMMARY}`',
    repl: '`  — the engine\'s one autonomous write · automatic remediation: ${ADMISSION_SUMMARY.slice(0, 4)}`',
    what: "the wide ENGINE header stops citing where the rule is decided" },
  // ── v0.72.0: the owner's pause on every autonomous write.
  { id: "pause-gate-removed", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  if (input.paused) {\n",
    repl: "  if (input.paused && nodes.length < 0) {\n",
    what: "a pause stops every lane" },
  { id: "pause-outranks-storm", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  if (booting && !input.bootDeadLane) return { ...base, suppressed: 'boot-window' };",
    repl: "  if (input.paused) return { ...base, suppressed: 'paused' };\n  if (booting && !input.bootDeadLane) return { ...base, suppressed: 'boot-window' };",
    what: "a pause ranks below storm and no-capability-data, so it cannot hide the degraded alarm" },
  { id: "pause-runner-not-read", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      paused: o.paused?.() ?? false,",
    repl: "      paused: false,",
    what: "the runner reads the pause each tick" },
  { id: "pause-not-persisted", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "        tui = { since: now(), reason: 'paused from the TUI' };\n        persist();",
    repl: "        tui = { since: now(), reason: 'paused from the TUI' };",
    what: "a TUI pause survives a restart" },
  { id: "pause-corrupt-fails-open", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "    tui = { since: now(), reason: 'pause file unreadable \u2014 paused until resumed' };",
    repl: "    tui = null;",
    what: "an unreadable pause file reads as paused" },
  { id: "pause-ha-unavailable-fails-open", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "    if (mem.last === 'off') return null;",
    repl: "    if (mem.last !== 'on') return null;",
    what: "a seen toggle reading unavailable is paused" },
  { id: "pause-ha-restart-fails-open", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      return mem.last === 'off' ? null : { since, reason:",
    repl: "      return mem.last !== 'nope' ? null : { since, reason:",
    what: "a toggle last seen on is paused until this run reads it" },
  { id: "pause-ha-off-lifts-tui", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      const prev = mem;\n      // The age is CARRIED",
    repl: "      if (state === 'off') tui = null;\n      const prev = mem;\n      // The age is CARRIED",
    what: "HA off never lifts the TUI source" },
  { id: "pause-resume-without-confirm", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "      autonomyChoice: choice,\n    };\n    this.confirmBuffer = '';",
    repl: "      autonomyChoice: choice,\n    };\n    this.confirmBuffer = '';\n    this.pendingAction = null;\n    this.executeAutonomyDecision(choice);",
    what: "the menu pause/resume rows take the typed CONFIRM" },
  { id: "pause-z-any-screen", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "      if (ev.type === 'char' && ev.ch === 'Z' && this.view.screen === 'engine') {",
    repl: "      if (ev.type === 'char' && ev.ch === 'Z') {",
    what: "Z pauses on ENGINE only" },
  { id: "pause-z-needs-write-actions", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "      if (ev.type === 'char' && ev.ch === 'Z' && this.view.screen === 'engine') {",
    repl: "      if (ev.type === 'char' && ev.ch === 'Z' && this.view.screen === 'engine' && this.actions?.enabled === true) {",
    what: "Z works with write actions off" },
  { id: "pause-readonly-locked", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    const isLocal = ip.type === 'catalog' && isLocalDecisionKind(ip.kind);",
    repl: "    const isLocal = ip.type === 'catalog' && isIdentityKind(ip.kind);",
    what: "the pause rows are answerable read-only" },
  { id: "pause-menu-not-exclusive", file: "src/telnet/actionsCatalog.ts", tests: ["actionsCatalog", "sessionActions"],
    find: "    if (d.kind === 'autonomyPause' && ctx.autonomyPausedByTui) continue;",
    repl: "    if (d.kind === 'autonomyPause' && ctx.autonomyPausedByTui && !ctx.scope) continue;",
    what: "only the row that would change something is offered" },
  { id: "pause-verify-not-cleared", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      onChange: (st) => { if (st) this.verifyOwed.clear(); },",
    repl: "      onChange: (st) => { if (st && st.since < 0) this.verifyOwed.clear(); },",
    what: "entering a pause clears what verification owed" },
  { id: "pause-request-verification-while-paused", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    if (this.autonomy.state() != null) return false;",
    repl: "    if (this.autonomy.state() != null && nodeId < 0) return false;",
    what: "nothing is owed from a paused period" },
  { id: "pause-burst-marked-when-refused", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      if (Number.isFinite(id) && this.requestVerification(id)) this.confirmBurstSent.set(key, now);",
    repl: "      if (Number.isFinite(id)) { this.requestVerification(id); this.confirmBurstSent.set(key, now); }",
    what: "a burst the pause refused is still owed after it lifts" },
  { id: "pause-no-confound", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      const paused = this.autonomy.state() != null && this.autoPingState()?.suppressed === 'paused';",
    repl: "      const paused = this.autonomy.state() != null && this.autoPingState()?.suppressed === 'paused' && now < 0;",
    what: "an episode open during a pause is confounded" },
  { id: "pause-ha-toggle-ignored", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      this.autonomy.noteHa(toggle?.state ?? null, stampOf(toggle?.last_changed), { source: 'full' });",
    repl: "      void stampOf;",
    what: "the HA toggle is read from get_states" },
  { id: "pause-overdue-never-degrades", file: "src/haStates.ts", tests: ["haStates"],
    find: "    || pauseOverdue\n",
    repl: "",
    what: "a pause past 24 h raises the degraded alarm" },
  { id: "pause-chip-no-age", file: "src/telnet/chrome.ts", tests: ["chrome"],
    find: "      (shown === 'paused' && o.pausedSince != null ? ' ' + ageWord(o.now - o.pausedSince) : ''))",
    repl: "      '')",
    what: "the pause chip carries its age" },
  { id: "index-pause-not-wired", file: "src/index.ts", tests: ["configContract"],
    find: "      paused: () => zwaveData.autonomyPause() != null,",
    repl: "      paused: () => false,",
    what: "auto-ping is handed the pause" },
  // ── v0.72.0: who acted, what died after, what a reply says.
  { id: "ledger-origin-dropped", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "            ...(meta?.origin != null ? { origin: meta.origin } : {}),",
    repl: "            ...(meta?.origin != null && atMs < 0 ? { origin: meta.origin } : {}),",
    what: "the actor is stored on the episode" },
  { id: "actor-key-without-origin", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "  const actorKey = (kind: SymptomKind, act: ActionKind, origin: ActionActor | undefined): string => `${kind}|${act}|${origin ?? 'you'}`;",
    repl: "  const actorKey = (kind: SymptomKind, act: ActionKind, origin: ActionActor | undefined): string => `${kind}|${act}|you` + (origin ? '' : '');",
    what: "the arm is split by actor" },
  { id: "efficacy-reads-pooled", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      const t = actorArms.get(actorKey(kind, act, 'you'));",
    repl: "      const t = action.get(aKey(kind, act));",
    what: "a claim is read from the operator arm only" },
  { id: "killed-after-window-zero", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (!e.alive || t < e.atMs || t > e.until) continue;",
    repl: "      if (!e.alive || t < e.atMs || t > e.atMs) continue;",
    what: "a death up to 15 min after counts" },
  { id: "killed-after-window-infinite", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (!e.alive || t < e.atMs || t > e.until) continue;",
    repl: "      if (!e.alive || t < e.atMs) continue;",
    what: "a death past 15 min does not" },
  { id: "killed-after-ignores-alive", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (!e.alive || t < e.atMs || t > e.until) continue;",
    repl: "      if (t < e.atMs || t > e.until) continue;",
    what: "the ladder pinging a Dead node never counts" },
  { id: "killed-after-counts-dead-flap", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (!ep || ep.killedAfter || kind === 'dead-flap' || nodeId == null) return;",
    repl: "      if (!ep || ep.killedAfter || nodeId == null) return;",
    what: "never for dead-flap" },
  { id: "killed-after-twice", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      ep.killedAfter = true;\n      // Counted NOW",
    repl: "      // Counted NOW",
    what: "once per episode" },
  { id: "confounded-tag-action-null-only", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "        : ep.confounded && ep.verdict !== 'unverifiable' && ep.verdict !== 'refused-misdiagnosis'",
    repl: "        : ep.confounded && ep.action == null && ep.verdict !== 'unverifiable' && ep.verdict !== 'refused-misdiagnosis'",
    what: "an attributed closure names its confound" },
  { id: "live-span-edge-strict", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "liveMin >= LIVE_SPAN_EDGES_MIN[0] ? 1 : 0;",
    repl: "liveMin > LIVE_SPAN_EDGES_MIN[0] ? 1 : 0;",
    what: "10:00 live is in the 10-minute bucket" },
  { id: "live-span-confounded-scored", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "ep.verdict === 'worse') && !ep.confounded;",
    repl: "ep.verdict === 'worse');",
    what: "a confounded closure is not scored" },
  { id: "lastat-not-set", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      lastAt: at,\n    };",
    repl: "    };",
    what: "every tally records when it last learned" },
  { id: "credit-episode-opened-after-action", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "        if ((ep.dwellStartMs ?? ep.onsetMs) > atMs) {",
    repl: "        if ((ep.dwellStartMs ?? ep.onsetMs) > atMs && atMs < 0) {",
    what: "an episode opened after the action is not credited to it" },
  { id: "reset-keeps-actor-arms", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "controlNodes.clear();\n      actorArms.clear(); actorNodes.clear(); killedAfter.clear(); routeAfter.clear(); recent.clear(); liveSpans.clear(); legacy.clear(); legacyNodes.clear();\n      // WITHOUT THIS",
    repl: "controlNodes.clear();\n      // WITHOUT THIS",
    what: "identity start-fresh clears the actor arms" },
  { id: "load-drops-actor-arms", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      for (const [k, t] of pairs(o.actorArms)) if (tallyOk(t)) actorArms.set(k, normalizeTally(t));",
    repl: "      for (const [k, t] of pairs([])) if (tallyOk(t)) actorArms.set(k, normalizeTally(t));",
    what: "the actor arms survive a restart" },
  { id: "route-after-window", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      const r = nodeId == null ? null : coveringAction(nodeId, start);",
    repl: "      const r = nodeId == null ? null : coveringAction(nodeId, start - KILLED_AFTER_MS);",
    what: "only symptoms \u226415 min after count as possible harm" },
  { id: "route-after-any-kind", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (r && ROUTE_EFFECT_KINDS.has(kind)) {",
    repl: "      if (r) {",
    what: "only route-kind symptoms count as possible harm" },
  { id: "ledger-atms-at-resolution", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    this.outcomes.recordAction(nodeId, actionKind, false, sentAt ?? Date.now(), skip, undefined,",
    repl: "    this.outcomes.recordAction(nodeId, actionKind, false, Date.now(), skip, undefined,",
    what: "the action is dated from its launch" },
  { id: "refusal-stamped-at-resolution", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ true, sentAt ?? Date.now(), skip, scope, { origin });",
    repl: "      this.outcomes.recordAction(nodeId, actionKind, /* refused */ true, Date.now(), skip, scope, { origin });",
    what: "a refusal is dated from its launch" },
  { id: "own-probe-death-charged", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "          if (!this.lastDeathOwnProbe.get(ep.nodeId)) oc.noteDeadAfterAction(ep.nodeId, ep.kind, now);",
    repl: "          oc.noteDeadAfterAction(ep.nodeId, ep.kind, now);",
    what: "a death on our own probe is not charged to the operator" },
  { id: "dead-after-not-noted", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "          if (!this.lastDeathOwnProbe.get(ep.nodeId)) oc.noteDeadAfterAction(ep.nodeId, ep.kind, now);",
    repl: "          if (!this.lastDeathOwnProbe.get(ep.nodeId) && now < 0) oc.noteDeadAfterAction(ep.nodeId, ep.kind, now);",
    what: "a death after an action reaches the ledger" },
  { id: "alive-read-after-await", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      const reply = await fn();\n",
    repl: "      const reply = await fn();\n      const aliveAtLaunch = nodeId != null && (o.statusOf?.(nodeId) ?? null) !== NodeStatus.Dead;\n",
    what: "the launch status is read before the await" },
  { id: "heal-false-is-ok", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      { interpret: boolResult(HEAL_FALSE, HEAL_TRUE_NOTE), unknownAfterMs: HEAL_TIMEOUT_MS, busy: 'healNode' }),",
    repl: "      { unknownAfterMs: HEAL_TIMEOUT_MS, busy: 'healNode' }),",
    what: "a heal that returns false is not ok" },
  { id: "rebuildall-false-is-ok", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "{ interpret: boolResult(REBUILD_FALSE) }",
    repl: "{}",
    what: "a rebuild that did not start is not ok" },
  { id: "stoprebuild-false-is-ok", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "{ interpret: boolResult(STOP_FALSE) }",
    repl: "{}",
    what: "a stop with nothing to stop says so" },
  { id: "config-queued-mains-is-ok", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "    return listening === false",
    repl: "    return listening !== null",
    what: "queued on a mains device is not confirmed" },
  { id: "config-queued-is-plain-ok", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "  if (status === 'queued') {",
    repl: "  if (status === 'queued' && listening === undefined) {",
    what: "queued is read, not passed over" },
  { id: "heal-default-timeout", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "() => deviceCmd('zwave_js/rebuild_node_routes', n, HEAL_TIMEOUT_MS)",
    repl: "() => deviceCmd('zwave_js/rebuild_node_routes', n)",
    what: "a heal waits 20 min for its reply" },
  { id: "remove-default-timeout", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "() => deviceCmd('zwave_js/remove_failed_node', n, REMOVE_FAILED_TIMEOUT_MS)",
    repl: "() => deviceCmd('zwave_js/remove_failed_node', n)",
    what: "a removal waits 3 min for its reply" },
  { id: "ready-timeout-is-reply", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "{ readyMs: READY_TIMEOUT_MS, replyMs }",
    repl: "{ readyMs: replyMs, replyMs }",
    what: "the socket wait stays 10 s" },
  { id: "haws-split-ignored", file: "src/ha/haWsClient.ts", tests: ["haWsClient"],
    find: "    const readyMs = typeof timeoutMs === 'number' ? timeoutMs : timeoutMs.readyMs;",
    repl: "    const readyMs = typeof timeoutMs === 'number' ? timeoutMs : timeoutMs.replyMs;",
    what: "send bounds the socket apart from the reply" },
  { id: "timeout-called-failed", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      if (opts.unknownAfterMs != null && (/^HA WS timeout/.test(msg) || /^HA WS connection closed/.test(msg))) {",
    repl: "      if (opts.unknownAfterMs != null && msg === '') {",
    what: "a heal HA stopped waiting for is unknown" },
  { id: "closed-called-failed", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "(/^HA WS timeout/.test(msg) || /^HA WS connection closed/.test(msg))) {",
    repl: "(/^HA WS timeout/.test(msg))) {",
    what: "a dropped socket mid-heal is unknown too" },
  { id: "unknown-fires-removed", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      if (res.ok) o.onNodeRemoved?.(n);",
    repl: "      if (res.ok || res.unknown) o.onNodeRemoved?.(n);",
    what: "an unknown removal never forgets the node" },
  { id: "busy-not-cleared", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      else inFlight.delete(seq);",
    repl: "      else void seq;",
    what: "the in-flight flag clears in a finally" },
  { id: "busy-not-set", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "    if (opts.busy && nodeId != null) inFlight.set(seq, { kind: opts.busy, nodeId, since: sentAt, until: null });",
    repl: "    if (opts.busy && nodeId == null) inFlight.set(seq, { kind: opts.busy, nodeId: -1, since: sentAt, until: null });",
    what: "a one-node rebuild is in flight while it runs" },
  { id: "operator-busy-not-suppressing", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  if (input.operatorBusy) return { ...base, suppressed: gate('operator-action') };",
    repl: "  if (input.operatorBusy && nodes.length < 0) return { ...base, suppressed: gate('operator-action') };",
    what: "auto-ping stands down for a one-node rebuild" },
  { id: "index-operator-busy-not-wired", file: "src/index.ts", tests: ["configContract"],
    find: "      operatorBusy: () => actions.operatorActionInFlight() != null,",
    repl: "      operatorBusy: () => false,",
    what: "auto-ping is handed the in-flight flag" },
  { id: "session-no-detach", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "          this.detached.add(this.actionSeq);\n",
    repl: "",
    what: "a detached action\u2019s late result stays in the Log" },
  { id: "session-detach-ignored", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    if (this.detached.delete(seq)) return;",
    repl: "    this.detached.delete(seq);",
    what: "a late result does not replace a newer card" },
  { id: "engine-ladder-row-claimed", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "        c.grey('shown, never claimed'),",
    repl: "        c.grey(''),",
    what: "the ladder arm says it is never claimed" },
  { id: "engine-pooled-row-unsplit", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "      return p != null && p.legacyN >= 0.5 ? { a, n: p.legacyN, nodes: p.legacyNodes } : null;",
    repl: "      return p != null && p.n >= 0.5 ? { a, n: p.n, nodes: p.legacyNodes } : null;",
    what: "the pre-actor row shows the decayed snapshot, not the whole pooled arm" },
  { id: "engine-your-deaths-hidden", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    for (const [a, k] of yourDeaths) bit(c.yellow(`${a}: Dead \u226415m after \u00d7${k}`), 0);",
  ...
  { id: "recommend-persist-zero", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    ...
    what: "nothing is named before it has lasted an hour" },
  { id: "recommend-automatic-true", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "    automatic: false,",
    repl: "    automatic: i.writeActions,",
    what: "automatic is false in every state" },
  { id: "recommend-extra-attribute", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "    watching_since: iso(i.startedAt),\n  });",
    repl: "    watching_since: iso(i.startedAt),\n    extra: null,\n  } as Recommendation['attrs']);",
    what: "the attribute set is exactly the allowlist" },
  { id: "recommend-summons-after", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "  if (given.length > 0) {",
    repl: "  if (given.length > 0 && i.symptoms.length === 0) {",
    what: "a give-up outranks every symptom" },
  { id: "recommend-ignores-ladder", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "  const step = firstStep(plan.candidates, worked);",
    repl: "  const step = firstStep(plan.candidates, worked && i.now < 0);",
    what: "a device the ladder is still working is physical" },
  { id: "recommend-leads-destructive", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "  return cands.find((c) => c.blocked == null && c.cost !== 'destructive' && (!physicalOnly || c.action == null)) ?? null;",
    repl: "  return cands.find((c) => c.blocked == null && (!physicalOnly || c.action == null)) ?? null;",
    what: "a notification never leads with an irreversible step" },
  { id: "recommend-subsumed-counted", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "    .filter((s) => s.subsumedBy == null && i.now - s.sinceMs >= RECOMMEND_PERSIST_MS)",
    repl: "    .filter((s) => i.now - s.sinceMs >= RECOMMEND_PERSIST_MS)",
    what: "a subsumed symptom is not named" },
  { id: "recommend-rank-by-age-only", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "    .sort((x, y) => SEVERITY_RANK[x.severity] - SEVERITY_RANK[y.severity] || x.sinceMs - y.sinceMs ||",
    repl: "    .sort((x, y) => x.sinceMs - y.sinceMs ||",
    what: "severity ranks before age" },
  { id: "recommend-not-published", file: "src/haStates.ts", tests: ["haStates"],
    find: "      return { entity: ENTITY_RECOMMENDATION, state: r.state, attrs: { ...r.attrs } };",
    repl: "      return { entity: ENTITY_ROUTE_FAILURES, state: r.state, attrs: { ...r.attrs } };",
    what: "the recommendation is published under its own id" },
  { id: "recommend-gate-not-passed", file: "src/haStates.ts", tests: ["haStates"],
    find: "    for (const s of buildStates(opts.data, Date.now(), { writeActions: opts.writeActions, startedAt })) {",
    repl: "    for (const s of buildStates(opts.data, Date.now(), { writeActions: false, startedAt })) {",
    what: "the publisher passes the master gate" },
  { id: "index-recommend-gate", file: "src/index.ts", tests: ["configContract"],
    find: "    writeActions: config.writeActions,\n    startedAt: bootedAt,",
    repl: "    writeActions: false,\n    startedAt: bootedAt,",
    what: "index hands the publisher the master gate" },
  // ── v0.72.0 review fixes.
  { id: "skip-dated-from-launch", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      return absentSince != null && absentSince <= launchedAt;",
    repl: "      return absentSince != null;",
    what: "a symptom that cleared while the action ran is credited to it" },
  { id: "hold-while-running", file: "src/zwave/outcomes.ts", tests: ["outcomes", "zwaveDataChurn"],
    find: "    const start = held == null ? since : Math.max(since, held);",
    repl: "    const start = since;",
    what: "an episode waits for an action running on its node, and confirms from its settle" },
  { id: "hold-wired", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      (nodeId) => this.holdRelease(nodeId, now));",
    repl: "      () => null);",
    what: "the data layer holds episodes for running actions" },
  { id: "launch-hook-before-await", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "    if (learn) o.onLaunch?.(kind, nodeId, origin, sentAt, aliveAtLaunch);\n    try {\n      const reply = await fn();",
    repl: "    try {\n      const reply = await fn();\n      if (learn) o.onLaunch?.(kind, nodeId, origin, sentAt, aliveAtLaunch);",
    what: "the ledger is told of a launch before the reply" },
  { id: "unknown-holds-stand-down", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      if (e && unknownOutcome && opts.unknownAfterMs != null) e.until = sentAt + opts.unknownAfterMs;",
    repl: "      if (e && unknownOutcome && opts.unknownAfterMs != null && seq < 0) e.until = sentAt + opts.unknownAfterMs;",
    what: "an unknown heal keeps auto-ping stood down until its reply bound" },
  { id: "unknown-settles-at-bound", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "          unknownOutcome && opts.unknownAfterMs != null ? Math.max(now(), sentAt + opts.unknownAfterMs) : now(), effect);",
    repl: "          now(), effect);",
    what: "an unknown outcome settles at its reply bound" },
  { id: "alive-is-strict", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "    const aliveAtLaunch = st === NodeStatus.Alive;",
    repl: "    const aliveAtLaunch = st != null && st !== NodeStatus.Dead;",
    what: "Unknown status is not evidence of life" },
  { id: "death-charged-to-latest-action", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      const a = coveringAction(nodeId, atMs);",
    repl: "      const a = ep.action ? { ...ep.action, alive: ep.action.aliveAtAction === true, until: ep.action.atMs + KILLED_AFTER_MS, origin: ep.action.origin ?? ('you' as ActionActor) } : null;",
    what: "a death is charged to the latest action, not the first" },
  { id: "launch-opens-harm-window", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      putLaunch(nodeId, { kind: actionKind, atMs, origin: meta.origin, alive: meta.alive, until: Infinity });",
    repl: "      void meta;",
    what: "harm windows open at the launch" },
  { id: "settle-closes-harm-window", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (r) { r.settledAt = settledAt; r.until = settledAt + KILLED_AFTER_MS; }",
    repl: "      if (r) { r.settledAt = settledAt; r.until = Infinity; }",
    what: "harm windows close 15 min after the settle" },
  { id: "harm-from-breach-start", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      const start = openOpts?.dwellStartMs ?? onsetMs;",
    repl: "      const start = onsetMs;",
    what: "route harm is judged from the breach, not dwell maturity" },
  { id: "confound-from-breach-start", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "        if ((ep.dwellStartMs ?? ep.onsetMs) > atMs) {",
    repl: "        if (ep.onsetMs > atMs) {",
    what: "a breach older than the action is attributed, not confounded" },
  { id: "route-harm-needs-alive", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      putLaunch(nodeId, { kind: actionKind, atMs, origin: meta.origin, alive: meta.alive, until: Infinity });",
    repl: "      putLaunch(nodeId, { kind: actionKind, atMs, origin: meta.origin, alive: true, until: Infinity });",
    what: "reviving a Dead node is not harm: the launch records whether it was alive" },
  { id: "legacy-snapshot-on-old-file", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (o.actorArms === undefined) {",
    repl: "      if (o.actorArms === undefined && action.size < 0) {",
    what: "an older file seeds the pre-actor share" },
  { id: "legacy-decays-with-pooled", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "          else legacy.set(ak, { n: lg.n * keep, ok: lg.ok * keep, bad: lg.bad * keep });",
    repl: "          else legacy.set(ak, lg);",
    what: "the pre-actor share decays with the pooled arm" },
  { id: "optional-load-guarded", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      const tallyOk = (t: unknown): t is Tally => t != null && typeof t === 'object' && validTally(t as Tally);",
    repl: "      const tallyOk = (t: unknown): t is Tally => validTally(t as Tally);",
    what: "one malformed entry does not wipe the ledger" },
  { id: "flirs-listens-for-commands", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    if (f.isListening === true || f.isFrequentListening === true) return true;",
    repl: "    if (f.isListening === true) return true;",
    what: "a FLiRS device listens for commands" },
  { id: "index-listening-wired", file: "src/index.ts", tests: ["configContract"],
    find: "    listeningOf: (n) => zwaveData.listensForCommands(n),",
    repl: "    listeningOf: (n) => provider.nodeById(n)?.isListening ?? null,",
    what: "the runner reads FLiRS as listening" },
  { id: "index-launch-wired", file: "src/index.ts", tests: ["configContract"],
    find: "    onLaunch: (kind, nodeId, origin, at, alive) => zwaveData.noteActionLaunched(kind, nodeId, origin, at, alive),",
    repl: "    onLaunch: () => {},",
    what: "the launch reaches the ledger" },
  { id: "operator-action-below-storm", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  if (controller?.isRebuildingRoutes) return { ...base, suppressed: gate('rebuilding-routes') };\n",
    repl: "  if (controller?.isRebuildingRoutes) return { ...base, suppressed: gate('rebuilding-routes') };\n  if (input.operatorBusy) return { ...base, suppressed: gate('operator-action') };\n",
    what: "a chain of heals cannot hide a storm" },
  { id: "paused-after-dead-pass", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  if (input.operatorBusy) return { ...base, suppressed: gate('operator-action') };",
    repl: "  if (input.operatorBusy) return { ...base, suppressed: gate('operator-action') };\n  if (input.paused) return { ...base, suppressed: 'paused' };",
    what: "a pause does not hold back a summons" },
  { id: "pause-ha-age-first-sight", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      if (nowPausing && mem.since == null) mem.since = t;",
    repl: "      if (nowPausing && mem.since == null) mem.since = at;",
    what: "the HA pause is aged from first sight, not last_changed" },
  { id: "pause-ha-age-kept", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      mem = { last: state, at, since: prev?.since ?? null };",
    repl: "      mem = { last: state, at, since: null };",
    what: "the HA pause keeps its age through readings" },
  { id: "pause-ha-stale-ignored", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      if (state != null && source === 'full' && orderFloor != null && at < orderFloor) {",
    repl: "      if (state != null && source === 'full' && orderFloor != null && at < 0) {",
    what: "a stale snapshot cannot undo a newer event" },
  { id: "pause-ha-absence-not-deletion", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      if (state == null && source === 'event') {",
    repl: "      if (state == null) {",
    what: "absence from a full read is not a deletion" },
  { id: "pause-unreadable-persisted", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "    // restart instead of starting again from zero every time.\n    persist();",
    repl: "    // restart instead of starting again from zero every time.",
    what: "an unreadable pause keeps its age" },
  { id: "recommend-worked-needs-running", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "  const worked = s.kind === 'node-down' && i.ap?.suppressed === 'none' && (",
    repl: "  const worked = s.kind === 'node-down' && (",
    what: "a paused ladder is not working a node" },
  { id: "recommend-no-per-minute-attribute", file: "src/zwave/recommendation.ts", tests: ["recommendation"],
    find: "    a.since = iso(s.sinceMs);",
    repl: "    a.since = iso(s.sinceMs); (a as Record<string, unknown>).persisting_min = Math.floor((i.now - s.sinceMs) / 60_000);",
    what: "no attribute moves every minute" },
  { id: "overdue-needs-a-biting-pause", file: "src/haStates.ts", tests: ["haStates"],
    find: "  const pauseOverdue = pause != null && ap != null && ap.suppressed !== 'disabled' &&\n    ap.suppressed !== 'write-actions-off' && data.autonomyPauseOverdue(now);",
    repl: "  const pauseOverdue = pause != null && data.autonomyPauseOverdue(now);",
    what: "a pause that stops nothing raises nothing" },
  { id: "engine-pause-read-directly", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    const showsPause = pause != null && (ap.suppressed === 'paused' || ap.suppressed === 'none');",
    repl: "    const showsPause = pause != null && ap.suppressed === 'paused';",
    what: "ENGINE shows the pause before the next pass" },
  { id: "chip-pause-read-directly", file: "src/telnet/chrome.ts", tests: ["chrome"],
    find: "  const pausedNow = o.pausedSince != null && (o.apSuppressed === 'none' || o.apSuppressed === 'paused');",
    repl: "  const pausedNow = o.pausedSince != null && o.apSuppressed === 'paused';",
    what: "the chip shows the pause before the next pass" },
  { id: "z-before-working-card", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "      if (ev.type === 'char' && ev.ch === 'Z' && this.view.screen === 'engine') {\n        this.pauseFromKey();",
    repl: "      if (ev.type === 'char' && ev.ch === 'Z' && this.view.screen === 'engine' && !this.actionInFlight && this.actionNotice == null) {\n        this.pauseFromKey();",
    what: "Z pauses over the working and result cards" },
  { id: "card-shows-note", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    this.actionNoticeDetail = [note, this.pauseAckText()].filter(Boolean).join(' \u00b7 ') || null;",
    repl: "    this.actionNoticeDetail = [this.pauseAckText()].filter(Boolean).join(' \u00b7 ') || null;",
    what: "the result card carries the reply note" },
  { id: "card-unknown-not-red", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    this.actionNotice = res.ok ? `\u2713  ${action.label}` : res.unknown ? `\u25f7  ${res.message}` : `\u2717  ${res.message}`;",
    repl: "    this.actionNotice = res.ok ? `\u2713  ${action.label}` : `\u2717  ${res.message}`;",
    what: "an unknown outcome is not shown as a failure" },
  // ── v0.72.0 second review.
  { id: "effect-maybe-confounds", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      if (this.outcomes) for (const ep of this.outcomes.openEpisodes()) if (ep.nodeId === nodeId) this.outcomes.markConfounded(ep.nodeId, ep.kind);",
    repl: "      void 0;",
    what: "an action that reached the driver without success confounds its node's episodes" },
  { id: "effect-ok-set", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      effect = 'ok';\n",
    repl: "",
    what: "a success settles as ok" },
  { id: "effect-none-for-unsent", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "      if (refusal === 'refused' || NOT_SENT.test(msg)) effect = 'none';",
    repl: "      if (refusal === 'refused') effect = 'none';",
    what: "a call that never left changes nothing" },
  { id: "index-effect-wired", file: "src/index.ts", tests: ["configContract"],
    find: "settledAt, effect) => zwaveData.noteActionSettled(kind, nodeId, origin, at, settledAt, effect),",
    repl: "settledAt, effect) => zwaveData.noteActionSettled(kind, nodeId, origin, at, settledAt),",
    what: "the settle effect reaches the data layer" },
  { id: "overlap-confounds", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    if (others) {",
    repl: "    if (others && replyAt < 0) {",
    what: "overlapping actions on a node credit neither" },
  { id: "mesh-rebuild-holds", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    if (meshRebuilding) this.meshRebuildUntil = Infinity;",
    repl: "    if (meshRebuilding && now < 0) this.meshRebuildUntil = Infinity;",
    what: "a mesh-wide rebuild holds every episode" },
  { id: "mesh-rebuild-confounds", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "        if (paused || meshRebuilding) oc.markConfounded(ep.nodeId, ep.kind);",
    repl: "        if (paused) oc.markConfounded(ep.nodeId, ep.kind);",
    what: "a mesh-wide rebuild confounds every episode" },
  { id: "mesh-rebuild-release-timed", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    else if (this.meshRebuildUntil === Infinity) this.meshRebuildUntil = now;",
    repl: "    else if (this.meshRebuildUntil === Infinity) this.meshRebuildUntil = null;",
    what: "confirmation restarts when a mesh rebuild ends" },
  { id: "burst-forgotten-while-held", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      if (sentAt != null && held != null && held > sentAt) this.confirmBurstSent.delete(key);",
    repl: "      if (sentAt != null && held != null && held > sentAt && now < 0) this.confirmBurstSent.delete(key);",
    what: "a burst sent before an action settled is sent again after it \u2014 however quick the action" },
  { id: "burst-timed-from-release", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      if (!confirmBurstDue(held == null ? since : Math.max(since, held), now, CONFIRM_MS, WINDOW_MS)) continue;",
    repl: "      if (!confirmBurstDue(since, now, CONFIRM_MS, WINDOW_MS)) continue;",
    what: "the burst is timed from the settle" },
  { id: "legacy-nodes-snapshot", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "        for (const [k, set] of armNodes) legacyNodes.set(k, set.size);",
    repl: "        void armNodes;",
    what: "the pre-actor row keeps its snapshot node count" },
  { id: "load-any-field-guarded", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      for (const [k, t] of pairs(o.control)) if (tallyOk(t)) control.set(k as SymptomKind, normalizeTally(t));",
    repl: "      for (const [k, t] of ((o.control ?? []) as [string, Tally][])) if (validTally(t)) control.set(k as SymptomKind, normalizeTally(t));",
    what: "a malformed entry in an older field drops only itself" },
  { id: "recent-per-launch", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "    m.set(r.atMs, r);\n    recent.set(nodeId, m);",
    repl: "    m.clear();\n    m.set(r.atMs, r);\n    recent.set(nodeId, m);",
    what: "a quick action does not replace a long one's harm window" },
  { id: "missing-toggle-forgotten-by-resume", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      const forgetMissing = mem?.last === 'missing';",
    repl: "      const forgetMissing = false;",
    what: "a TUI resume forgets a missing toggle" },
  { id: "order-floor-per-connection", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "    newConnection(): void {\n      orderFloor = null;",
    repl: "    newConnection(): void {\n      void orderFloor;",
    what: "readings are ordered within one connection" },
  { id: "new-connection-wired", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "      this.autonomy.newConnection();",
    repl: "      void 0;",
    what: "a reconnect resets the toggle ordering" },
  { id: "paused-spent-read-gives-up", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "    return { ...base, gaveUp: [...gaveUp, ...spent], launchGaveUp, talkingWhileDead, suppressed: 'paused' };",
    repl: "    return { ...base, gaveUp: [...gaveUp, ...spent.slice(0, 0)], launchGaveUp, talkingWhileDead, suppressed: 'paused' };",
    what: "paused, a spent node with an owed read still gives up" },
  { id: "chip-resume-lag", file: "src/telnet/chrome.ts", tests: ["chrome"],
    find: "  const shown = pausedNow ? 'paused' : o.apSuppressed === 'paused' ? null : o.apSuppressed;",
    repl: "  const shown = pausedNow ? 'paused' : o.apSuppressed;",
    what: "no PAUSED chip after a resume" },
  { id: "engine-first-pass-pause", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    if (pause != null) push('  ' + c.yellow(pick(view.cols - 2, pauseLines(pause, 'automatic writes paused'))));",
    repl: "    if (pause != null && now < 0) push('  ' + c.yellow(pick(view.cols - 2, pauseLines(pause, 'automatic writes paused'))));",
    what: "the pause shows before auto-ping's first pass" },
  { id: "engine-hint-short-forms", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    const ha = p.by.includes('ha') && !p.haMissing ? (short ? 'the HA toggle off' : `turn off ${PAUSE_ENTITY}`) : null;",
    repl: "    const ha = p.by.includes('ha') && !p.haMissing ? `turn off ${PAUSE_ENTITY}` : null;",
    what: "the resume hint fits 80 columns" },
  { id: "pause-ack-shown", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "        ...(this.pauseAckText() ? ['', c.yellow(this.pauseAckText()!)] : [])]);",
    repl: "        ...([] as string[])]);",
    what: "a pause taken over the WORKING card is acknowledged there" },
  { id: "pause-ack-busy-path", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    if (this.actionInFlight || this.pendingAction != null) {",
    repl: "    if (this.actionInFlight && this.pendingAction === undefined) {",
    what: "a pause during an action is not lost under its result" },
  { id: "overdue-transient", file: "src/haStates.ts", tests: ["haStates"],
    find: "  const pauseOverdue = pause != null && ap != null && ap.suppressed !== 'disabled' &&",
    repl: "  const pauseOverdue = pause != null && ap != null && ap.suppressed !== 'operator-action' && ap.suppressed !== 'disabled' &&",
    what: "a transient suppressor does not clear an overdue pause" },
  // ── v0.72.0 third review.
  { id: "covering-prefers-running", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      if (e.settledAt == null || t <= e.settledAt) {",
    repl: "      if (e.settledAt == null && t < 0) {",
    what: "a death during a running heal is the heal's, not a quick verb's inside it" },
  { id: "drop-unsent-launch", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "    if (effect === 'none') {",
    repl: "    if (effect === 'none' && nodeId < 0) {",
    what: "a launch that never left holds nothing" },
  { id: "drop-launch-store", file: "src/zwave/outcomes.ts", tests: ["outcomes"],
    find: "      recent.get(nodeId)?.delete(atMs);",
    repl: "      void atMs;",
    what: "a launch that never left opens no harm window" },
  { id: "maybe-until-confounds", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "          if (now <= mu) oc.markConfounded(ep.nodeId, ep.kind);",
    repl: "          if (now <= mu && now < 0) oc.markConfounded(ep.nodeId, ep.kind);",
    what: "an episode opening while an unknown heal may still run is confounded" },
  { id: "not-sent-client-errors", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "^HA WS not ready|^HA WS not open|^HA WS client not configured|",
    repl: "^HA WS not ready|",
    what: "the HA client's own pre-send failures are \"none\"" },
  { id: "overview-shared-masthead", file: "src/telnet/screens/overview.ts", tests: ["overviewScreen"],
    find: "  out.push(masthead(view, mastheadOpts(data)));",
    repl: "  out.push(masthead(view, { ...mastheadOpts(data), pausedSince: null }));",
    what: "OVERVIEW shows the PAUSED chip" },
  { id: "storm-reason-first", file: "src/haStates.ts", tests: ["haStates"],
    find: "              : pauseOverdue && ap?.suppressed !== 'storm' && ap?.suppressed !== 'no-capability-data'",
    repl: "              : pauseOverdue",
    what: "a storm names itself under an overdue pause" },
  { id: "paused-give-up-awaits-answer", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      (input.state.awaitingAnswer.get(id)?.length ?? 0) === 0);",
    repl: "      true);",
    what: "a paused give-up waits out a probe's answer grace" },
  { id: "resume-offered-when-missing", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "(this.data.autonomyPause()?.by.includes('tui') || this.data.autonomyPause()?.haMissing === true) ?? false,",
    repl: "(this.data.autonomyPause()?.by.includes('tui')) ?? false,",
    what: "Resume is offered for a missing toggle" },
  { id: "ack-from-current-state", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "    const p = this.pauseAckPending ? this.data.autonomyPause() : null;",
    repl: "    const p = this.pauseAckPending ? (this.data.autonomyPause() ?? { since: 0 }) : null;",
    what: "a stale pause acknowledgement is never shown" },
  { id: "confirm-box-ack", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "        notice: this.pauseAckText() ?? undefined,",
    repl: "        notice: undefined,",
    what: "Z over a CONFIRM box is acknowledged on it" },
  { id: "events-never-dropped", file: "src/zwave/autonomyPause.ts", tests: ["autonomyPause"],
    find: "      if (state != null && source === 'full' && orderFloor != null && at < orderFloor) {",
    repl: "      if (state != null && orderFloor != null && at < orderFloor) {",
    what: "a live event is never dropped on its stamp" },
  { id: "engine-40-col-form", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    `paused \u00b7 resume: ${p.by.includes('ha') && !p.haMissing ? (p.by.includes('tui') ? '3 \u2192 A + HA toggle' : 'HA toggle off') : 'Controller 3 \u2192 A'}`,\n",
    repl: "",
    what: "every pause line fits 40 columns" },
  { id: "missing-exit-via-menu", file: "src/telnet/screens/engine.ts", tests: ["engineScreen"],
    find: "    const viaMenu = p.by.includes('tui') || p.haMissing === true;",
    repl: "    const viaMenu = p.by.includes('tui');",
    what: "the hint names the menu exit for a missing toggle" },
  ...
  { id: 'result-modal-wraps', file: 'src/telnet/session.ts', tests: ['sessionActions'],
    ...
    find: "        ...fit(this.actionNotice).map((l) => (ok ? c.green : pending ? c.yellow : c.red)(l)),",
    repl: "        (ok ? c.green : pending ? c.yellow : c.red)(this.actionNotice),",
  ...
  { id: "routed-read-needs-a-readable-value", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "    if (input.state.readOwed.has(n.nodeId) && (input.canRead?.(n.nodeId) ?? false) &&",
    repl: "    if (input.state.readOwed.has(n.nodeId) &&",
    what: "a node with no switch/light value never gets a routed read" },
  { id: "routed-read-revivals-are-capped", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "        readRevivalsWithin(input.state, n.nodeId, now) < READ_REVIVAL_CAP) {",
    repl: "        true) {",
    what: "past the revival cap the read is withheld, so a kill–revive loop ends in a summons" },
  { id: "routed-read-only-after-a-ladder-miss", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      if (lane === 'dead' && !answered) state.readOwed.add(nodeId);",
    repl: "      if (!answered) state.readOwed.add(nodeId);",
    what: "a sweep or verification miss never earns a routed read" },
  { id: "routed-read-once-per-rung", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      state.readOwed.delete(nodeId);",
    repl: "      void 0;",
    what: "one routed read per unanswered ping, not one per tick" },
  { id: "routed-read-launch-failure-not-retried", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      }, 'no switch/light value to read, or transport error');",
    repl: "        state.readOwed.add(nodeId);\n      }, 'no switch/light value to read, or transport error');",
    what: "a read that never left is not re-sent every tick" },
  { id: "routed-read-launch-failure-uncounted", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "        state.reads.set(nodeId, Math.max(0, (state.reads.get(nodeId) ?? 1) - 1));\n      }, 'no switch/light value",
    repl: "        void 0;\n      }, 'no switch/light value",
    what: "a read that never left is not counted as sent" },
  { id: "routed-read-in-the-boot-ladder", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "if (booting) return { ...base, ping, read, gaveUp,",
    repl: "if (booting) return { ...base, ping, gaveUp,",
    what: "with the dead lane released in the boot window, the read goes out like the ping" },
  { id: "routed-read-in-the-decision", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "return { ...base, ping, read, stale: staleDeduped,",
    repl: "return { ...base, ping, stale: staleDeduped,",
    what: "an owed read reaches the runner" },
  { id: "routed-read-cleared-on-recovery", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      state.readOwed.delete(n.nodeId);",
    repl: "      void 0;",
    what: "a recovered node owes no read" },
  { id: "routed-read-revivals-span-episodes", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      state.reads.delete(n.nodeId);",
    repl: "      state.reads.delete(n.nodeId); state.readRevivals.delete(n.nodeId);",
    what: "revival history survives recovery — that is what bounds the loop" },
  { id: "routed-read-revivals-leave-with-the-node", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "  for (const id of [...state.readRevivals.keys()]) if (!seen.has(id)) state.readRevivals.delete(id);",
    repl: "",
    what: "a device re-included on the same id starts with no revival history" },
  { id: "routed-read-reowed-after-blackout", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "      if (unjudgeable && p.lane === 'read') {",
    repl: "      if (unjudgeable && p.lane === 'dead') {",
    what: "a read the RF blackout swallowed is owed again; a ping is not" },
  { id: "routed-read-give-up-says-so", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "        : reads > 0\n",
    repl: "        : false\n",
    what: "the summons names the routed reads the node ignored" },
  { id: "routed-read-cap-warns-once", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "          if (revivals.length === READ_REVIVAL_CAP) {",
    repl: "          if (revivals.length > READ_REVIVAL_CAP) {",
    what: "the kill–revive loop is named as it is capped" },
  { id: "routed-read-revival-recorded", file: "src/zwave/autoPing.ts", tests: ["autoPing"],
    find: "          state.readRevivals.set(nodeId, revivals);",
    repl: "          void revivals;",
    what: "a routed read that revived the node is counted" },
  { id: "routed-read-is-one-value", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "service_data: { entity_id: ent, refresh_all_values: false } });",
    repl: "service_data: { entity_id: ent, refresh_all_values: true } });",
    what: "a routed read is ONE Get, not every value of the entity" },
  { id: "routed-read-is-refresh-value", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "domain: 'zwave_js', service: 'refresh_value',",
    repl: "domain: 'zwave_js', service: 'refresh_values',",
    what: "the read goes through refresh_value (Node.pollValue), which is sent to a Dead node" },
  { id: "routed-read-never-learned", file: "src/zwave/zwaveActions.ts", tests: ["zwaveActions"],
    find: "refresh_all_values: false } });\n      }, /* learn: never, see above */ false, /* origin */ 'engine'),",
    repl: "refresh_all_values: false } });\n      }, /* learn: never, see above */ true, /* origin */ 'engine'),",
    what: "the read never enters the outcome ledger" },
  { id: "read-entity-is-a-currentValue", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "/^\\d+\\.(\\d+)-(37|38)-(\\d+)-currentValue$/",
    repl: "/^\\d+\\.(\\d+)-(37|38|112)-(\\d+)-/",
    what: "only a Binary/Multilevel Switch currentValue is a value to read" },
  { id: "read-entity-belongs-to-the-node", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "if (rv && Number(rv[1]) === nodeId && (domain === 'switch' || domain === 'light')) {",
    repl: "if (rv && (domain === 'switch' || domain === 'light')) {",
    what: "a value id naming another node is refused" },
  { id: "read-entity-lowest-endpoint", file: "src/zwave/zwaveData.ts", tests: ["zwaveDataChurn"],
    find: "if (prev == null || ep < prev) {",
    repl: "if (true) {",
    what: "the read targets the lowest endpoint, whatever the registry order" },
  { id: "dead-refresh-blocked-in-the-plan", file: "src/zwave/planner.ts", tests: ["planner"],
    find: "node?.status === NodeStatus.Dead ? 'node is Dead",
    repl: "node?.status === NodeStatus.Asleep ? 'node is Dead",
    what: "the plan never offers a refresh that would send nothing" },
  { id: "dead-refresh-refused-in-the-session", file: "src/telnet/session.ts", tests: ["sessionActions"],
    find: "?.status === NodeStatus.Dead\n          ? { ok: false, message: `node ${action.nodeId} is Dead",
    repl: "?.status === NodeStatus.Asleep\n          ? { ok: false, message: `node ${action.nodeId} is Dead",
    what: "a manual refresh of a Dead node says it would send nothing" },
  /* ── v0.69.0 ─────────────────────────────────────────────────────────── */
  ...
  /* ── v0.71.0: the measurement lanes read; our own kills are contained ── */
  { id: 'probe-death-measurement-lanes-only', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  if (newest.lane === 'sweep' || newest.lane === 'verify') {\n    state.probeDeath.set(nodeId, newest.lane);",
    repl: "  if (newest.lane !== 'dead' && newest.lane !== 'read') {\n    state.probeDeath.set(nodeId, newest.lane as 'sweep' | 'verify');",
    what: "a MANUAL kill keeps the dwell and is not counted as our own probe kill" },
  { id: 'probe-death-covers-verify', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  if (newest.lane === 'sweep' || newest.lane === 'verify') {",
    repl: "  if (newest.lane === 'sweep') {",
    what: "a verification kill is retried at once \u2014 the 101-minute outage of 2026-09-22" },
  { id: 'probe-death-names-the-lane', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "killedBy === 'verify' ? 'verification' : 'sweep'",
    repl: "'sweep'",
    what: "the retry line names the lane that killed the node" },
  { id: 'measurement-lanes-read', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "(o.canRead?.(id) ?? false) && !readFallbackActive(state, id, at) ? 'read' : 'ping';",
    repl: "(o.canRead?.(id) ?? false) && !readFallbackActive(state, id, at) ? 'ping' : 'ping';",
    what: "the sweep and verification lanes send the routed read, not the killing NoOp" },
  { id: 'measurement-read-needs-a-readable-value', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "o.probeRead != null && (o.canRead?.(id) ?? false) && !readFallbackActive(state, id, at)",
    repl: "o.probeRead != null && !readFallbackActive(state, id, at)",
    what: "a node with no switch/light value keeps the NoOp rather than a read that cannot be sent" },
  { id: 'measurement-read-fallback-honoured', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "&& !readFallbackActive(state, id, at) ? 'read'",
    repl: "? 'read'",
    what: "a node whose read could not be sent falls back to the NoOp, so the sweep does not stall on it" },
  { id: 'measurement-read-goes-to-probeRead', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "frame === 'read' ? o.probeRead!(id) :",
    repl: "frame === 'read' ? o.read!(id) :",
    what: "the measurement read is logged as a probe, never as the ladder read" },
  { id: 'measurement-ping-is-the-probe', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: ": (o.probe ?? o.ping)(id);",
    repl: ": o.ping(id);",
    what: "the NoOp fallback is the non-learning probe, never the learning ping" },
  { id: 'sweep-probe-stays-lane-sweep', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      pendProbe(state, nodeId, t, 'sweep', cls, frame);",
    repl: "      pendProbe(state, nodeId, t, frame === 'read' ? 'read' : 'sweep', cls, frame);",
    what: "a sweep read is judged as a SWEEP probe \u2014 it feeds the reply rate and is never booked as a revival" },
  { id: 'probe-frame-rides-the-judgment', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      out.push({ nodeId, answered, misses, cls, lane, frame: frame ?? 'ping' });",
    repl: "      out.push({ nodeId, answered, misses, cls, lane, frame: 'ping' as const });",
    what: "a judged probe carries the frame it sent" },
  { id: 'probe-frame-rides-the-death', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    return { nodeId, misses, cls, lane, frame: frame ?? 'ping' };",
    repl: "    return { nodeId, misses, cls, lane, frame: 'ping' as const };",
    what: "a probe settled on a death carries the frame it sent" },
  { id: 'probe-frame-reaches-the-rate', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        if (lane === 'sweep') o.onProbeResult?.(nodeId, true, cls, frame);",
    repl: "        if (lane === 'sweep') o.onProbeResult?.(nodeId, true, cls, 'ping');",
    what: "the persisted reply rate is told which frame was answered" },
  { id: 'sweep-line-names-the-frame', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        frameNote(frame, nodeId, t);",
    repl: "        '';",
    what: "every sweep line names its frame, and why a NoOp" },
  { id: 'verify-line-names-the-frame', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "owed)` + frameNote(frame, nodeId, t);",
    repl: "owed)`;",
    what: "every verification line names its frame" },
  { id: 'frame-note-says-no-value', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        : ' \u2014 NoOp ping (no switch/light value to read)';",
    repl: "        : ' \u2014 NoOp ping';",
    what: "a NoOp sweep says it had no value to read" },
  { id: 'frame-note-says-fallback', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      : readFallbackActive(state, id, at)\n        ?",
    repl: "      : false\n        ?",
    what: "a fallback NoOp says the read could not be sent" },
  { id: 'settled-miss-names-lane-and-frame', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "consecutive miss \u2014 the node has since been marked Dead)` + probeWord(s.lane, s.frame);",
    repl: "consecutive miss \u2014 the node has since been marked Dead)`;",
    what: "a miss settled on a death names its lane and frame" },
  { id: 'judged-miss-names-lane-and-frame', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "(${ord} consecutive miss, lastSeen did not advance)` + probeWord(lane, frame);",
    repl: "(${ord} consecutive miss, lastSeen did not advance)`;",
    what: "a judged miss names its lane and frame" },
  { id: 'probe-hold-starts-at-recovery', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (state.probeDeath.has(n.nodeId)) state.probeHoldFrom.set(n.nodeId, now);",
    repl: "",
    what: "a node revived from our own kill is held from measurement probes" },
  { id: 'probe-hold-holds-the-sweep', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      .filter((n) => !inProbeHold(input.state, n.nodeId, now))",
    repl: "      .filter(() => true)",
    what: "the sweep leaves a held node alone" },
  { id: 'probe-hold-holds-verification', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...candidates]) if (inProbeHold(input.state, id, now)) candidates.delete(id);",
    repl: "",
    what: "no verification probe goes to a held node, even from a queue that ignores the skip" },
  { id: 'probe-hold-leaves-bursts-owed', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      verifyDue: () => o.verifyRequests?.(t, (id) => inProbeHold(state, id, t)) ?? [],",
    repl: "      verifyDue: () => o.verifyRequests?.(t) ?? [],",
    what: "a held node's burst stays OWED, not drained on probes that are never sent" },
  { id: 'probe-hold-outlasts-dead-flap-window', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "export const PROBE_KILL_HOLD_MS = 15 * 60_000;",
    repl: "export const PROBE_KILL_HOLD_MS = 5 * 60_000;",
    what: "our own probing cannot make dead-flap's three crossings inside one 10-minute window" },
  { id: 'probe-hold-expires', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  return at != null && now - at < PROBE_KILL_HOLD_MS;",
    repl: "  return at != null;",
    what: "the hold lifts on time" },
  { id: 'probe-hold-leaves-with-the-node', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...state.probeHoldFrom.keys()]) if (!seen.has(id)) state.probeHoldFrom.delete(id);",
    repl: "",
    what: "a departed node takes its hold with it" },
  { id: 'probe-kills-counted', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    if (!newest.killBooked) recordProbeKill(state, nodeId, now);",
    repl: "",
    what: "a death on our own measurement probe is counted" },
  { id: 'probe-kills-age-out', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  const kills = [...(state.probeKills.get(nodeId) ?? []).filter((k) => at - k < PROBE_KILL_WINDOW_MS), at];",
    repl: "  const kills = [...(state.probeKills.get(nodeId) ?? []), at];",
    what: "a kill older than 24 h does not count toward the warning" },
  { id: 'probe-kill-warns-once', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  if (kills.length === PROBE_KILL_WARN_AT) state.probeKillWarn.add(nodeId);",
    repl: "  if (kills.length >= PROBE_KILL_WARN_AT) state.probeKillWarn.add(nodeId);",
    what: "the repeat-kill warning is said once per crossing, not on every later kill" },
  { id: 'probe-kill-warning-is-said', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    for (const id of state.probeKillWarn) {",
    repl: "    for (const id of [] as number[]) {",
    what: "the repeat-kill warning reaches the log" },
  { id: 'probe-kill-warnings-cleared', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    state.probeKillWarn.clear();",
    repl: "",
    what: "the warning is not repeated on every tick" },
  { id: 'probe-kills-leave-with-the-node', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...state.probeKills.keys()]) if (!seen.has(id)) state.probeKills.delete(id);",
    repl: "",
    what: "a departed node takes its kill history with it" },
  { id: 'probe-kill-warn-leaves-with-the-node', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...state.probeKillWarn]) if (!seen.has(id)) state.probeKillWarn.delete(id);",
    repl: "",
    what: "a departed node takes its pending warning with it" },
  { id: 'between-tick-death-counted', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      recordProbeKill(state, d.nodeId, d.at);",
    repl: "",
    what: "a death that clears between ticks is still counted as our kill" },
  { id: 'between-tick-death-held', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      state.probeHoldFrom.set(d.nodeId, t);",
    repl: "",
    what: "and it starts the hold" },
  { id: 'between-tick-death-needs-our-probe', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (newest == null || (newest.lane !== 'sweep' && newest.lane !== 'verify')) continue;",
    repl: "      if (newest == null) continue;",
    what: "a between-tick death after an operator's ping is not our measurement's kill" },
    { id: 'read-launch-failure-stamped', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        else state.lastStaleAt.set(nodeId, priorStale);\n        o.onMeasurementWithdrawn?.(nodeId, t);\n        if (frame === 'read') readLaunchFailed(nodeId, t);",
    repl: "        else state.lastStaleAt.set(nodeId, priorStale);\n        o.onMeasurementWithdrawn?.(nodeId, t);",
    what: "a sweep read that could not be sent moves the node to the NoOp fallback" },
  { id: 'read-launch-failure-stamped-verify', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        else state.lastVerifyAt.set(nodeId, prevVerify);\n        o.onMeasurementWithdrawn?.(nodeId, t);\n        if (frame === 'read') readLaunchFailed(nodeId, t);",
    repl: "        else state.lastVerifyAt.set(nodeId, prevVerify);\n        o.onMeasurementWithdrawn?.(nodeId, t);",
    what: "a verification read that could not be sent moves the node to the NoOp fallback" },
  { id: 'verify-refund-restores-previous', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        if (prevVerify == null) state.lastVerifyAt.delete(nodeId);\n        else state.lastVerifyAt.set(nodeId, prevVerify);",
    repl: "        if (true) state.lastVerifyAt.set(nodeId, t);",
    what: "a refused verification probe gives back the PREVIOUS stamp, not its own" },
  { id: 'read-fallback-expires', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  return at != null && now - at < READ_LAUNCH_FALLBACK_MS;",
    repl: "  return at != null;",
    what: "the NoOp fallback lifts after its window" },
  { id: 'read-fallback-leaves-with-the-node', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...state.readLaunchFailedAt.keys()]) if (!seen.has(id)) state.readLaunchFailedAt.delete(id);",
    repl: "",
    what: "a departed node takes its fallback mark with it" },
  { id: 'read-after-own-kill-spares-the-cap', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        if (answered && ownKill) {",
    repl: "        if (false) {",
    what: "a revival after our own measurement kill does not spend the read cap" },
  { id: 'read-after-own-kill-captured', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (state.probeDeath.has(nodeId)) state.readAfterOwnKill.add(nodeId);",
    repl: "      if (false) state.readAfterOwnKill.add(nodeId);",
    what: "the own-kill mark is captured at the read's launch" },
  { id: 'read-after-own-kill-is-per-read', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      else state.readAfterOwnKill.delete(nodeId);",
    repl: "",
    what: "an unjudged read's mark does not leak into a later, unprovoked episode" },
  { id: 'read-after-own-kill-leaves-with-the-node', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  for (const id of [...state.readAfterOwnKill]) if (!seen.has(id)) state.readAfterOwnKill.delete(id);",
    repl: "",
    what: "a departed node takes its own-kill mark with it" },
  { id: 'snapshot-reports-own-probe-kills', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        probeKills24h: lastTickMs == null ? 0 : probeKillsWithin(state, nodeId, lastTickMs),",
    repl: "        probeKills24h: 0,",
    what: "the snapshot reports own-probe kills" },
  { id: 'snapshot-reports-the-hold', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "          ? state.probeHoldFrom.get(nodeId)! + PROBE_KILL_HOLD_MS : null,",
    repl: "          ? null : null,",
    what: "the snapshot reports when the hold lifts" },
  { id: 'snapshot-tracks-held-nodes', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      ...state.probeHoldFrom.keys(), ...state.probeKills.keys(),",
    repl: "",
    what: "a held node with nothing else pending is still in the snapshot" },
  { id: 'measurement-read-logs-as-a-probe', file: 'src/zwave/zwaveActions.ts', tests: ["zwaveActions"],
    find: "purpose === 'probe' ? `probe node ${n} (routed read)` : `routed read node ${n}`",
    repl: "`routed read node ${n}`",
    what: "a measurement read is logged as a probe, the ladder read as a read" },
  { id: 'read-counts-need-the-frame', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "      if (frame === 'read') { m.probesReadAsked += 1;",
    repl: "      if (true) { m.probesReadAsked += 1;",
    what: "only a routed read counts in the read subset" },
  { id: 'read-answered-needs-an-answer', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "if (answered) m.probesReadAnswered += 1; }",
    repl: "m.probesReadAnswered += 1; }",
    what: "an unanswered read is not a read answered" },
  { id: 'read-counts-persist', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "          Object.assign(metaOut[String(id)], { ra: m.probesReadAsked, rk: m.probesReadAnswered, rr: m.probeReroutes });",
    repl: "",
    what: "the read and reroute counters are saved" },
  { id: 'read-counts-load', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "              probesReadAsked: num(fm.ra), probesReadAnswered: num(fm.rk), probeReroutes: num(fm.rr),",
    repl: "              probesReadAsked: 0, probesReadAnswered: 0, probeReroutes: 0,",
    what: "and loaded" },
  { id: 'read-counts-load-at-zero', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "              probesReadAsked: num(fm.ra), probesReadAnswered: num(fm.rk), probeReroutes: num(fm.rr),",
    repl: "              probesReadAsked: fm.ra as number, probesReadAnswered: fm.rk as number, probeReroutes: fm.rr as number,",
    what: "a v0.70.0 file loads them at zero, not undefined" },
  { id: 'probe-reroute-counted', file: 'src/zwave/evidenceStore.ts', tests: ["evidenceStore"],
    find: "      m.probeReroutes += 1;",
    repl: "",
    what: "a stored-route failover on our sweep is counted" },
  { id: 'measurement-reroute-marks', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        this.measurementReroutes.add(nodeId);",
    repl: "        void 0;",
    what: "a failover our read caused is marked for the confound guard" },
  { id: 'measurement-reroute-window', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (txNow === sent.tx0 + 1 && sinceMs >= 0 && sinceMs <= ANSWER_GRACE_MS) {",
    repl: "        if (txNow === sent.tx0 + 1 && sinceMs >= 0) {",
    what: "a probe long past its grace did not cause the change" },
  { id: 'measurement-reroute-needs-a-failover', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      const ours = ourFrame != null && ourFrame.frame === 'read' && stats.lwr?.routeFailedBetween != null;",
    repl: "      const ours = ourFrame != null && ourFrame.frame === 'read';",
    what: "a route change merely REVEALED by our probe (an owner rebuild) is not ours" },
  { id: 'measurement-reroute-needs-a-read', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      const ours = ourFrame != null && ourFrame.frame === 'read' && stats.lwr?.routeFailedBetween != null;",
    repl: "      const ours = ourFrame != null && stats.lwr?.routeFailedBetween != null;",
    what: "a NoOp cannot compute a route" },
  { id: 'measurement-reroute-not-churn', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      } else {\n        this.routeChangeAccum.set(nodeId, (this.routeChangeAccum.get(nodeId) ?? 0) + 1);\n      }",
    repl: "      }\n      this.routeChangeAccum.set(nodeId, (this.routeChangeAccum.get(nodeId) ?? 0) + 1);",
    what: "our own failover does not feed route-churn" },
  { id: 'measurement-reroute-confounds', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (this.measurementReroutes.has(ep.nodeId) && MEASUREMENT_REROUTE_KINDS.has(ep.kind)) oc.markConfounded(ep.nodeId, ep.kind);",
    repl: "",
    what: "an episode our read may have cleared is credited to neither arm" },
  { id: 'measurement-reroute-scoped', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (this.measurementReroutes.has(ep.nodeId) && MEASUREMENT_REROUTE_KINDS.has(ep.kind)) oc.markConfounded(ep.nodeId, ep.kind);",
    repl: "        if (this.measurementReroutes.has(ep.nodeId)) oc.markConfounded(ep.nodeId, ep.kind);",
    what: "dead-flap, route-churn, quiet-node and s2-desync keep their control arms" },
  { id: 'measurement-reroute-consumed', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    this.flapsThisTick.clear();\n    this.measurementReroutes.clear();",
    repl: "    this.flapsThisTick.clear();",
    what: "the mark is consumed by one engine pass, not latched" },
  { id: 'sweep-reroute-counted', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (ourFrame!.lane === 'sweep') this.evidenceStore?.recordProbeReroute(nodeId);",
    repl: "",
    what: "a sweep failover reaches the persisted count" },
  { id: 'sweep-reroute-counted-sweep-only', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (ourFrame!.lane === 'sweep') this.evidenceStore?.recordProbeReroute(nodeId);",
    repl: "        this.evidenceStore?.recordProbeReroute(nodeId);",
    what: "a verification failover stays out of the comparable count" },
  { id: 'probe-frame-reaches-the-store', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    this.evidenceStore?.recordProbe(nodeId, answered, cls, undefined, frame);",
    repl: "    this.evidenceStore?.recordProbe(nodeId, answered, cls);",
    what: "the data layer forwards the frame" },
  { id: 'dead-event-from-the-feed', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (crossedDead) this.noteDeadCrossing(nodeId, next === NodeStatus.Dead);",
    repl: "",
    what: "a death on the status feed reaches the runner" },
  { id: 'dead-event-only-on-death', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      for (const e of this.deadEvents) if (e.nodeId === nodeId && !e.cleared) { e.cleared = true; e.clearedAt = now; }\n      return;\n    }",
    repl: "      for (const e of this.deadEvents) if (e.nodeId === nodeId && !e.cleared) { e.cleared = true; e.clearedAt = now; }\n    }",
    what: "a revival is not a death" },
  { id: 'dead-event-from-the-roster-fallback', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "          if (crossedDead) this.noteDeadCrossing(n.nodeId, n.status === NodeStatus.Dead);",
    repl: "",
    what: "the roster-diff fallback feeds the runner too" },
  { id: 'dead-events-drained', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    this.deadEvents = this.deadEvents.filter((e) => !e.cleared);",
    repl: "",
    what: "the death feed is drained, not latched" },
  { id: 'verify-skip-leaves-owed', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      if (skip?.(id)) continue;",
    repl: "",
    what: "a skipped node's burst is left owed" },
  { id: 'index-wires-the-measurement-read', file: 'src/index.ts', tests: ["configContract"],
    find: "      probeRead: (n) => actions.routedRead(n, 'probe'),",
    repl: "",
    what: "production wires the measurement read" },
  { id: 'index-wires-the-measurement-stamp', file: 'src/index.ts', tests: ["configContract"],
    find: "      onMeasurementSent: (n, at, lane, frame) => zwaveData.noteMeasurementProbe(n, at, lane, frame),",
    repl: "",
    what: "production wires the measurement stamp" },
  { id: 'index-wires-the-death-feed', file: 'src/index.ts', tests: ["configContract"],
    find: "      deaths: () => zwaveData.drainDeadEvents(),",
    repl: "",
    what: "production wires the death feed" },
  { id: 'index-forwards-the-verify-skip', file: 'src/index.ts', tests: ["configContract"],
    find: "      verifyRequests: (now, skip) => zwaveData.drainVerifyRequests(now, skip),",
    repl: "      verifyRequests: (now) => zwaveData.drainVerifyRequests(now),",
    what: "production forwards the hold to the queue" },
  { id: 'index-forwards-the-probe-frame', file: 'src/index.ts', tests: ["configContract"],
    find: "      onProbeResult: (nodeId, answered, cls, frame) => zwaveData.recordProbeResult(nodeId, answered, cls, frame),",
    repl: "      onProbeResult: (nodeId, answered, cls) => zwaveData.recordProbeResult(nodeId, answered, cls),",
    what: "production forwards the frame to the store" },
  { id: 'detail-frame-split', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "          if (ra === 0) {",
    repl: "          if (true) {",
    what: "DETAIL splits a mixed history by frame" },
  { id: 'detail-frame-split-subtracts', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "          const na = cov.probesAsked - ra;",
    repl: "          const na = cov.probesAsked;",
    what: "the ping share excludes the reads" },
  { id: 'detail-frame-all-read', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "          } else if (na === 0) {",
    repl: "          } else if (false) {",
    what: "an all-read history says so" },
  { id: 'detail-frame-forms-fit', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "x.length + 3 + y.length <= room",
    repl: "true",
    what: "the Frame row picks a form that fits, never a cut one" },
  { id: 'detail-rerouted-row', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "        if ((cov.probeReroutes ?? 0) > 0) {",
    repl: "        if ((cov.probeReroutes ?? 0) > 1e9) {",
    what: "DETAIL shows the stored-route failovers" },
  { id: 'detail-rerouted-row-fits', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "inner - KV_GUTTER >= long.length ? long : short",
    repl: "long",
    what: "the Rerouted row is never cut mid-claim" },
  { id: 'engine-tracks-own-probe-kills', file: 'src/telnet/screens/engine.ts', tests: ["engineScreen"],
    find: "      (n.probeKills24h ?? 0) > 0 || (n.probeHeldUntilMs != null && n.probeHeldUntilMs > now));",
    repl: "      false);",
    what: "ENGINE tracks a node our probes knocked Dead" },
  { id: 'engine-shows-own-probe-kills', file: 'src/telnet/screens/engine.ts', tests: ["engineScreen"],
    find: "        if ((n.probeKills24h ?? 0) > 0) bits.push(",
    repl: "        if (false) bits.push(",
    what: "ENGINE shows the own-probe kill count" },
  { id: 'engine-shows-probe-hold', file: 'src/telnet/screens/engine.ts', tests: ["engineScreen"],
    find: "        if (n.probeHeldUntilMs != null && n.probeHeldUntilMs > now) bits.push(",
    repl: "        if (n.probeHeldUntilMs != null && n.probeHeldUntilMs < 0) bits.push(",
    what: "ENGINE shows the hold" },
  { id: 'ping-note-names-the-kill', file: 'src/telnet/actionsCatalog.ts', tests: ["actionsCatalog"],
    find: "\"No data change, but a ping is ONE ACK-only attempt on the stored routes: if it goes unanswered the driver marks the node Dead \u2014 even a working one \u2014 until something reaches it. HA does not",
    repl: "\"Harmless \u2014 no mesh disruption, no data change. NOTE: HA does not",
    what: "the Ping node note says an unanswered ping marks the node Dead" },
  { id: 'quiet-node-card-honest', file: 'src/zwave/planner.ts', tests: ["planner"],
    find: "A ping is one ACK-only attempt on the stored routes, so on a marginal link it can itself mark the node Dead; with auto-ping\u2019s liveness sweep on, every mains node is already asked on a cadence, by a routed read where the node has a switch/light value.",
    repl: "It can mark a truly-marginal node dead, so it is not run automatically.",
    what: "the quiet-node card no longer claims pings never run automatically" },
  { id: 'confounded-row-names-reroute', file: 'src/telnet/screens/remedy.ts', tests: ["remedyScreen"],
    find: "confounded by a mid-episode death, remediation or probe re-route \u2014 ",
    repl: "confounded by a mid-episode death or remediation \u2014 ",
    what: "REMEDY names the new confound cause" },
  /* ── v0.71.0 review: between-tick attribution, our frame's own TX report ── */
  { id: 'between-tick-needs-unanswered', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        (d.seen == null || d.seen < p.at) && !p.killBooked);",
    repl: "        !p.killBooked);",
    what: "a death after the node ANSWERED our probe is not blamed on it" },
  { id: 'between-tick-grace', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "p.at <= d.at && d.at - p.at <= ANSWER_GRACE_MS &&\n",
    repl: "p.at <= d.at &&\n",
    what: "a death past the answer grace is not blamed on the probe" },
  { id: 'between-tick-before-death', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "pending.filter((p) => p.at <= d.at && d.at - p.at",
    repl: "pending.filter((p) => true && d.at - p.at",
    what: "a probe sent after the death cannot have caused it" },
  { id: 'between-tick-newest-decides', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      const newest = before[before.length - 1];",
    repl: "      const newest = before[0];",
    what: "the newest probe before a between-tick death decides whose it was" },
  { id: 'between-tick-noop-is-a-miss', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (frame === 'read' && revivedIn != null && revivedIn <= READ_REPORT_REVIVAL_MS) {",
    repl: "      if (revivedIn != null && revivedIn <= READ_REPORT_REVIVAL_MS) {",
    what: "a between-tick NoOp kill is settled a miss, never later credited as answered" },
  { id: 'between-tick-noop-feeds-the-rate', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (newest.lane === 'sweep') o.onProbeResult?.(d.nodeId, false, newest.cls, frame);",
    repl: "",
    what: "that miss reaches the reply rate" },
  { id: 'between-tick-noop-unpended', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      const rest = pending.filter((p) => p !== newest);",
    repl: "      const rest = pending;",
    what: "and is withdrawn from the ordinary judgment" },
  { id: 'settled-frame-reaches-the-rate', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "s.cls, s.frame);",
    repl: "s.cls, 'ping');",
    what: "a sweep read settled on a death reaches the rate as a missed READ" },
  { id: 'ladder-never-held', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    if (started == null || (now - started < config.afterMs && !input.state.probeDeath.has(n.nodeId))) continue;",
    repl: "    if (started == null || (now - started < config.afterMs && !input.state.probeDeath.has(n.nodeId))) continue;\n    if (inProbeHold(input.state, n.nodeId, now)) continue;",
    what: "the post-kill hold never holds the dead ladder" },
  { id: 'ladder-read-is-not-a-measurement', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      pendProbe(state, nodeId, t, 'read', 'unheard', 'read');",
    repl: "      pendProbe(state, nodeId, t, 'read', 'unheard', 'read'); o.onMeasurementSent?.(nodeId, t, 'sweep', 'read');",
    what: "the ladder's read is never stamped as a measurement" },
  { id: 'ladder-read-carries-its-frame', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      pendProbe(state, nodeId, t, 'read', 'unheard', 'read');",
    repl: "      pendProbe(state, nodeId, t, 'read');",
    what: "a ladder read settled on a death is labelled a routed read" },
  { id: 'lane-word-verification', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "verify: 'verification', dead: 'ladder'",
    repl: "verify: 'sweep', dead: 'ladder'",
    what: "a miss line names the verification lane as such" },
  { id: 'frame-word-noop', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "  return frame === 'read' ? 'routed read' : 'NoOp ping';",
    repl: "  return 'routed read';",
    what: "a miss line names a NoOp as such" },
  { id: 'read-fallback-only-for-reads', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        else state.lastStaleAt.set(nodeId, priorStale);\n        o.onMeasurementWithdrawn?.(nodeId, t);\n        if (frame === 'read') readLaunchFailed(nodeId, t);",
    repl: "        else state.lastStaleAt.set(nodeId, priorStale);\n        o.onMeasurementWithdrawn?.(nodeId, t);\n        readLaunchFailed(nodeId, t);",
    what: "a NoOp that could not be sent is not reported as a failed read" },
  { id: 'dead-event-waits-for-clear', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    const out = this.deadEvents.filter((e) => e.cleared);",
    repl: "    const out = this.deadEvents.filter(() => true);",
    what: "a death is handed over only once it cleared, so the lagging roster cannot make it count twice" },
  { id: 'dead-event-clears', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      for (const e of this.deadEvents) if (e.nodeId === nodeId && !e.cleared) { e.cleared = true; e.clearedAt = now; }",
    repl: "      for (const e of this.deadEvents) if (e.nodeId === nodeId && e.cleared) { e.clearedAt = now; }",
    what: "a revival clears the queued death" },
  { id: 'dead-event-seen-captured', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    this.deadEvents.push({ nodeId, at: Date.now(), seen: this.statsByNode.get(nodeId)?.lastSeen ?? null,",
    repl: "    this.deadEvents.push({ nodeId, at: Date.now(), seen: null,",
    what: "the death carries the node's lastSeen at that moment" },
  { id: 'dead-events-bounded', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (this.deadEvents.length > DEAD_EVENTS_MAX) this.deadEvents.splice(0, this.deadEvents.length - DEAD_EVENTS_MAX);",
    repl: "",
    what: "the death queue is bounded with no reader" },
  { id: 'death-drops-the-stamp', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    // node next is not our measurement probe's TX report.\n    this.measurementSent.delete(nodeId);",
    repl: "    // node next is not our measurement probe's TX report.",
    what: "a node marked Dead drops its measurement stamp" },
  { id: 'stamp-consumed-by-our-frame', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      } else if (sent != null && sent.tx0 != null && txNow > sent.tx0) {\n        this.measurementSent.delete(nodeId);",
    repl: "      } else if (sent != null && sent.tx0 != null && txNow > sent.tx0) {",
    what: "only our frame's own TX report can be attributed; a later frame's failover is churn" },
  { id: 'stamp-needs-tx-movement', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "sent.tx0 != null && txNow > sent.tx0",
    repl: "sent.tx0 != null && txNow >= sent.tx0",
    what: "an event that moves no TX counter is not our TX report" },
  { id: 'route-churn-confounded', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "'chronic-return-path', 'route-churn',",
    repl: "'chronic-return-path',",
    what: "a route-churn episode our reads re-routed is not scored improved" },
  { id: 'route-event-names-our-failover', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "(ours ? ` (our ${lane} routed read failed over ${Math.round(ourFrame!.sinceMs / 1000)}s after it went out)` : ''));",
    repl: "(''));",
    what: "the Log ring's route event says when the failover was our read's" },
  { id: 'detail-frame-all-noop-fits', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "frameText = c.grey(room >= long.length ? long : 'all NoOp pings (stored routes only)');",
    repl: "frameText = c.grey(long);",
    what: "the all-NoOp Frame form fits at 60 columns" },
  { id: 'detail-frame-all-read-fits', file: 'src/telnet/screens/detail.ts', tests: ["detailScreen"],
    find: "frameText = c.grey(room >= long.length ? long : 'all routed reads (any route)');",
    repl: "frameText = c.grey(long);",
    what: "the all-read Frame form fits at 60 columns" },
  /* ── v0.71.0 re-review: the throttle's trailing emit, withdrawn stamps, a second death ── */
  { id: 'trailing-emit-is-ours', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "          this.ourTxReport.set(nodeId, { at: sent.at, txAt: txNow, lane: sent.lane, frame: sent.frame, sinceMs, until: Date.now() + OUR_TX_TRAIL_MS });",
    repl: "",
    what: "our read's route is recognised in the throttle's trailing emit, not only in a combined event" },
  { id: 'trailing-attribution-applies', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      if (held != null) ourFrame = { lane: held.lane, frame: held.frame, sinceMs: held.sinceMs };",
    repl: "      if (held != null && held.txAt < 0) ourFrame = { lane: held.lane, frame: held.frame, sinceMs: held.sinceMs };",
    what: "the held TX report is what attribution reads" },
  { id: 'trailing-window-closes', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (held != null && (txNow !== held.txAt || Date.now() > held.until)) this.ourTxReport.delete(nodeId);",
    repl: "",
    what: "the trailing window closes" },
  { id: 'trailing-window-closes-on-new-tx', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (held != null && (txNow !== held.txAt || Date.now() > held.until)) this.ourTxReport.delete(nodeId);",
    repl: "        if (held != null && Date.now() > held.until) this.ourTxReport.delete(nodeId);",
    what: "another frame's TX report closes it" },
  { id: 'trailing-window-has-a-deadline', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (held != null && (txNow !== held.txAt || Date.now() > held.until)) this.ourTxReport.delete(nodeId);",
    repl: "        if (held != null && txNow !== held.txAt) this.ourTxReport.delete(nodeId);",
    what: "a route arriving after the window is not ours" },
  { id: 'one-report-one-route', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      // One report, one route: judged, it cannot be ours again.\n      this.ourTxReport.delete(nodeId);",
    repl: "      // One report, one route: judged, it cannot be ours again.",
    what: "a TX report is judged for a route change once" },
  { id: 'withdrawn-stamp-is-dropped', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (this.measurementSent.get(nodeId)?.at === at) this.measurementSent.delete(nodeId);",
    repl: "",
    what: "a refused read's stamp is withdrawn" },
  { id: 'withdrawn-stamp-matches', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (this.measurementSent.get(nodeId)?.at === at) this.measurementSent.delete(nodeId);",
    repl: "    this.measurementSent.delete(nodeId);",
    what: "only the stamp of the probe that never left is withdrawn" },
  { id: 'sweep-refund-withdraws', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        else state.lastStaleAt.set(nodeId, priorStale);\n        o.onMeasurementWithdrawn?.(nodeId, t);",
    repl: "        else state.lastStaleAt.set(nodeId, priorStale);",
    what: "a sweep probe that never left withdraws its stamp" },
  { id: 'verify-refund-withdraws', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        else state.lastVerifyAt.set(nodeId, prevVerify);\n        o.onMeasurementWithdrawn?.(nodeId, t);",
    repl: "        else state.lastVerifyAt.set(nodeId, prevVerify);",
    what: "a verification probe that never left withdraws its stamp" },
  { id: 'index-wires-the-withdrawal', file: 'src/index.ts', tests: ["configContract"],
    find: "      onMeasurementWithdrawn: (n, at) => zwaveData.clearMeasurementProbe(n, at),",
    repl: "",
    what: "production wires the withdrawal" },
  { id: 'stamp-counts-dropped-tx', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "tx0: s == null ? null : s.commandsTX + s.commandsDroppedTX",
    repl: "tx0: s == null ? null : s.commandsTX",
    what: "the stamp's snapshot counts dropped TX, as the comparison does" },
  { id: 'revival-clears-only-its-node', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      for (const e of this.deadEvents) if (e.nodeId === nodeId && !e.cleared) { e.cleared = true; e.clearedAt = now; }",
    repl: "      for (const e of this.deadEvents) if (!e.cleared) { e.cleared = true; e.clearedAt = now; }",
    what: "a revival clears only that node's queued deaths" },
  { id: 'between-tick-noop-rate-sweep-only', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (newest.lane === 'sweep') o.onProbeResult?.(d.nodeId, false, newest.cls, frame);",
    repl: "      o.onProbeResult?.(d.nodeId, false, newest.cls, frame);",
    what: "a between-tick verification NoOp miss stays out of the comparable rate" },
  { id: 'between-tick-noop-moves-the-streak', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      const misses = (state.missStreak.get(d.nodeId) ?? 0) + 1;\n      state.missStreak.set(d.nodeId, misses);",
    repl: "      const misses = (state.missStreak.get(d.nodeId) ?? 0) + 1;",
    what: "a between-tick NoOp miss moves the consecutive-miss streak" },
  { id: 'between-tick-unknown-seen-is-blamed', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "(d.seen == null || d.seen < p.at)",
    repl: "(d.seen != null && d.seen < p.at)",
    what: "a death with no lastSeen on record is still blamed on the unanswered probe" },
  { id: 'between-tick-judged-when-dead-again', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    for (const d of o.deaths?.() ?? []) {\n      // With the node's statistics feed not delivering",
    repl: "    for (const d of o.deaths?.() ?? []) {\n      if (state.deadSince.has(d.nodeId)) continue;\n      // With the node's statistics feed not delivering",
    what: "a cleared death is judged even when the node is Dead again at the tick" },
  { id: 'between-tick-says-dead-again', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "const back = state.deadSince.has(d.nodeId) ? 'came back, and is Dead again at this tick on something later' : 'was Alive again before the next tick';",
    repl: "const back = 'was Alive again before the next tick';",
    what: "the line does not claim the node is Alive when it is Dead again" },
  { id: 'measurement-stamp-leaves-with-the-node', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "          this.measurementSent.delete(id);\n          this.ourTxReport.delete(id);\n          this.measurementReroutes.delete(id);",
    repl: "",
    what: "a departed node's measurement state goes with it",
    equivalent: true,
    why: "a node is evicted only after five minutes absent from the roster; by then its stamp is past the 90 s grace (a stale stamp attributes nothing when consumed), its TX-report window past its 2 s deadline, and its reroute mark consumed by an engine pass \u2014 a re-included device on the same id cannot inherit any of them" },
  { id: 'measurement-state-cleared-on-home-change', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        this.measurementSent.clear();\n        this.ourTxReport.clear();\n        this.measurementReroutes.clear();\n        this.deadEvents = [];",
    repl: "",
    what: "a home-id change drops measurement stamps, held TX reports, reroute marks and queued deaths",
    equivalent: true,
    why: "each of those expires on its own inside two minutes (the 90 s grace, the 2 s trailing window, the next engine pass, the next runner tick), and each still describes a frame this runner sent to that node id, so keeping it through a home-id change changes nothing a test or an operator can observe" },
  { id: 'measurement-window-not-negative', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (txNow === sent.tx0 + 1 && sinceMs >= 0 && sinceMs <= ANSWER_GRACE_MS) {",
    repl: "        if (txNow === sent.tx0 + 1 && sinceMs <= ANSWER_GRACE_MS) {",
    what: "a stamp from the future attributes nothing",
    equivalent: true,
    why: "the stamp and the event are both read from Date.now() in one process, so sinceMs is negative only if the wall clock steps backwards between a probe and its TX report" },
  /* ── v0.71.0 final verification: the Report's revival window, feed liveness, one booking ── */
  { id: 'read-revival-window', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (frame === 'read' && revivedIn != null && revivedIn <= READ_REPORT_REVIVAL_MS) {",
    repl: "      if (frame === 'read' && revivedIn != null) {",
    what: "a read revived long after its death is a miss, not an answer" },
  { id: 'feed-not-live-is-not-blamed', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "      if (d.feedLive === false) continue;",
    repl: "",
    what: "a death with no live statistics feed is not blamed on our probe" },
  { id: 'kill-booked-excluded', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        (d.seen == null || d.seen < p.at) && !p.killBooked);",
    repl: "        (d.seen == null || d.seen < p.at));",
    what: "a read already booked is not booked again by a second death" },
  { id: 'kill-booked-marked', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "        newest.killBooked = true;",
    repl: "",
    what: "the booked read is marked" },
  { id: 'settle-skips-a-booked-read', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    find: "    if (!newest.killBooked) recordProbeKill(state, nodeId, now);",
    repl: "    recordProbeKill(state, nodeId, now);",
    what: "a later tick that sees the node Dead does not book the read again" },
  { id: 'one-tx-report-only', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (txNow === sent.tx0 + 1 && sinceMs >= 0 && sinceMs <= ANSWER_GRACE_MS) {",
    repl: "        if (sinceMs >= 0 && sinceMs <= ANSWER_GRACE_MS) {",
    what: "an event carrying more than one TX report is not attributed" },
  { id: 'withdrawn-held-report', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (this.ourTxReport.get(nodeId)?.at === at) this.ourTxReport.delete(nodeId);",
    repl: "",
    what: "a refused read also drops a TX report already taken for it" },
  { id: 'dead-event-feed-live', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "      feedLive: this.statsSubbedNodes.has(nodeId), cleared: false, clearedAt: null });",
    repl: "      feedLive: true, cleared: false, clearedAt: null });",
    what: "a death says whether its statistics feed was live" },
  { id: 'dead-event-cleared-at', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "e.cleared = true; e.clearedAt = now; }",
    repl: "e.cleared = true; e.clearedAt = now - 60_000; }",
    what: "a death records when it cleared" },
];
...
const SOURCE_TESTS = {
  ...
  'src/zwave/autonomyPause.ts': ['autonomyPause', 'autoPing', 'zwaveDataChurn', 'sessionActions'],
  'src/zwave/admission.ts': ['admission', 'engineScreen'],
  'src/zwave/recommendation.ts': ['recommendation', 'haStates'],
};
