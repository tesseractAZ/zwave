...
const MUTANTS = [
  ...
  { id: 'between-tick-death-not-double-counted', file: 'src/zwave/autoPing.ts', tests: ["autoPing"],
    ...
    equivalent: true,
    why: "settleProbeDeath has already removed the killing probe from awaitingAnswer by the time the feed is read, so the second path finds no measurement probe to blame; the guard is defence in depth" },
  ...
  { id: 'measurement-reroute-window', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "sinceMs <= ANSWER_GRACE_MS && stats.lwr?.routeFailedBetween != null",
    repl: "stats.lwr?.routeFailedBetween != null",
  ...
  { id: 'measurement-reroute-needs-a-read', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "const ours = sent != null && sent.frame === 'read' && sinceMs != null",
    repl: "const ours = sent != null && sinceMs != null",
  ...
  { id: 'sweep-reroute-counted', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "        if (sent!.lane === 'sweep') this.evidenceStore?.recordProbeReroute(nodeId);",
    repl: "",
  ...
  { id: 'dead-event-from-the-feed', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (crossedDead && next === NodeStatus.Dead) this.noteDeadEvent(nodeId);",
    repl: "",
  ...
  { id: 'dead-event-only-on-death', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "    if (crossedDead && next === NodeStatus.Dead) this.noteDeadEvent(nodeId);",
    repl: "    if (crossedDead) this.noteDeadEvent(nodeId);",
    what: "a revival is not a death" },
  { id: 'dead-event-from-the-roster-fallback', file: 'src/zwave/zwaveData.ts', tests: ["zwaveDataChurn"],
    find: "          if (crossedDead && n.status === NodeStatus.Dead) this.noteDeadEvent(n.nodeId);",
    repl: "",
  ...
  { id: 'quiet-node-card-honest', file: 'src/zwave/planner.ts', tests: ["planner"],
    find: "A ping is one ACK-only attempt on the stored routes, so on a marginal link it can itself mark the node Dead; the liveness sweep already asks every mains node on a cadence, by a routed read where the node has a switch/light value.",
