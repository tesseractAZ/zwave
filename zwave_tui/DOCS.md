...
**The scope line (v0.35).** `degradedNodes` was computed for every view since M6 and only ever reached the screen through the *inactive* narrative above — that is, it went dark at exactly the moment it mattered. During an ACTIVE event the panel now adds `scope · 7 distinct nodes symptomatic (quiet ones included)`. It is deliberately labelled **apart** from the detector's ratio, because the two count different populations: the narrative's "degraded X of Y active" is a share of the nodes that were *transmitting* in the window, while this is every node carrying a symptom at all. Printed as a bare second number they would read as one measurement disagreeing with itself; the label is what makes them two answers to two questions. The line is suppressed at zero, and the inactive case is untouched — the narrative already states the count there, and repeating it would be the duplicate the panel's whole ratio discipline exists to avoid.
