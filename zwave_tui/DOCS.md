...
> **A death on our own probe is not an outage (v0.64.4).** The ping verb is a single-attempt NoOp with no route exploration, so on a marginal hop the probe itself fails and the driver marks the node `Dead` on that frame. On the reference mesh every Dead flag outside a host reboot — five in 49 h — landed 0.26–1.74 s after a liveness-sweep probe, and the next ping reached the node in 30–50 ms. The dwell leaves a self-healing outage alone, and a quiet mains node that nothing else addresses cannot heal itself: under v0.64.3 each such blip became 11 minutes Dead, a critical `node-down` and six minutes of `degraded`. `trackEpisodes` now settles the case on the tick it first sees a watched death. Every pending probe the node has not answered — not heard since the probe's `at`, the judgment's own test applied early — is booked as a miss there and then: logged, added to the miss streak, and for the sweep lane passed to `onProbeResult`. The node joins `probeDeath`, which exempts it from the dwell gate and from nothing else; `deadSince` keeps the observed death. Booking at the death instead of at `ANSWER_GRACE_MS` also closes a credit leak present from v0.50.0 to v0.64.2: the ladder's retry revived the node inside the grace, and the probe that killed it was booked answered. NODE DETAIL discloses those credits under **Probes** until `creditEpoch` retires the caveat for a node first seen after it. The mark clears on recovery and on roster departure.
...
| Boot window | 5 m after start (or roster not ready) for the sweep and verification lanes; the dead ladder waits only for a ready roster (v0.64.4) | Right after start every node can read Dead; without this the engine would ping the whole mesh on every restart. A ready roster is that hazard's own measure, and holding the ladder to the wall clock kept a node's Home Assistant entities unavailable ~3 min longer after a host reboot |
...
| Dead dwell `auto_ping_after_min` | 10 m (2–120); not applied to a node that went Dead on this add-on's own unanswered probe (v0.64.4) | Measured on the reference mesh: self-healing episodes resolve inside ~5 m, stuck ones run 6–9 h — 10 m sits in the gap. A node our own probe knocked Dead is not healing, so the dwell would only hold it down |
