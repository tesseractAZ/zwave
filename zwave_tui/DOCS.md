...
>
> **Those nodes have no liveness coverage from either layer (v0.65.0, live audit).** The driver marks a node Dead only when a transmission it attempted fails, and it never attempts one to a sleeping device; this add-on never sweeps them, and `quiet-node` is gated on `isListening === true`. So a battery sensor that stops reporting altogether is invisible to both. On the reference mesh that is 3 of 39 nodes (two motion sensors reporting every ~71 min, one every ~61 min). The audit measured their cadence over 49 h: zero missed slots and under 0.1 % spread between the median and worst gap, which is the data the original design note lacked when it worried that watching sleeping nodes would make "every sleeping sensor a standing alert". A per-node cadence watch is therefore cheap and low-false-positive, and remains UNBUILT — recorded here so the gap is a known one rather than an assumption.
...
> **A driver restart is a third party's probe (v0.65.0).** zwave-js pings the whole mesh itself as it comes up, which advances every node's `lastSeen` within seconds — evidence this add-on did not produce and cannot attribute. Read as the node's own voice it becomes a `self-proven` credit, and the 2026-09-15 audit counted 28 of them across two driver restarts in 23 h, into the same persisted, never-decaying counter v0.40.2 was written to protect. A `lastSeen` that lands inside the driver-WS reconnect burst (`DRIVER_BURST_MS`, 2 min, plus a 5 s lead) is therefore classed `attribution-unknown` and credits nothing. The window is BOUNDED on purpose: long after the burst a node's voice is its own again, so a reconnect can never suppress self-proof for the life of the process.
...
| Controller RF off | The controller reported `Turning RF off` on the driver log stream and has not reported it back on (v0.65.0). The reading expires after `RF_OFF_MAX_MS` (2 min), on a dark log lane, and on any driver-WS reconnect | The nightly NVM backup takes the radio down for ~10 s and soft-resets the controller (observed 07:00 UTC, both nights of the 2026-09-15 audit). A frame sent across that window cannot be acknowledged, and ONE unacknowledged sweep frame is enough for the driver to mark a node Dead — a false death this engine would then report, remediate and count. Probes already in flight when the radio goes down are dropped **unjudged** rather than booked as misses: an unanswered frame there measures the radio, not the node |
