...
| Controller RF off | The controller reported `Turning RF off` on the driver log stream and has not reported it back on (v0.65.0). The reading expires 2 min (`RF_OFF_MAX_MS`) after the controller LAST reported RF off — each `off` line refreshes it — and also on a dark log lane and on any driver-WS reconnect | The nightly NVM backup takes the radio down for ~10 s and soft-resets the controller (observed 07:00 UTC, both nights of the 2026-09-15 audit). A frame sent across that window cannot be acknowledged, and ONE unacknowledged sweep frame is enough for the driver to mark a node Dead — a false death this engine would then report, remediate and count. Probes already in flight when the radio goes down are dropped **unjudged** rather than booked as misses: an unanswered frame there measures the radio, not the node. Two limits, stated so the guarantee is not read wider than it is: the drop can only act on a tick that sees the reading, so for a blackout shorter than the 60 s tick it covers the ticks that land inside the window and not the probe launched just before it; and a probe whose node was already seen to go Dead on it is settled first and stays booked — that death was observed with the radio on, and keeps its v0.64.4 dwell exemption |
