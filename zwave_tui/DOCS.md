...
> **A manual ping is dated from its launch (v0.64.5).** An operator's `p` is judged by the same `judgeProbeAnswers` as an engine probe (v0.47.0), but its pending entry was stamped by `notePending` when the data layer heard the outcome — after Home Assistant's `button.press` call had returned. HA's zwave_js ping button awaits the driver's ping, and the service call awaits the button, so the node's answer can already be on record by then (on the reference mesh the driver logs a revived node alive 54–197 ms after the ladder's probe goes out). The stamp post-dated that answer, `lastSeen >= at` failed, and an answered manual ping was logged *did NOT answer* after the 90 s grace, with a miss on the node's consecutive-miss streak; the same test in `settleProbeDeath` could book it as the probe a later, unrelated death happened on. `zwaveActions.run()` now reads the clock before awaiting the call and passes it as `sentAt` through `onOutcome` → `recordActionOutcome` → the probe hook → `notePending(nodeId, 'manual', at)`. The runner's `now` and auto-ping's both default to `Date.now`, the wall clock `lastSeen` is compared against. The stamp now precedes the send, as every engine lane's does, so a frame the node happens to send between launch and transmission (the HA round trip plus the driver's send queue) counts as the answer — the sweep's tolerance, applied to the operator's lane. The persisted reply rate and the ledger's action arm are unchanged: the manual lane still feeds neither through this path. Manual kills still keep the dwell, for different reasons than v0.64.4 gave. No manual kill has been observed — the 50.5 h of log reviewed for this release holds 1,081 ping-button presses, every one the engine's — and `probeDeath` discards traffic heard before the death. After a manual ping that traffic is most likely the operator's own command: the give-up notice tells them to operate the device and then ping it, and for a device that ignores NOPs the exemption would trade the v0.42.0 Dead-but-talking notice for an immediate retry of the frame it ignores.
