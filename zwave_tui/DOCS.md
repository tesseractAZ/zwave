...
before it (source: `main()` in `server/src/index.ts`):
...
| 4c | Auto-ping | `startAutoPing` | The engine's one autonomous mesh write (§11.12), started only when both `auto_ping_enabled` and `write_actions_enabled` are on (with only the first on, a warning is logged and nothing starts). The dead-node ladder rides the runner's learning `ping`; the liveness sweep and verification probes ride non-learning verbs (v0.38.1) — since v0.71.0 the routed read `routedRead(n, 'probe')` wherever the node has a switch/light value, with the NoOp `probe` as the fallback. It also reads two v0.65.0 driver-side readings: the controller receiver-off accessor (`controllerRfOffSince`), which suppresses every lane, and the driver-reconnect stamp (`driverReconnectedAt`), so a driver restart's mesh-wide ping is never credited to a node as self-proven. |
| 5 | Auth | `createAuth` + `createAuthPolicy` | CORS/ws-origin allow-list and the direct-LAN login gate. |
| 6 | HTTP + ingress console | `Fastify` + `registerWsConsole` | The `/console` xterm.js page, `/console/ws` transport, `/`, `/api/version`, `/api/health`. `/api/health` returns 200 only when the HA socket is authenticated **and** the roster is loaded **and** the provider reports no error; its body carries `ok`, `ready`, `nodes`, `haReady`, `lastUpdated`, `lastStatsUpdated`, `error` (the roster's), and — from v0.35 — `haError`, the HA socket's **own** last failure. The two are distinct and the second was previously unreachable: an auth rejection or a refused connect leaves the data layer with nothing to report but "not ready", so the only line naming the actual cause was the one the endpoint did not print. From v0.57.0 the body also carries the engine's conclusions — `degraded`, `degradedReason` and an `engine` object mirroring every published entity (§12.11) — while the HTTP code still tracks `ok` alone. |
...
- **Read-only telemetry only**: the `start_listening` state dump plus
  `statistics updated` events for `backgroundRSSI` and `lastSeen` (node flags
  come from the dump alone), plus driver `logging` events matched client-side
  for S2 resyncs (v0.26) and the controller's `Turning RF off` / `Turning RF on`
  (v0.65.0), their payloads never stored.
...
(`zwaveData.ts`). This guard gates *every* consumer of driver data — the
...
rebuild them — and they die just as surely when the `zwave_js` **config entry
reloads**, which leaves the socket up and fires no event at all (v0.65.0, §2.5); failed per-node subscriptions are retried on a 60 s timer rather
...
which misses sub-window flaps), and (b) `timeoutResponse`, a reply-expecting
command (a Get, a supervised Set, or a secure send's nonce Get) whose reply
never came while the node stays Alive.
...
synchronously (so the very first frame has data, not nulls), then the telnet
server binds, then Fastify, and — last — the Home Assistant states publisher
starts (§12.11). `main().catch()` logs `FATAL:` and exits 1
on any bootstrap rejection.

**Shutdown** is the reverse, ordered so no consumer reads a half-torn producer
(`shutdown` in `index.ts`). A `closing` guard makes it idempotent across
`SIGTERM` / `SIGINT`:

```
telnet.stop()      // stop accepting/serving TCP sessions
haStates.stop()    // clear the Home Assistant states publish timer
...
`start()` in zwaveData.ts registers an `onReady` handler on the HA WebSocket client and kicks the first `tick()`. The `onReady` handler is the spine of the whole integration's resilience: **every (re)authentication reloads the registry join and re-establishes the statistics subscriptions**, because those subscriptions are per-connection and die when the socket closes.

**A socket that stays up is not a feed that stays alive (v0.65.0).** The subscriptions also die when the `zwave_js` config entry reloads — Home Assistant binds each one to the `Node` objects of the driver that is loaded at subscribe time, and a reload replaces them, silently, with no error and no close on the subscriber's side. The socket never closes, so `onReady` never fires and nothing rebuilds the feeds. The 2026-09-15 live audit measured what that costs: a driver restart at 03:05 UTC left the engine blind for the remaining 22 h 48 m of a 23 h run, every detector starved, every episode closing `unverifiable`, while auto-ping — judged from the separate driver-WS `lastSeen` — kept logging healthy-looking verdicts. `tick()` therefore latches a `not_loaded` failure and rebuilds the feeds on the first roster poll that succeeds afterwards, and a watchdog rebuilds them anyway if no statistics event arrives **from the mesh — any node *or* the controller** — for `STATS_FEED_DEAD_MS` (10 min) while the socket is live (§2.5). Events that arrive within `STATS_REPLAY_GRACE_MS` (15 s) of a feed rebuild starting do not count as arrivals, for this watchdog or for `binary_sensor.zwave_tui_degraded`, which measures the same silence (v0.67.0): subscribing replays every node's and the controller's current snapshot, so without the window a re-arm that failed to revive the feed would zero the very clock that measures the silence, and sustained blindness would become a ten-minute sawtooth the sensor could never hold `on` through. The window is fixed and short, so a wedged rebuild cannot suppress the stamp indefinitely. It opens for the feed-set subscribe, meaning the first one and every re-arm, and not for the per-node retry timer's resubscribes. Any statistics event after it closes still marks the feed alive. The controller feed is the bounded-latency half of that signal: zwave-js polls background RSSI about every 30 s whenever the send queue is idle, so in health the stamp refreshes twice a minute however quiet the nodes are, which is what gives the ten minutes meaning. Node feeds alone could not: on the reference mesh a healthy node is legitimately silent for up to ~54 min. Both feeds are bound to the same driver, so a reload orphans them together — which is why one stamp is enough, and also why a *partial* feed death (one node's subscribe failing while the rest live) is NOT what this watchdog detects; that is covered per node by the subscription bookkeeping and the retry timer.

The re-arm releases the orphaned subscriptions first, keeps the cached counters, and is throttled to one rebuild per window so a Core that is still recovering is not stormed. Two things the throttle deliberately does *not* do (v0.65.0 review): it does not spend the reload latch — a reload that arrives inside the window stays latched and rebuilds the moment the window expires, rather than being dropped and left to the watchdog — and it does not let a re-arm run beside a rebuild already in flight, which would clear the "a run owns this connection" flag under it and double every feed on a socket that never closed.
...
The ordered pipeline, per `refresh()` in zwaveData.ts:
...
Most `zwave_js/*` commands need a config-entry id, and it is **not** stable across a re-add of the integration. `ensureEntryId()` in zwaveData.ts resolves it lazily:
...
- **Self-heal** (in `tick()`, zwaveData.ts): after `errStreak >= 3` consecutive failures on an *auto-discovered* entry, the id is assumed stale (integration removed + re-added → new `entry_id` + `device_id`s). The layer nulls `entryId`, forces a registry reload, clears the frozen `statsByNode`/`batteryByNode`/`firmwareByNode`, and calls `client.reconnect()` so the old socket's subscriptions are released before re-subscribing. A seeded entry is left untouched by this path. Sparkline history (`histByNode`/`histLongByNode`) is deliberately **not** cleared — it is keyed by Z-Wave node id, which is stable across a config-entry re-discovery.

### 2.3 The device + entity registry join

`network_status` speaks only in numeric node ids. The registry join (`ensureRegistries` → `buildRegistryMaps`, in zwaveData.ts) supplies everything else — names, areas, and the entity ids the TUI needs to ping, read battery, and log activity.

**Identifier → node id.** Z-Wave JS device identifiers are tuples shaped `['zwave_js', '<home_id>-<node_id>...']`. `nodeIdOfDevice()` in zwaveData.ts extracts the node id as:

```
Number(identifier[1].split('-')[1])   // '<home_id>-<node_id>' → node_id
```

The controller is **node 1** (`is_controller_node === true || nodeId === 1`; `buildNode()` in zwaveData.ts).
...
Only `platform === 'zwave_js'` entities that are **not** `disabled_by` and whose `device_id` resolves to a known node are indexed. Names come from `name_by_user || name` (device) and `original_name ?? name` (entity), and every externally-sourced string is run through `sanitizeLabel()` in zwaveData.ts, which strips C0/C1 control bytes (including ESC `0x1b`, so a crafted device name cannot inject ANSI into a TUI frame), folds wide/astral code points to `?`, and caps length at 48. The join is rebuilt fresh on every (re)auth so a removed node leaves no stale mapping.

**Live entity state + config parameters (v0.22).** Two node-scoped caches feed the Detail dossier's new sections, both exposed on `DataProvider` as `entityStates(nodeId)`, `configParams(nodeId)`, and `requestConfigParams(nodeId)`:

- **`stateByEntityId`** — the current `{state, attrs}` of every tracked mesh entity, seeded in one `get_states` pass (`fetchEntityStates`, on the same slow cadence as the battery/firmware poll) and kept fresh by the existing `state_changed` subscription (`updateLiveState`, which updates on *every* change — including attribute-only ones like a dimmer level moving — independent of the throttled value-log). `attrs` is bounded to a display whitelist (`pickDisplayAttrs` — brightness, percentage, temps, position, unit, device_class, …) so the cache stays small. Both the state string and every string attribute pass through `sanitizeLabel()` at this boundary, exactly like device names — a device-reported state can never inject a newline/ESC into the exact-rows frame. `entityStates(nodeId)` joins the node's `NodeEntity[]` (via `deviceIdOf`) with this cache; the per-domain rendering lives in the pure `formatEntityState()` in `detail.ts`.
- **`configByNode`** — a *lazy* per-node config-parameter cache. The Detail screen calls `requestConfigParams(nodeId)`, which idempotently fires `zwave_js/get_config_parameters {device_id}` and flips the entry through `idle → loading → ready | error | unsupported`; `mapConfigParams()` sorts the raw response by parameter `property` and resolves each value's enum label from `metadata.states`. A failed fetch retries only after `CONFIG_RETRY_MS` (15 s) so a screen re-requesting every frame can't hammer HA; `ready`/`unsupported` are terminal (never re-fetched) — which is why cache eviction matters.

Both caches are **node-id / entity-id keyed and must not alias across devices**. They are cleared on a home-id change and on the self-heal re-discovery, and — the load-bearing case — `buildRegistryMaps` evicts `configByNode` for any node whose **backing `device_id` changed** (A→B under `replace_failed_node` node-id reuse, or `null`→present when a node gains its HA device later, or present→absent), which subsumes the plain removed-node prune; `stateByEntityId` is pruned to the freshly-rebuilt `entityIndex`. A departed node (absent 5 min) also evicts both. Because `client.reconnect()` rejects every in-flight WS send, an async config fetch can never resolve `ready` *across* a device flip (the flip only happens on a reconnect that already rejected the pending send) — so no stale-config write-back is reachable.

### 2.4 The `network_status` roster poll

The core poll (in `refresh()`, zwaveData.ts) is the cheapest complete mesh snapshot:
...
Each raw node is mapped by `buildNode()` in zwaveData.ts into a `NodeSnapshot`. The status field is the Z-Wave JS `NodeStatus` enum (types.ts):
...
`buildController()` in zwaveData.ts constructs the `ControllerSnapshot` from the `controller` object: `home_id`, `sdk_version`, `firmware_version`, `rf_region` (via `RF_REGION_LABEL` — region 9/11 are the Long-Range variants), `is_primary`, `is_suc`, and `is_sis_present` (note the lowercase `sis` in the raw key). It also latches the rebuild-routes clock: on the `is_rebuilding_routes` false→true edge it stamps `rebuildStartedAt = Date.now()` so the UI shows honest elapsed time (HA exposes only the boolean — no per-node progress — so the UI never fabricates a percentage).
...
- **Departed-node eviction** (`evictAfterMs`, in `refresh()`, zwaveData.ts): a node absent from the roster for **5+ minutes** is treated as removed (excluded / `replace_failed_node`) and its evidence + all node-id-keyed caches are evicted, so node-id reuse can't merge two physical devices' histories.
- **Network-identity guard** (in `refresh()`, zwaveData.ts): if the controller's `home_id` changes (stick swap / different NVM restore), *every* node-id-keyed cache — stats, sparklines, evidence accumulators, baselines, outcomes, driver telemetry — is dropped and a full reconnect + re-discovery is forced. A plain reconnect keeps `home_id` stable, which is exactly what lets sparkline persistence survive an HA-Core restart.

### 2.5 Live statistics subscriptions

Polling gives the roster; the *statistics* arrive push-driven. `subscribeStatistics()` in zwaveData.ts is idempotent per connection (guarded by `statsSubscribed`) and opens three kinds of subscription. It **retains** the handles it creates (v0.65.0): a config-entry reload orphans every one of them on Home Assistant's side, and `rearmStatsFeeds()` needs that same list to release them before rebuilding — re-subscribing without releasing doubles every event on the socket. Subscribing delivers each node's **current** statistics immediately, so the roster fully populates within seconds without any pinging.
...
`onControllerStats()` in zwaveData.ts reads the host↔stick serial-link counters. The raw keys are snake_case, and **one is misspelled by HA's dev source**: the response-timeout key arrives as `timout_response` (missing the second `e`). The handler accepts either spelling so an upstream fix can never silently zero the field:

```
const tRes = num(e.timout_response) ?? num(e.timeout_response);
```

The controller counters cached into `ctrlStats` (typed `ControllerSnapshot['statistics']`, types.ts): `messages_tx/rx`, `messages_dropped_tx/rx`, `nak`, `can`, `timeout_ack`, and `timout_response` → `messagesTX/RX`, `messagesDroppedTX/RX`, `NAK`, `CAN`, `timeoutACK`, `timeoutResponse`. Per RESEARCH.md §2.11 these are *host↔stick* counters, not per-node RF metrics — they measure the serial link, and the engine reads them only for serial-link health.

**Per-node statistics + status** — two subscriptions per end node (node 1 excluded, covered by the controller feed). `subscribeNode()` in zwaveData.ts is *per-feed idempotent* — a retry re-attempts only the feed that failed, because re-subscribing a live feed leaks a duplicate subscription and double-counts every subsequent event:
...
`onNodeStats()` in zwaveData.ts maps the raw event into `NodeStats`. Two subtleties:

- **Node-id key drift.** `statsNodeId()` in zwaveData.ts accepts **both** `nodeId` (camelCase) and `node_id` (snake_case). Through HA 2026.8.3 the *initial* on-subscribe event carried `nodeId` while every *subsequent* live push carried `node_id`; from HA 2026.9.0 both use `node_id`. Accepting both spellings covers either: on the older releases, reading only `nodeId` would freeze every node's stats at their subscribe-time values, and on 2026.9.0 or later it would drop every event.
- **Counter validation.** `statsCounters()` in zwaveData.ts requires **all five** cumulative counters (`commands_tx`, `commands_rx`, `commands_dropped_tx`, `commands_dropped_rx`, `timeout_response`) to be finite numbers, else the event is rejected whole. Coercing a missing field to 0 would re-baseline the evidence deltas at zero, and the next real event's cumulative value would then land as one giant fabricated "valid" delta.

The `NodeStats` shape (types.ts):
...
interface NodeStats {
  ...
  lastSeen: number | null;           // epoch ms: arrival of a stats event in which commandsTX/RX/DroppedRX moved; cached unmerged (see below)
}
```

**Which counters mean "heard" (v0.64.6).** Home Assistant's node statistics carry no `lastSeen`, so `onNodeStats` stamps the arrival of an event in which a counter moved. Only three counters qualify: `commandsTX`, `commandsRX` and `commandsDroppedRX` — the zwave-js increments that sit beside the driver's own `lastSeen` update (a send the node acknowledged, or a frame received from it, decodable or not). `timeoutResponse` moves when the node did NOT answer a command that expected a reply, once the report timeout (1 s by default) plus the command's round-trip time — and a nonce exchange's, when a secure send fetched one first — has passed since the acknowledgement that already moved `commandsTX`, in its own throttled statistics event. Through v0.64.5 a timeout moving alone stamped "heard" at the moment the node went silent — and auto-ping reads this stamp as a probe's answer (§11.12). `commandsDroppedTX` is excluded defensively: on zwave-js 15.27.1 a plain NoAck throws before its only increment, which runs only after a send cut short because the node's S0 nonce report or S2 SOS nonce report arrived before the ACK (RESEARCH §2.2), when the node was just heard. A replay with no movement carries the previous stamp; a first delivery stamps nothing. The evidence sampler reads this cached stamp unmerged (`isFreshSample`, §5.4). `NodeSnapshot.stats` — built by `buildNode()` through `mergedStats()`, and what the screens, health scoring, the symptom detectors and auto-ping read — carries the later of this stamp and the driver-WS `lastSeen`.
...
`onNodeStatusEvent()` in zwaveData.ts maps the event name to a status and counts every crossing of the Dead boundary:
...
The event feed is seeded from the roster on subscribe, so the *first* real event (e.g. a genuine Alive→Dead) diffs against known status instead of being swallowed. Nodes **without** a live status feed (subscribe failed, retry pending) fall back to the roster-poll status diff (in `refresh()`, zwaveData.ts), which counts a Dead-boundary crossing into the same `flapAccum` — the event feed is primary, and `statusSubbed` membership prevents feeding both and double-counting. `flapAccum` and `routeChangeAccum` are drained per evidence sample (§ the evidence chapter) so that sub-window flaps are attributed to the correct window.
...
2. **`timeoutResponse`** — the node MAC-ACKed a reply-expecting command (a Get, a supervised Set, or a secure send's nonce Get), so the RF link up to the ACK demonstrably works, but the expected report never arrived; zwave-js increments this and the node **stays Alive**. It is a return-path / responsiveness problem, and it accrues only for that reply-expecting traffic. On zwave-js 15.27.1 a supervised Set travels inside a Supervision Get (`SupervisionCCGet` declares `@expectedCCResponse(SupervisionCCReport)`), so the SendData message's `expectsNodeUpdate` is true (`SendDataRequest`, or `SendDataBridgeRequest` on a controller that supports bridge sends, as on the reference mesh; the two share the method body), a missing report makes `waitForNodeUpdate` throw `Controller_NodeTimeout`, and Driver.ts `sendMessage` increments `timeoutResponse`. An unsupervised Set expects no reply of its own, so a plain one never accrues it. A secure send that holds no usable nonce (S0 with no free nonce, S2 with no usable SPAN) first sends a nonce Get, which expects a Nonce Report: if the node acknowledges it and never answers, the same timeout increments `timeoutResponse` and the command itself is never sent, supervised or not (source reading, not reproduced; every nonce Get in the reference mesh's saved driver logs was answered).

**The TMO metric.** The one shared definition lives in `responseTimeoutPct()` in health.ts:
...
- **Reply-expecting numerator.** Because `timeoutResponse` accrues only for reply-expecting traffic (a Get, a supervised Set, or a secure send's nonce Get) while `commandsTX` counts all successful sends, TMO is a *conservative under-estimate* for a node whose traffic is mostly unsupervised Sets that need no nonce — an honest floor, never an over-statement.
...
- **Ping** is not a `zwave_js/*` WS command — it is a **button entity press**. `pingEntityOf(nodeId)` resolves the node's discovered `button.*_ping` entity, and the action calls `call_service { domain: 'button', service: 'press', service_data: { entity_id } }` (the `ping` verb in `createActionRunner()`, zwaveActions.ts). It is treated as safe/idempotent. (The `zwave_js.ping` HA *service* is deprecated and returns nothing; the raw `invoke_cc_api` NOP-ping can mark a marginal node DEAD, so the button entity is the sanctioned path.)
- **Heal / "rebuild routes"** is `zwave_js/rebuild_node_routes { device_id }` (the `healNode` verb, zwaveActions.ts), with the network-wide `begin_rebuilding_routes { entry_id }` / `stop_rebuilding_routes { entry_id }` variants.
...
- **`controlEntity(nodeId, entityId, verb)`** actuates an HA entity via `call_service`. The verb→service mapping is the pure `zwave/entityControl.ts` (`resolveService`): light/switch/fan/siren/input_boolean on/off/toggle → `homeassistant.turn_on|turn_off|toggle`; cover open/close/toggle → `cover.open_cover|close_cover|toggle`; lock/unlock → `lock.lock|lock.unlock`. `resolveService` returns `null` for any verb a domain doesn't offer, so a mismatched call (e.g. `turn_on` a lock) can never be formed. High-stakes verbs (unlock, open a garage) render a **DESTRUCTIVE** badge, but — like every menu action — all require the typed CONFIRM regardless.
- **`setConfigParam(nodeId, param, value)`** writes `zwave_js/set_config_parameter { device_id, property, [property_key], [endpoint], value }`. The `property`/`property_key`/`endpoint` come from the parameter's HA value-id (parsed by position in `mapConfigParams`, not by grabbing the key tail — which for a partial parameter is the `property_key`, not the `property`). The value is bounded before it is sent: enum parameters offer only their `states`; numeric ones are checked against the device `min`/`max`, with a signed-32-bit sanity floor when the device reports no bounds. A successful write fires `onConfigWritten`, which invalidates the node's config cache so the Detail dossier re-reads it.
- **`onNodeRemoved(nodeId)` (v0.35)** fires after a **successful** `removeFailed` and only then, calling `zwaveData.forgetNodeBaselines(nodeId)` → `BaselineStore.resetNode` (§7.3). Once the node has left the mesh, a later re-include on the same node id is different physical hardware, and measuring it against the departed device's learned normals is how the engine manufactures symptoms out of a device swap. A **failed** removal fires nothing: the node and its history are both still present, and discarding a live node's baselines is the larger harm. The hook covers only removals issued from THIS TUI; the departed-node eviction sweep (§5.7's roster dwell — 5 minutes absent) calls the same `forgetNodeBaselines` for every other exit path — HA's own exclude, `replace_failed_node` — so no path leaves a dead device's yardstick keyed to a reusable id (v0.35 review). The two are idempotent.

Both are **operator** ops, not remediations: `run(..., learn=false)` skips the M5 outcome hook, so toggling a light or setting a parameter is never mis-attributed to an open symptom episode. `write_actions_enabled=false` hard-disables the whole runner — the menu still opens (read-only, to *read* each action's impact) but nothing executes.
...
Shadow-key migration is worse, not better: `bashio::config.exists` cannot distinguish a value cleared to `""` from an absent one, and `users` is a structured list that cannot be shadowed at all, so a restrictive merge locks the operator out of telnet, the direct port and the sidebar simultaneously.

> **Never nest or rename an existing option key to tidy this form.** If a future release must regroup, it needs a major version, an explicit breaking note, and a documented manual re-entry step. `write_actions_enabled` is filed under `Safety · `, not `Display · ` — it is the master gate for mesh-mutating actions, not a display preference.
...
| `9` | engine | `screens/engine.ts` | *(engine — §4.15)* |

Overview is the **home**; the other eight render as full-frame overlays over it. The context object each renderer consumes is `ScreenCtx` (`types.ts`):
...
`SORT_ORDER = ['health', 'id', 'name', 'rssi', 'seen', 'symptom']`. `visibleNodes()` applies the substring filter (over name / id / manufacturer / model / status label) first, then sorts: `health` worst-first by `data.scoreFor().score`, `rssi` weakest-first, `seen` most-stale-first, `symptom` (v0.58.0) crit-symptom nodes first, then nodes with any lesser symptom, then the rest, worst score first within each group — each with a `nodeId` tiebreak. RSSI sorting/scoring skips the driver sentinels `{127, 126, 125}` — `effectiveRssi()` maps a null or sentinel reading to `-999` so unknown-signal nodes surface at the "weakest" end rather than being treated as strong.
...
**The Detail screen's private navigation** (`applyDetailKey` in `input.ts`, v0.22) owns the dossier scroll (`view.detailScroll`, the first visible body row) because the per-node dossier is now taller than a terminal: `↑`/`↓`/`j`/`k` scroll one row, `space`/`b` page (≈ `rows − 4`), `g`/`G` jump to top/bottom. Because scrolling claims `j/k`, **node stepping moves to `<`/`>`** (with unshifted `,`/`.` aliases), which advances/retreats `view.selected` and resets `detailScroll` to `0` so a new node opens at its header. Every Detail entry path — `Enter`, the Log-screen jump, `<`/`>`, and the `2` screen key — resets `detailScroll` to `0`. The renderer clamps `detailScroll` to the real content height and **writes the clamped value back** into the view each frame (the same sticky-window pattern the Log screen uses for `logScroll`), so the input layer never needs to know how tall the dossier is; the current window rides as an `a–b/N` token in the title rule.
...
Screens whose bodies can overflow (Controller, Topology, Heatmap, Log, Detail) each compute their own `bodyCap = max(1, H − 3)`. Most emit a `…N more` marker or a scroll window rather than letting `frame()` silently clip real data; **Detail scrolls** its full-height dossier and pins the flag legend as the last body row (see §3.7).
...
- `chartRows(values, width, height, {min,max,color,colorFor})` — **v0.28**: the same series a `sparkline` compresses into one row, drawn across `height` rows at sub-cell resolution (each row is 8 levels, so a column is measured in eighths and split into full cells plus one partial cap). Scaling follows the `sparkline` rule — auto-scale over what is **drawn**, never over samples that have scrolled off, or a long-past extreme flattens the visible trend against an invisible one. `values` accepts `null`: a null is **not** zero but "not measured", it holds its position so columns stay aligned with any axis or strip beside the chart, and it is marked with a `·` on the **baseline only** (a full column of dots would itself read as data). `colorFor(v)` colours each column by its own band, which a chart drawn above a banded strip must do or the two contradict each other for the same number.
- `spinner(nowMs)` — braille spinner advancing at ~120 ms/frame; `fmtElapsed(ms)` → `45s` / `3m12s` / `1h05m`.

**Column composition (v0.27).** `chrome.ts` adds `hstack(cols, gap)` and `splitCols(total, n, gap, min)`. `hstack` returns a block whose height is the **tallest** column with the rest blank-filled, and every row exactly `Σw + gap × (n−1)` visible columns: each cell is truncated to its **own** width and padded back up, so a long line can never shove its neighbour rightwards, and the padding is ANSI-aware so a cut cell cannot leak an unterminated colour across a boundary. `splitCols` returns nothing rather than dividing a frame below a usable pane width — the caller's signal to fall back to fewer panes, because density bought by truncating values is not density.

> **Use it on lists, not on blocks.** Content must be **built at the column width**, never rendered at the frame width and cut afterwards: `hstack`'s trim is left-anchored and takes whatever a row right-aligned. Detail's entity and parameter lists are the right case (short rows, one per line, wasting width). Controller and Interference are not — v0.28 tried it, measured it, and reverted it: their blocks are gauges, bar rows and field strips *designed* to use width, so narrowing costs information. The check that settles it is **ink**: layout alone cannot change the number of characters drawn, so a drop means characters were destroyed and a rise means information was added.
...
**Mesh roll-up (v0.27).** Below the roster the Overview draws a compact roll-up — member count and alive/dead/asleep/unknown tallies, an A–F grade distribution, and a worst-nodes work queue — funded **strictly by surplus**: it is drawn only when every visible node already has a row *and* rows remain, so it can never push the roster into scrolling. Membership matches the Controller screen's NETWORK HEALTH exactly (node 1 excluded — this is the mesh the controller serves, not the controller itself), because two screens reporting different mesh totals would be a worse defect than the blank rows this replaced. Its grade colours come from the roster's own `scoreColor` via each grade's real band floor (`gradeFor`: A≥90, B≥80, C≥70, D≥55, else F) rather than a second letter→colour table. The `NODE` column's cap rose 40 → 64 in the same release: the fixed columns and separators total 107, so the old cap froze content at 147 columns and left 53 dead columns on every body row at 200 wide.
...
- **Detail** (`detail.ts`) — a **scrollable eight-section per-node dossier** (IDENTITY / LIVE LINK / **LIVE ENTITIES** / **CONFIG PARAMETERS** / ROUTES / TRAFFIC / **EVIDENCE** + a pinned flag legend). As of v0.22 the dossier is taller than a terminal, so the body scrolls (`view.detailScroll`, §3.2) instead of shedding graphics: the health/battery gauges, SNR meter, and RSSI/RTT/coarse trend sparklines are all kept as scrollable rows (a graphic still returns `null` and is skipped when it can't fit its *columns*). **LIVE ENTITIES** joins every HA entity on the node with its current state via `data.entityStates(nodeId)`, formatted per-domain by the pure `formatEntityState()` — a light's `on · 72%`, a sensor's value + unit, a `binary_sensor` read through its `device_class` (motion → *detected*, door → *open*), climate mode + setpoint/current temp, cover open/closed + position, lock state, firmware-update availability, and a button/event's last-fired age. **CONFIG PARAMETERS** lazily fetches the device's Z-Wave configuration via `data.requestConfigParams(nodeId)` / `data.configParams(nodeId)` (see §2.3) and lists each parameter's label · value + unit · enum meaning, marking read-only params `(ro)`; it shows an honest *loading / unavailable / none* line while resolving. **EVIDENCE** (v0.35) is the section about the *engine* rather than the node: everything above it is what the device reported, and this is whether anyone was listening. It reads `DataProvider.evidenceCoverage(nodeId)` (§5.7) for the cumulative sample count, the **fresh** share, and a live/down badge per feed; `evidenceCoarse(nodeId)` for the persisted history span; and `rssiNormal(nodeId)` for the engine's own learned yardstick. Since v0.71.0 the sweep's **Probes** row is followed by a **Frame** row — `all NoOp pings: answered = reached on a stored route`, `all routed reads: answered = reached by any route`, or the lifetime tally split `read k/a (p%, any route) · ping k/a (q%, stored routes)` with each rate toned on its own, the longest whole form that fits — and, above zero, a **Rerouted** row counting sweep reads that failed over from a stored route (§11.12). Three honesty rules govern it. (1) A node whose status **and** stats feeds are both down is named a **MONITORING HOLE** in red — silence from an unwatched node is not health, and every quiet verdict elsewhere on the dossier silently depends on the opposite. (2) The fresh share is labelled **lifetime** and toned by band (≥80 % green, ≥40 % yellow, below that red) — the counters are cumulative since first sight, so this is a *chronic* share that cannot move fast; a node that went stale yesterday still shows last month's green here, and the label says so rather than implying a now-reading (v0.35 review). Zero samples renders a dash, never a confident `0 %`. (3) An **un-graduated** baseline says `still learning · Nd so far — not yet a yardstick` rather than quoting a median: quoting one is indistinguishable on screen from a learned band and is actionable exactly when it must not be. A graduated band is suffixed `· this time-of-day band` — the store keeps a separate normal per 4-hour band and the row answers for the band containing *now*, so the same node legitimately shows a different yardstick at 3 am and 3 pm; unlabelled, that read as the baseline contradicting itself. The `History` figure is labelled a **span** (first persisted bucket to now), not coverage — an observation gap lives inside it. And the feed badges mean *subscription AND socket*: the idempotency sets survive a disconnect, so unguarded they would glow green through the largest monitoring hole there is — an HA-socket outage — which is exactly when the MONITORING HOLE line must be able to fire. IDENTITY additionally carries the **HA device id** (v0.35) — the string a `device_id:` automation target needs, omitted rather than blanked when the registry join failed. TMO here is the same `responseTimeoutPct`; the raw `commandsDroppedTX/RX` counters live honestly in TRAFFIC. Long-Range nodes show `direct to controller (Long-Range star)` instead of a mesh route. The Detail screen itself is **read-only** — it surfaces state + configuration but changes nothing; *acting* on a device (turning it on/off, unlocking a lock, writing a config parameter) is done from the **Actions Menu** (`a`), behind the write-actions + type-CONFIRM gate (v0.23, §2.8).
...
- **Topology** (`topology.ts`) — end nodes bucketed by LWR hop count (Direct / 1 / 2 / 3+, plus Long-Range and route-pending groups), an optional hop-distribution histogram (shown only when `bodyCap ≥ 15 && W ≥ 64`), per-node route `signalBars` (only when `W ≥ 72`), and a pinned **Repeater-load panel** where a *full* bar is *bad* (a repeater carrying many nodes is a single point of failure, coloured by load not fill). The route tree **scrolls** (v0.24 — `↑↓`/`j`/`k`, `space`/`b`, `g`/`G`) with a footer reading `▴N ▾M more · ↑↓ scroll · X–Y/Z`; the repeater panel is always kept. A second, **leftover-funded Route-stability panel** (v0.34) reports what the routing has actually DONE over days: `DataProvider.routeStability(nodeId)` sums the same `dRouteChanges` accumulator the `route-churn` detector reads (§7.2.4) across the persisted coarse tier, and carries the measured span so the claim cannot outrun its window. Its purpose is epistemic as much as visual — route-churn has never fired on the reference mesh, and this panel is what separates *the mesh is stable* from *the detector cannot see*. **Zero is a finding, not an empty state:** it renders as one line (`every path held — N node(s), Xd measured, zero re-routes`) rather than N rows each restating `0`; when paths HAVE moved it ranks them worst-first BY that per-**day** rate — the same number each row shows — since stores of different ages are not comparable by raw count and the detector's own threshold is a rate. The reported span is the SHORTEST node window, never the longest: it sits beside a count of ALL nodes, so it must be the window every one of them actually has. An **empty** measurement (`hours: 0`) renders nothing at all — a confident zero over no data is precisely the fabrication the collapse-method discipline forbids. It spends only rows the tree would have left blank, so a scrolling tree never loses a row to it; at 80×24 the frame is byte-identical to v0.33. It previously windowed from index 0 under a `…N more (taller terminal shows all)` note that was false: the tree is ordered shallowest-first, so overflow always cut the deep-hop, Long-Range and route-pending groups — the anomalies the screen exists to show — and 39 nodes need roughly 45 lines. A third, also leftover-funded **Route-failures panel** (v0.35) draws the persisted `routeFailedBetween` history (§5.7) — the one quantity on this screen that names a **link** rather than a node. It tallies by the **pair, not the reporter**: one marginal hop that six nodes all witnessed is one row reading `n12 ⇢ n44   Hall Dimmer → Shed Lock   6 failures · last 4m ago`, not six rows each reading `1`. The panel is empty and costs nothing on a healthy mesh. Both leftover-funded panels draw from the same pad, and **failures claim it first, but bounded to half** (floor 3): a link-level finding is strictly more actionable than a node-level one, yet the stability list will happily enumerate every churning node on a 39-node mesh and take everything. Stability degrades honestly under that pressure — the rows it stops showing stay counted in its own `+N more` disclosure, so it becomes a shorter panel, never a smaller claim.
...
- **Log** (`log.ts`) — the live activity stream. `logLayout(rows)` splits the frame into a list and an optional detail pane: the detail pane (`LOG_DETAIL_ROWS = 9`) appears only when `rows ≥ LOG_MIN_ROWS_FOR_DETAIL = 22`. Rows are `cursor · time · 3-letter kind tag · #node name · text`, coloured by severity. Errors latch **bold-red until acknowledged**: `M` (v0.33) releases the selected error's latch to plain red via `DataProvider.ackEvent(seq)` — keyed by `seq` so the head-inserting ring cannot drift the target, error-only and once-only by refusal, and SHARED (the ring is one control-room log, so an ack means *a human has seen it*, not *this terminal has*). Not a mesh write, so it sits outside the `write_actions_enabled` gate. The list window uses the shared `windowStart`, and navigation/window math live together in `input.ts` so cursor and viewport can never disagree. The detail pane's **Entity** row leads with the entity's friendly `name` and keeps the `entity_id` as the secondary (v0.35) — but only when the id still fits beside it. `field()` truncates blindly from the right, so on an 80-column frame a leading name pushed the id past the cut, clipping `sensor.node_27_illumina…` into a different, plausible id (v0.35 review). The id is the one part that must never be mangled — it is what you type into HA — so when both cannot fit, the id wins and the name is dropped, which is exactly the pre-v0.35 row. The name itself was captured on every value event by `mapStateChanged` and discarded at the render, so a pane whose entire job is *which thing did this?* answered with a slug while `Back Porch Motion` sat unread on the same object.

### 3.8 The anti-flicker 1 Hz draw loop

Both transports drive the same cadence: `setInterval(() => session.draw(), 1000)` (telnet `startTelnetServer()` in `server.ts`; xterm `registerWsConsole()` in `wsConsole.ts`). Input events call `session.draw()` immediately on `redraw`; the 1 s tick catches live data changes (clocks, spinners, roster refreshes). `TuiSession.draw()` (`session.ts`) makes that repaint flicker-free through four mechanisms:
...
**Why `timeoutResponse`, not `commandsDroppedTX`** (`RESEARCH.md §0`, load-bearing): `commandsDroppedTX` does **not** track RF ACK failures. When a listening node stops acknowledging, the driver retries and marks it Dead (the `D` gate handles that) while the drop counter stays 0 — it was near-silent for `NoAck`, controller-cannot-send, and Get-timeout against zwave-js@15.25.3. It ticks only on a NOK transmit report and can even false-positive on a premature-response abort that actually succeeded (on zwave-js 15.27.1, only one triggered by the node's S0 nonce report or S2 SOS nonce report — RESEARCH §2.2). So it is silent for the failure it names and noisy otherwise. `timeoutResponse` is the honest, node-stays-Alive signal: the node MAC-ACKed a reply-expecting command (a Get, a supervised Set, or a secure send's nonce Get), so the RF link up to the ACK works, but the expected report never arrived — a return-path / responsiveness problem. The raw `commandsDroppedTX` / `commandsDroppedRX` counters are still displayed on the Detail TRAFFIC row as honest context, but never folded into the score.
...
Two guards worth noting: it clamps `timeoutResponse` to `commandsTX` before dividing (a defensive cap the *lane* math in §4.4 doesn't apply, though `linstep`'s own `[0,1]` clamp makes the outcome equivalent), and it returns `null` — not 0 — when nothing has been sent, so the UI can distinguish "clean" from "no data." A documented caveat: `timeoutResponse` accrues only for reply-expecting traffic (a Get, a supervised Set, or a secure send's nonce Get) while `commandsTX` counts *all* successful sends, so for a node whose traffic is mostly unsupervised Sets that need no nonce the rate is a conservative *under*-estimate of the true response-failure rate — an honest floor, never an over-statement.
...
Note the non-standard boundary: the `D` band is unusually wide (55–69) and `F` begins at 54, not 59 — an intentional choice so that a node has to be genuinely broken (roughly, more than one lane failing) before it reads `F`.

> **`rating` removed in v0.35.** `HealthResult` used to also carry
> `rating: clamp(Math.round(score / 10), 0, 10)`. It was computed in five
> places and read in none — and being a total function of `score`, it could
> never disagree with the grade until someone edited one of them. The score
> and the grade are now the only published health scalars.
...
The store is created by `createEvidenceStore(opts)` (a closure-based factory, not a class) and wired into the poller by the constructor in `zwave/zwaveData.ts`:
...
interface EvidenceSample {
  ...
  dS2Resync:     number | null;    // S2 SPAN resyncs (event-accumulated, v0.26); null when the S2 log lane is not listening (§6.11)
...
- **`dTimeout` is the primary reliability signal, not `dDropTx`.** Per `RESEARCH.md §0`, `commandsDroppedTX` does **not** count RF ACK failures — an RF failure marks the node **DEAD** while that counter stays 0. The real RF-failure signals are (a) `dFlaps` (Alive↔Dead status transitions) and (b) `dTimeout` (a reply-expecting command — a Get, a supervised Set, or a secure send's nonce Get — whose reply never arrived, with the node staying Alive). The TUI's reliability metric is therefore `timeoutResponse/commandsTX` (shown as **TMO**), and `dDropTx` is captured only for completeness.
...
`fresh` answers "did a real stats event arrive since the previous sample?" It is computed by the caller in `isFreshSample(prev, stats)` (`zwaveData.ts`) and requires **both** conjuncts:
...
For a **(re)subscribe** replay the counter conjunct is defence in depth. Since v0.26 a replay with no counter movement carries the previous arrival stamp, and a first delivery stamps nothing (§2.5), so `seenAdvanced` alone keeps it out; the counter conjunct would still stop it if that stamping rule regressed. Treating a replay as an observation is the pseudo-replication leak that would drive downstream MAD to 0 and manufacture false confidence. Inside the store, `fresh` gates every re-sampled EMA aggregate — the coarse fold only counts `rssi`/`rtt`/`rate` into a bucket `if (s.fresh && …)`, and `NodeCoverage.freshSamples` counts only genuine observations.

Since v0.64.6 `seenAdvanced` reads an arrival stamp that moves only on the three hearing counters (§2.5), so an interval whose only movement was a response timeout or a dropped send is not fresh: it no longer counts as an RTT/RSSI observation or toward an after-window's liveness floor (`freshN`, §9). The counter conjunct is load-bearing for one case: an interval whose only movement was a dropped RX advances the stamp, but the sample signature does not carry that counter, so that interval is not fresh either.
...
`recordRouteFailure(nodeId, between, at)` latches a `routeFailedBetween` event the moment it appears — this datum is transient (overwritten on the next successful transmission), so it must be captured event-driven, never by polling. Each node keeps a small ring capped at `ROUTE_FAIL_RING = 20`; `between` is the `[last-functional, first-non-functional]` node-id pair. **Read path (v0.35):** `zwaveData.routeFailures(nodeId)` → `DataProvider.routeFailures` → TOPOLOGY's Route-failures panel (§3.7), and (v0.68.0) `routeFailureTally` in `haStates.ts` → `sensor.zwave_tui_route_failures`, the seven-day count ranked by link (§12.11). Between v0.13 and v0.35 this ring was written and never read — the add-on knew precisely which hop each transmission had died on and told the operator only that a node was unhappy.

**Rate run (v0.64.6).** `foldRateRun(prev, sample)` folds every recorded sample into a per-node `RateRun { routeKey, sawHundred, belowTx, lastBelowTxAt, rateKbps }`, read by rate-fallback (§7) through `rateRun(nodeId)`. It exists because the ~40-minute fine window cannot hold a sweep-only node's previous transmission, and its rules follow where zwave-js sets the rate (Node.ts `updateRouteStatistics`: `protocolDataRate: txReport.routeSpeed`, called only under Driver.ts `if (hasTXReport(result))`): only an acknowledged transmission is a reading; a 100k reading proves its route and clears the count; a below-100k reading counts once, unless it lands < 30 s (`RATE_FALLBACK_MIN_GAP_MS`) after the previous counted one; a reading on a different route starts a new run that remembers nothing of the old route (a 100k reading proves the new route at once, a below-100k one leaves it without capability); a tick with no route visibility or no reading leaves the run as it was. The run is saved with the snapshot (`rateRuns`, optional) and restored on `load()` whatever the fine tier's fate — it exists for evidence older than the ring; a file written without it re-folds from the restored ring. `evictNode` and `reset()` drop it.
...
interface NodeCoverage {
  ...
  probesAsked: number; probesAnswered: number;                 // liveness sweep (v0.37; sweep lane only since v0.40.2)
  probesSelfProven: number;                                    // persisted since v0.37; one arm of the four-way ProbeClass judgment
  probesEchoOnly: number; probesAttribUnknown: number;         // the other three arms, persisted since v0.49.0
  probesUnheard: number;
  probesReadAsked: number; probesReadAnswered: number;         // the sweep probes sent as a routed read (v0.71.0), a subset of the two above
  probeReroutes: number;                                       // sweep reads that failed over from a stored route (v0.71.0)
  laneEpoch?: number | null; creditEpoch?: number | null;      // store-level disclosure stamps (v0.63.1, v0.64.4)
}
```

`registerNode(nodeId)` (called for **every** roster node each tick, even before its first stats event) is idempotent and seeds `firstSeenAt`. Store-level `recordingSince()` records when collection first began. `samples`/`freshSamples` are cumulative and *not* ring-bounded. **Read path (v0.35):** `zwaveData.evidenceCoverage(nodeId)` joins this record with live subscription state (`statusFeedLive` / `statsFeedLive`, from the `statusSubbed` / `statsSubbedNodes` sets) and surfaces it in NODE DETAIL's EVIDENCE section (§3.7). Subscription state is part of coverage by DESIGN §3.1: "no evidence" from a node whose feeds are DOWN is a monitoring hole, not node silence, and the two must not render identically. `evictNode(nodeId)` wipes **all** state for a node that left the network — after `replace_failed_node`, a reused node-id must start from a clean slate, or inherited history would merge two physical devices and pre-satisfy the ghost detector's coverage precondition.

### 5.8 homeId binding + reset-through-to-disk

The persisted envelope carries the controller `homeId`. `bindHomeId(id)` is called on the first poll that reveals the live home id (in `refresh()`, `zwaveData.ts`). On a mismatch with the loaded/live id it calls `reset()`:
...
```ts
const grace = bootGraceMs > 0 && uptimeMs() < bootGraceMs && !clockCarried;   // DEFAULT_BOOT_GRACE_MS = 180_000
```

`uptimeMs()` is host uptime (`os.uptime()·1000`). `clockCarried` (v0.40) is `ageMs > uptimeMs() + 60_000` — an age only a clock that kept running while the host was down can show — and lifts grace for such a clock (§7.1.7). On a no-RTC host, the clock restores *behind* the last flush's `savedAt` after a power blip, so `ageMs = now() - savedAt < 0` is the **normal** post-blip state — it means the clock is bogus *now*, not that the file is bad. Grace is evaluated first, so:
...
Under grace or when the fine ring is too old, the store **still loads the coarse tier, coverage metadata, controller ring, controller noise-floor coarse tier (`controllerCoarse`), route-failure rings, and (v0.64.6) the per-node rate runs (`rateRuns`)** — dropping *only* the recency-dependent fine ring. Coarse pruning is even skipped under grace (`if (!grace && t0 < cutoff) continue`) because the clock can't be trusted to judge "old." The fine tier is loaded only when `!grace && !fineTooOld`. Rate runs are re-folded from the fine ring inside that fine-only block; the persisted `rateRuns` are applied afterwards and override the re-fold, so a file written without `rateRuns` (before v0.64.6) rebuilds its runs only when the fine ring itself loads, and under grace or a stale ring starts with none. The controller ring restore was added by the review after it was found to be write-only and silently dropped on every restart. Each loaded field is validated (finite `t`, integer node id > 0, `status` clamped to `0..4` else `NodeStatus.Unknown`), the fine ring is re-bounded to `maxSamples`, and a top-level `try/catch` starts fresh on any parse failure.
...
| `EVIDENCE_SAMPLE_MS` → `cadenceMs` | `routePollMs` (`ROUTE_POLL_INTERVAL_MS`, from `route_poll_interval`, default 10 s) | sample cadence; **drives `maxWindowMs = cadenceMs·3`** |
...
| `maxSamples` | `DEFAULT_MAX_SAMPLES = 240`, floored at `ceil(MAX_DETECTOR_WINDOW_MS / cadenceMs) + 1` (`MAX_DETECTOR_WINDOW_MS` = 30 min, v0.63.0) | fine-ring cap per node; the floor keeps the ring spanning the longest detector lookback at a fast cadence and applies to the default only — an explicit value is honoured exactly |
...
The producer is `zwaveData.sampleEvidence()` (`zwaveData.ts`), fired on `evidenceSampleTimer` every `evidenceSampleMs`:
...
2. For each roster node: `registerNode(nodeId, now)` (coverage), then look up cached `statsByNode`. **A node with no cached stats is skipped** — fabricating zero counters would poison the delta guards; its flap/route events stay accumulated and drain into its first real sample. **A node whose latest statistics event is younger than `STATS_SETTLE_MS` (300 ms) is sampled on the next tick instead, at most one tick in a row (v0.64.6):** zwave-js throttles statistics to one event per 250 ms, emitting the first change at once, and it increments `commandsTX` before it rewrites the route statistics the transmission produced — so a sample between the two events would read the previous frame's rate as this transmission's reading. Counter deltas carry into the deferred sample.
3. Compute `fresh = isFreshSample(prev, stats)`; update the freshness signature; drain and delete `flapAccum` / `routeChangeAccum` / `s2Accum`. Flaps are also copied into `flapsThisTick` for the §9 confound guard.
4. Attach `s2Resyncs` (the drained `s2Accum` count, stored as `dS2Resync`; `null`, never 0, when the driver-WS S2 log lane is not listening — §6.11), then the driver-WS telemetry (`lastSeen`, `isListening`, `isFrequentListening`) or `null`.
...
7. `runEngine(now)` — the just-recorded samples are read straight back out via `forNode` / `coarseForNode` / `controllerSamples` / `coverage` / `rateRun` / `recordingSince` by `detectSymptoms(...)`, then folded into baselines (quarantining symptomatic nodes so the baseline never chases the pathology). The route-failure ring (`routeFailures`) is not a detector input: its readers are TOPOLOGY's Route-failures panel (§5.7, §3.7) and, since v0.68.0, `sensor.zwave_tui_route_failures` (§12.11).

On shutdown (`stop()` in `zwaveData.ts`) the store is flushed once more. The reader side — `baselines.ts`, `symptoms.ts`, `interference.ts`, `outcomes.ts` — never mutates the store; it only queries the accessor methods, which return the live in-memory arrays (`forNode` returns `fine.get(nodeId) ?? []`).
...
- **`set_api_schema`, `start_listening` and the log-stream pair only.** No `ping`, no health check, no route command — nothing that transmits RF. `start_listening` returns the full state dump and then streams passive statistics events; `set_api_schema` merely pins the wire format; `start_listening_logs` / `stop_listening_logs` (v0.26, §6.11) toggle this client's own log subscription server-side. All mesh-mutating actions stay on the *authenticated* HA WS through the existing `ActionRunner` chokepoint (DESIGN §2). The driver WS is never used for anything that changes state.
...
### 6.7 What it feeds: the five callbacks

`start_listening` returns a `result` frame carrying the full `state` dump (parsed by `onStateDump`), after which `event` frames stream in (parsed by `onEvent`). Both funnel into five typed callbacks — the fifth, `onS2Resync`, is fed by the driver log stream of §6.11. The controller RF-off reading (v0.65.0), matched on that same log stream, is not a callback but a pull accessor, `controllerRfOffSince()`, read by auto-ping (§11.12). The callbacks:
...
interface DriverWsCallbacks {
  ...
  onS2Resync?: (nodeId: number) => void;                                 // v0.26: an S2 SPAN-resync log event attributed to a node (§6.11)
}
```
...
not fatality**: the S2 lane goes quiet, `dS2Resync` is recorded as `null`
(unknown, never 0) for every sample, the `s2-desync` detector never fires, and
the controller RF-off reading (v0.65.0) reads `null` too: a dark lane delivers
neither `Turning RF off` nor its `Turning RF on`, and a reading already set is
dropped rather than held, so the Controller RF off gate (§11.12) never holds
auto-ping off through the nightly NVM backup. Every other signal is unaffected.
...
`dS2Resync`, folded into the coarse tier as `s2` (a `null` adds nothing). Both
are optional on disk: from a pre-v0.26 snapshot the fine `dS2Resync` revives as
`null` (unknown) and the coarse `s2` as `0`, so the schema version does not
change and an upgrade loses no history.
...
Note the counting series measures `timeoutResponse`, not `commandsDroppedTX`. Per RESEARCH §0, `commandsDroppedTX` does **not** count RF ACK failures (those mark a node Dead while the counter stays 0); the real return-path signal is `timeoutResponse` — a reply-expecting command (a Get, a supervised Set, or a secure send's nonce Get) whose reply never came while the node stays Alive. The TUI's reliability metric is `timeoutResponse/commandsTX`.
...
4. **Forced resets.** `resetNode(id)` drops one node; `reset()` drops everything and rewrites disk immediately (controller home-id change). Until v0.35 `resetNode` had **no caller** — the documented triggers above were aspirational. It is now wired to exactly one: a **successful `removeFailed`**, via the `onNodeRemoved` hook on the ActionRunner (§11.2), which calls `zwaveData.forgetNodeBaselines(nodeId)`. The reasoning is narrow and checkable — once the node has left the mesh, a later re-include on the same node id is *different hardware*, and comparing it against the departed device's normals is how the engine manufactures symptoms out of a swap. A **failed** removal fires nothing: the node and its history are both still there, and discarding a live node's learned baselines is the larger harm.
...
> **Boot-grace trusts only a clock that PROVABLY CARRIED through the outage (v0.40).** The guard's premise — "no battery RTC, the clock may be hours stale pre-NTP" — is falsified on this hardware: through the 59-minute 08-25 power cut the Pi 5's RTC held time to within 0.2 s, and the uptime-only guard discarded the fine evidence ring and sparkline history ~20 s AFTER NTP had confirmed the clock. The proof of carrying, in both `historyStore` and `evidenceStore`, is an age reading **strictly greater than this boot's uptime plus a minute of slack** — something only a clock that kept running while the host was down can show. The first draft trusted any at-or-past clock, and the pre-release review caught why that fails open: an RTC-less host restores its boot clock from a shutdown-time file, so its ages read ≈ uptime (never negative), and `historyStore`'s bare `number[]` rings carry no per-point timestamps — a wrongly-admitted outage-old sparkline could never self-heal after the NTP step and would render as the live trend for up to its ring lifetime. Under the carried proof both RTC-less signatures (near-epoch and file-restored) start fresh exactly as before v0.40, an RTC host with a sub-minute outage falls back to the old conservative behavior, and the residual is an unclean cut landing more than one dirty-save cadence after the last flush — bounded by the save cadence, not by the outage. Baselines were never dropped by grace and still are not.
...
(resyncs) and `route-churn` (re-routes) carry it. `rtt-degraded` deliberately does not: RTT is a
level, not an event count, and a conjunct would stop a node that reports less often than every
5 minutes from ever maturing it; its staleness bound is `latestFresh`'s own 10-minute window. A storm that ends
...
#### 7.2.3 The 15 `SymptomKind`s

The `SymptomKind` union declares **15** kinds, and as of v0.38 **all fifteen have live detector bodies** in `detectSymptoms`. `quiet-node` was the last reserved name — declared since the DESIGN table and unemitted for eleven minor versions, the final instance of the declared-but-unreachable class this cycle cleared:
...
| 4 | `quiet-node` | node | warn | measured | yes (v0.38) |
...
**`node-down` — the ordinary outage (crit, measured, v0.37).**
`b = node.status === Dead`, dwell-gated like every other kind. `dead-flap` needs three transitions and so fires only on an oscillating node; a node that dies and stays dead produces one. The driver sets Dead only on a failed transmission, so this is a measured verdict, not an inference from silence. It does **not** call `markDegrading` — a dead node contributes no RF readings, and one dead device must not read as environmental degradation — and it opens no M5 episode (§9.7a).

**`quiet-node` — silence on a mains node the driver has not marked Dead (warn, measured, v0.38).**

```ts
quietMs = max(QUIET_MS (6 h), sweepMs * QUIET_SWEEPS (3))   // sweepMs = the configured liveness-sweep cadence (v0.63.0); undefined, so the 6 h floor, when the sweep is off
b = node.isListening === true && status !== Dead && lastSeen != null && now - lastSeen >= quietMs
```

With the sweep running (auto-ping on, §11.12), it asks every listening node once per cadence and an answered probe refreshes `lastSeen`, so silence this long means the probes are not landing, while the driver, which marks Dead only on a transmission it attempted, has not marked the node Dead. With auto-ping or write actions off, or `auto_ping_stale_min: 0`, no sweep runs and the 6 h floor is measured against the node's own traffic alone. Battery/FLiRS nodes are excluded (silent between wakeups by design), a Dead node belongs to `node-down`, and a node with no `lastSeen` at all fails closed. No `markDegrading`.

**`return-path-degraded` — relative timeout breach (watch/warn, measured).**
`windowTimeoutRate` computes Σ`dTimeout`/Σ`dTx` over the window, returning `null` when traffic is below `MIN_WINDOW_TX (20)`. Such a window is **unmeasured, not clear**: this detector and `chronic-return-path` skip it, and `engineStatus()` counts each node in that state as `timeoutWindowBlind`, published as `detectors_unmeasured` beside `detectors_ready` (v0.67.0; `weak-signal` corroborates on a failed delivery instead, v0.69.0). On a measured window the detector compares against the node's own graduated `timeoutNormal`:

```ts
relBreach = (norm.ready ? rateAnomalous(w.rate, norm.rate, TIMEOUT_RATE_MULT) : false)
          && hadRecent(samples, now, DWELL_MS, (s) => s.dTimeout ?? 0)   // recency conjunct (§7.2.2)
```
...
Fires only on a `!node.isLongRange` node whose rate run (`evidenceStore.rateRun`, §5.7) is on the node's **current** `routeKey`, has seen that route carry an acknowledged transmission at ≥ 100 kbps, and has counted at least `RATE_FALLBACK_MIN_TX` (2) separate below-100k readings since that route's newest 100k reading (v0.64.6):
...
This is DESIGN §3.3's "persistently below 100k", with the single-exchange retry it names as a confound rejected. zwave-js rewrites a node's cached route rate only when a transmit report arrives, and `record()` copies that value onto every sample, so a sample is a *reading* only on a tick where an acknowledged transmission landed (`fresh && dTx > 0`); a send cut short because the node's S0 nonce report or S2 SOS nonce report arrived before the ACK reports NoAck yet still rewrites the rate without moving `commandsTX` (RESEARCH §2.2), so it is not a reading either; a plain NoAck, or any other premature response, never reaches the route statistics. A below-100k reading less than `RATE_FALLBACK_MIN_GAP_MS` (30 s) after the previous *counted* below-100k reading is the same exchange and is not counted. The gap is measured from that counted reading, not from the reading just before, so below-100k readings at 0 s, 20 s and 40 s count twice, and a reading exactly 30 s after the counted one counts. The newest reading decides: one 100k reading clears the run, anchor included, on that tick. The card reports the run's newest reading, not the newest sample's rate. The run is saved with the evidence. A node whose latest statistics event is under `STATS_SETTLE_MS` (300 ms) old — inside zwave-js's statistics throttle window, where a sample could read the previous frame's rate — is sampled one tick later instead (§5.13), but at most one tick in a row: a node that talks continuously is sampled on the next tick even if it is still inside the window, the one case in which a sample can still read the previous frame's rate. Through v0.64.5 the detector scanned ~30 min of samples (`WINDOW_MS * 3`) for any sub-100k value — the store's copies of ONE retried frame matured the symptom by themselves and held it ~30 min after the link was back at 100k; every onset on the reference mesh was one liveness-sweep ping going out at 40 kbit/s with 3 routing attempts. A route change starts over with no remembered capability, where the old scan remembered it for 30 minutes. The trade: a sustained fallback on a node that only transmits for the two-hourly sweep surfaces at its second slow transmission plus the dwell (up to ~2 h), and then stays until the link recovers, where the scan cleared it after ~25 minutes and never raised it again.
...
```ts
routed        = latestFresh(routeKey, lookback) !== 'direct'   // null (no lwr) is unknown, not direct (v0.54.0)
margin        = rssi - floor
...
windowS2(samples, 30 min) >= S2_ABS (12)
  AND hadRecent(samples, now, S2_RECENT_MS (5 min = DWELL_MS), s => s.dS2Resync)
```
...
| `EDGE_MIN_MEMBERS` | 2 | Degrading dependents a shared repeater needs before an edge-cluster forms |
...
| `QUIET_MS` | 6 h | Floor on the silence after which a mains node is `quiet-node` (effective threshold = max(`QUIET_MS`, sweep cadence × `QUIET_SWEEPS`)) |
| `QUIET_SWEEPS` | 3 | Multiple of the configured liveness-sweep cadence that sets the quiet-node silence above the `QUIET_MS` floor (v0.63.0) |
| `RATE_FALLBACK_MIN_TX` | 2 | Separate below-100k readings on the node's current route, counted since that route's newest 100k reading, before rate-fallback arms (v0.64.6) |
| `ROUTE_CHURN_WINDOW` | 4 | LWR route changes within `WINDOW_MS` (with one inside the dwell) → route-churn |
...
| `S2_RECENT_MS` | 5 min (= `DWELL_MS`) | Recency conjunct for `s2-desync` |
...
| `dead-flap` | "Reachability runbook — a rebuild cannot repair an unreachable node" | `ping` "Ping the node (reachability request)" — source / **safe**, gated w/ probes | `action: null` "Power-cycle the device, then exclude/re-include if it persists" — lore / physical | — |
...
| `s2-desync` | "Secure link is losing nonce sync — an RF problem, not a security one" | `action: null` "Improve the RF path (distance, obstruction, interference)" — source / physical (Long-Range: "Close the distance or clear the obstruction (Long-Range is a direct link)" — source / physical) | `action: null` "Check whether the device is rebooting" — lore / physical | `reInterview` "Re-interview — NOT a fix for nonce desync" — source / **caution**, **blocked** `RF-link symptom — will not repair it` |
...
2. **`{probes: true}` is used for the probes that actually put frames on the air** — `ping` on `dead-flap`, `node-down`, `quiet-node` and `edge-cluster`. For those, an absent snapshot yields `'node not in roster — probe withheld'` and a battery/FLiRS node yields `'battery/FLiRS — probe skipped'`.
...
**Severity sort.** Symptoms are sorted by `bySeverity`: plan-owners first — a subsumed symptom renders without a plan, so it never displaces the event that owns its remedy (v0.24.0) — then worst-first by `SEV_RANK = { crit: 0, warn: 1, watch: 2 }`, with newest-breaching (`b.sinceMs - a.sinceMs`) as the tiebreak. This guarantees "a low-severity watch can never bury a critical off the bottom." The right-hand status and a `summaryLine` (`N critical · N warning · N watch  —  the engine only recommends; you run the actions`) head the body.
...
- **The top candidate (`i === 0`) carries a wrapped rationale line**, with a trailing "…" signalling more detail exists — so a screenful of symptoms stays scannable. A rationale line naming an irreversible side effect or a known failure mode (`CAVEAT`: deletes, discards, priority route, half-interviewed, comes back, re-churn) renders at any rank, and is the line the top candidate shows when it has one (v0.45.0).
- The M5 efficacy note is rendered on **every candidate with an action** — blocked or not (v0.35; before that, runnable only). Below readiness it is not silent (v0.44.0): with a measured control arm it renders grey `≈ still learning this action (n≈… of 4) — Y% self-heal`, or `≈ never tried here — this kind self-heals Y%` for an arm never run, and nothing only when no self-heal rate is measured either. Once `e.ready` it picks its **voice** from whether the candidate is blocked:

  | | runnable | blocked |
  |---|---|---|
  | beat the control arm | green `✓ helped X% (n=…) vs Y% self-heal` | yellow `⚠ ledger measured X% here (n=…) … — the block above still applies` |
  | ready, not distinguishable | grey `≈ n=…: not distinguishable from self-healing` | grey `≈ n=…: measured — not distinguishable from self-healing` |
  | still learning | grey `≈ still learning this action (n≈… of 4) — Y% self-heal`, with a `⚠ … made it worse` clause when attempts so far regressed; grey `≈ never tried here — this kind self-heals Y%` at n = 0; *(nothing)* with no measured control rate | same as runnable |

  The blocked voice deliberately does NOT characterize the block. `blocked` is
  one string carrying three different kinds of gate — a planner-authored
  advisory ("physical-link symptom"), the write-actions master gate, and a
  hard safety gate (battery/FLiRS probe skip) — and an earlier draft that said
  "the block above is lore" was true only of the first kind: it told the
  operator a SAFETY gate was unfounded folklore contradicted by measurement
  (v0.35 review). The note states the measurement and that the block still
  applies, which is true for all three, and endorses nothing.
...
**Cluster members (v0.45.0).** The edge-cluster detector populates `Symptom.members`; before v0.45.0 no screen read it, and the ids survived only inside an evidence string that `truncate` cuts mid-list — so `#4, #17, #23` rendered as `#4, #17, #2`, naming a node that is not degraded. REMEDY now renders a `downstream` row of atomic id tokens, and `symptoms.ts` emits a count alone in the evidence. A half-shed id is a different device.

**The Log carries a symptom's whole life (v0.45.0).** `diffSymptomLog` (exported from `zwaveData.ts`, pure, because the tick that owned the loop is unreachable from tests) diffs the live symptom set against what the log has already said and returns three transitions: **onset** (severity crit→error, warn→warn, watch→info), **escalation** (upward only), and **clearance** (always `info`, never the symptom's old severity — a red line saying a problem ENDED reads as a fault). A de-escalation updates the stored severity silently: "it got less bad" is not news, and logging it would double the volume on a flapping node. A mesh-identity change clears the map without emitting clearances — the old network's symptoms did not end, they stopped being ours to know about.

**Five distinct empty states (v0.46.0).** When `symptoms.length === 0`, the screen never renders a generic "all clear." It reads `data.engineStatus()` and distinguishes: **engine disabled** (`● Engine disabled` — no baselines store configured, nothing is being diagnosed), **no nodes yet** (`◷ No nodes yet` — the engine is running against an empty roster, the state in the first seconds after a restart), **still learning** (`◷ Learning — graduated for the 12:00–16:00 band:` followed by per-series counts), **zero coverage** (`◷ Learning — no detector has a yardstick yet` — deliberately silent about symptoms, because with nothing able to fire "no symptoms" would describe the instrument), **partial coverage** (`◑ No symptoms — partial detector coverage`, naming the blind detectors and the node count each cannot judge), and **genuinely healthy** (`✓ All clear — N nodes learned, no symptoms detected`). Rendering these identically would let "learning" masquerade as "healthy," so they are kept separate.

**An unseen route is not a moved route (v0.47.0).** `baselines.observe()` compared route keys with a bare `!==` over a nullable value, and the driver reports statistics with no `lwr` at all (`routeKeyOfLwr` returns null for those). A node routed via `r4` whose next sample omitted the route saw `null !== 'r4'` and had **all six rssi and rtt bands wiped**, obs and days rings included — then wiped again on reappearance. Two full resets for zero re-routing. This is the same hazard `isRouteChange()` was written and tested to prevent for the route-churn detector (*"a route we cannot see has not moved; we have merely stopped watching it"*), applied there and not to the far more destructive consumer here. Measured attribution: the wipe explains 13 of 13 blocked nodes in a busy band and 13 of 21 in a quiet one, the remainder being genuine fresh-sample starvation. A wipe victim is blocked in EVERY band at once (a shared per-node day-clock); a starved node has 6-10 distinct days and fails only where traffic is thin.

**The coarse tier reaches a screen (v0.49.0).** 14 of the 18 persisted `CoarseBucket` fields had zero consumers. DETAIL renders the RF ENVELOPE (worst…best dBm, mean, worst negotiated rate) rather than a mean that hides an hour-long collapse; the `invalidW` share when any window's counter arithmetic was void; and the coarse RTT trend, whose RSSI twin has rendered since v0.41. INTERFERENCE carries each 30-minute bucket's noise PEAK alongside its mean — a five-minute burst diluted across 25 quiet minutes drew a flat line — falling back to that bucket's mean, never 0 dBm, where no max was recorded.

**The sweep's judgment is four-way (v0.49.0).** `ProbeClass` is `self-proven | echo-only | attribution-unknown | unheard`. Only the first was ever persisted; the rest were computed, described in a log line and discarded every tick, so "never speaks except to answer us" and "genuinely silent" — opposite readings of the same answered/asked ratio — were indistinguishable. One value now derives both the counter and the log string, so they cannot disagree, and the four partition the asked total exactly. A pre-v0.49.0 evidence file loads the new counters at zero rather than being rejected.

**The diurnal denominator (v0.49.0).** At `MIN_HOUR_TX` = 20 a single timeout is a 5% rate — exactly the screen's `HEAT_MAX` — so one dropped frame in a dead hour won "worst hour" over a genuinely bad busy one. The winner is chosen from hours with at least median traffic, prints its `tx`, and is flagged when thin. `MIN_HOUR_TX` is exported so the screen can see the floor it renders against.

**All three yardsticks reach the dossier (v0.48.0).** `rssiNormal` was bridged and `rttNormal`/`timeoutNormal` were not — backwards, since those two are the only baselines any detector reads. DETAIL renders all three with the band qualifier the store's semantics require. A node whose learning is PAUSED (a symptom live or arming quarantines the baseline so it cannot chase the pathology) says so, naming which hold applies, answered as of the last engine tick. A routed node's RSSI normal is labelled `last-hop, not the device` and loses its health colouring — `stats.rssi` is the signal from the relaying repeater, a link the device is not on either end of.

**The EVIDENCE verdict is pinned (v0.48.0).** That block is last in a dossier that does not fit 80x24 for any device shape, so its verdict was unreachable without scrolling. It renders in `frame()`'s telemetry slot, outside the scroll window. `frame()` spends an extra row for that slot while `detail.ts` sizes its own body, so the body budget subtracts one to match. A subscribed-but-silent statistics feed is reported there too: the feed badges prove a subscription, and `lastStatsUpdated` is the only number that proves data is arriving.

**Only two series arm a detector (v0.47.0).** `grep -n 'baselines\.' src/zwave/symptoms.ts` returns exactly `timeoutNormal` and `rttNormal`. `rssiNormal` has **zero** detector consumers — `weak-signal` compares against an absolute `WEAK_MARGIN_DB` over the measured noise floor, never the learned baseline; its only readers are the DETAIL dossier row and the readiness counter. So the green all-clear is gated on timeout and rtt, and an ungraduated rssi baseline is reported (in both the partial and all-clear states) as the dossier yardstick it is, never counted as a detector blind spot.

**Why partial coverage needs its own state (v0.46.0).** The green all-clear requires all three series graduated fleet-wide in the current band, and on a mesh with routed nodes that is unreachable: `baselines.observe()` resets rssi/rtt across all six bands on any route change, and both fold only on FRESH samples — measured here at ~410-922 of 363,586 lifetime (~1.5-3.3 per band per day) against `MIN_OBS` 20, so a repeater-routed node must hold one route ~2 weeks to graduate a single band. Measured 2026-08-31 the ceiling is 23 of 38 — the direct-routed subset. Rendering that as "Learning" forever implied a convergence that never arrives. The open-episode disclosure (v0.43.1) sat only in the green branch and was therefore structurally unreachable in production; it now belongs to every no-symptom state, which is the actual unblocking — without it the fourth state would merely relocate the trap.

**What "learned" is scoped to (v0.43.1).** `engineStatus()` returns `{enabled, ready, total, timeoutReady, timeoutWindowBlind, rttReady, rssiReady, band, bands}`. `timeoutWindowBlind` (v0.67.0) is not band-scoped: it counts nodes whose 10-minute window holds fewer than `MIN_WINDOW_TX` sends — unmeasured, not clear — and is published as `detectors_unmeasured`. The counts are **per series** and **per time-of-day band**: `bandOf()` divides the day into `N_BANDS = 6` four-hour bands, and a graduation count is only true for the band containing now. Before v0.43.1 the whole predicate was one line — nodes whose *timeout* baseline had graduated in the current band — and REMEDY rendered it as "Every node has a graduated baseline." A fleet with a full timeout baseline and an empty RSSI baseline read as fully learned, and the same fleet could read learned at 03:00 and unlearned at 15:00 with nothing having changed. All three series must be graduated in the named band before the green all-clear renders, `total === 0` gets its own state rather than passing every `0 < 0` comparison, and both the learning and all-clear branches name the band. On a narrow terminal the band is kept in a compact form (`12–16h`) rather than truncated away, and the counts go through `fieldStrip` so one is **dropped whole** rather than clipped to a plausible wrong number (`rssi 1` where the truth was `12/39`).

**Harm is as hard to claim as help (v0.44.0).** `scoreRecovery` distinguishes `worse` from `no-change` at eight sites — a rising timeout rate, new flaps, fresh S2 resyncs, more route changes, lost margin, slower RTT, a dropped negotiated rate — and until v0.44.0 the tally discarded that distinction, so an action that harmed and one that did nothing rendered identically. Both arms now carry a decayed regression count. The warning is gated the same way the benefit claim is: the Wilson lower bound on the action's regression rate must clear the **control arm's own** regression rate by the effect size, across at least two distinct nodes. Comparing against the control's *improvement* rate would compare two different quantities; requiring only a point ratio let one regression on one node accuse a remedy, with the outcome depending on where in the sequence that regression fell. A harm finding is **appended** to a granted efficacy claim, never substituted for it — an action can genuinely help most of the time and hurt some of the time, and returning early on harm hid a claim ENGINE was rendering in green from the same ledger row.

**What a confounded episode counts toward: nothing (v0.44.0).** A node that died or was remediated mid-episode is not a clean observation of *either* arm. The rule existed from v0.40 but was applied only to the control arm while the action arm kept being fed — which matters more now regressions are tracked, because a death generates re-routes and S2 resyncs by construction and `worse` for those metrics is literally `after > before`.

**The two numbers beside every learned rate.** `n` is a decayed weight, not a tally: `bump()` is `n = n·0.97 + 1`, so it is an exponentially-weighted effective sample size saturating near 33. The node count beside it *is* a cumulative tally. Rounding the first produced `n=6 · 7 nodes` on an ordinary run — seven closures on seven nodes give Σ0.97ⁱ = 6.4005 — so both screens now write `n≈6.4`, and `src/telnet/ledgerText.ts` holds the shared wording. `nodes === 0` means UNKNOWN (a ledger written before provenance was tracked), and renders as `sources not recorded` on **both** screens; REMEDY used to fall silent while ENGINE disclosed it.

The whole surface reiterates the engine's contract in its own chrome: the summary line ends "the engine only recommends; you run the actions," and the key bar offers `↑↓` SYMPTOM, `A` ACTIONS, `1-9` SCREENS and `Q` BACK — no key selects or runs a plan's candidate as such. The symptom cursor picks the node the device action keys target: `A` opens the Actions Menu for it, where executable candidates run through type-CONFIRM; with write actions enabled, the `i`/`h`/`x` shortcuts arm the same type-CONFIRM box and `p` pings it immediately, the one action with no CONFIRM box.
...
interface Episode {
  ...
  nodeId: number | null;
  onsetMs: number;
  dwellStartMs?: number;        // dwell start — the first breaching reading (v0.64.6); absent = read as onsetMs − DWELL_MS
  ...
  unprobeable?: boolean;        // node could not be probed at all (sleeping/FLiRS) — unverifiable is structural (v0.38)
  transient?: boolean;          // starved before-window, after met its floor, live span < DWELL_MS + UNDERSAMPLED_AFTER_MS (v0.39; live span since v0.64.6)
  undersampled?: boolean;       // starved before-window though live long enough, or any starved quiet-node closure (v0.41.2; live span and quiet-node rule since v0.64.6)
  confounded?: boolean;         // node died, or an unattributed remediation ran, mid-episode — credited to neither arm (v0.40)
}
...
interface WindowMetrics {
  ...
  rx: number;                   // Σ dRx
  timeouts: number;             // Σ dTimeout (replies to reply-expecting commands that never came — the reliability signal)
...
interface Tally {
  ...
  bad: number;                  // decayed count that resolved `worse` (v0.44.0)
}
...
updateEpisodes(symptoms, now):
  ...
  for s in toOpen:    oc.open(s.nodeId, s.kind, now, degradedWindow(s.nodeId, s.sinceMs, now),
                              { dwellStartMs: s.sinceMs })                            // capture BEFORE
  for r in toResolve: oc.resolve(r.nodeId, r.kind, now, nodeWindow(r.nodeId, now),
                                 { unprobeable, absentSinceMs: r.absentSinceMs })      // capture AFTER
```
...
for ep in openEpisodes:
  ...
  if now - since >= confirmMs:  toResolve += { ...ep, absentSinceMs: since };  pending.delete(ep.key)
```
...
- **The confirmation window is not live time (v0.64.6).** The first-absent tick travels out in each `toResolve` entry as `absentSinceMs`, because `pending` loses it on the resolve tick, and each episode keeps the symptom's dwell start as `dwellStartMs`. The transient/undersampled split of starved `unverifiable` closures (§11.12) measures the symptom's LIVE span from `dwellStartMs` to `absentSinceMs`. Through v0.64.5 it measured open-to-resolve, which always included these ten minutes.
...
- **A driver refusal is recorded, scoped to the detectors that asked for the action (v0.43.1).** `run()` classifies each failure while the driver's error is still in hand: only a `removeFailed` whose error `isNotFailedRefusal` recognises as "the node is not failed" is `'refused'`; every other failure is `'transport'`. A refusal goes to `recordAction` with `refused = true` and the kinds `refusalScope(actionKind)` returns — the symptom kinds whose own plan offers that action (`REFUSAL_INDICTS` in planner.ts; today a refused `removeFailed` reaches `ghost-suspect` only). An empty scope records nothing. This is what makes the `refused-misdiagnosis` verdict and the `fp` counter reachable (§9.4). The exact production wording of a refusal has not been observed, so a `removeFailed` failure with a ZW0360 reason the classifier does not recognise is flagged in the log.

Inside `recordAction`, a success is attributed node-scoped, not symptom-scoped — the operator picks an action for a *node*, and any of its active symptoms could be the one it addresses. A refusal passes its scope as `onlyKinds` and reaches only those kinds:

```ts
const prefix = `${nodeId ?? 'mesh'}:`;
for (const [k, ep] of open) {
  if (!k.startsWith(prefix)) continue;                  // every open episode on this node
  if (onlyKinds && !onlyKinds.has(ep.kind)) continue;   // a refusal: only the kinds that asked for the action
  if (skip?.(k)) { ep.confounded = true; continue; }    // …except ones already recovering on their own (confounded, v0.40)
...
- **Evidence, or it's unverifiable — and each metric gates on evidence of ITS OWN signal.** This is the load-bearing subtlety: `freshN` (fresh-sample count) is **not** a valid evidence floor for rssi/rtt, because a fresh sample routinely carries a null rssi/rtt (the no-signal sentinels), so a median built from a single reading could pass a `freshN ≥ 3` gate. Each branch therefore gates on its own denominator:
    ...
    - **rate** — `rateKbpsMin` is folded from **fresh** samples only, so a fully quiet after-window (all stale carry-forwards) is `null` → `unverifiable`. A non-null value is not necessarily a *reading* in the §5.7/§7.2.4 sense, though: a fresh sample with no acknowledged transmission (`dTx` 0) still carries the driver's cached route rate, so an after-window whose fresh samples are all RX-only is scored on the last transmitted — possibly pre-fix — rate, and the rewrite from a send cut short by the node's S0 nonce report or S2 SOS nonce report (RESEARCH §2.2) can score `improved`. This metric does not apply the rate run's reading rule (`fresh && dTx > 0`).
...
- **`refused-misdiagnosis` is keyed to the symptom, and auto-detected since v0.43.1.** A driver refusal (e.g. `remove_failed_node` on a node that actually responds) refutes the *diagnosis*, so it bumps that detector's false-positive tally (`fp`) and NEVER counts as action efficacy. Until v0.43.1 it was present in the model but never detected, for two reasons that are now both handled (§9.3): the operator-action hook could not distinguish a genuine driver refusal from a transient WS/connectivity error — `run()` now classifies the failure on the driver's own error, and only `removeFailed` can be refused — and a node-scoped stamp would wrongly mark non-ghost symptoms — the refusal now reaches only the kinds `refusalScope(action)` returns.
...
interface Efficacy {
  ...
  lowerBound: number | null;       // Wilson 95% lower bound on ok/n — what must clear the bar (v0.43.1)
  bar: number | null;              // baseRate + minEffect; null until ready or with no measured control arm (v0.43.1)
  minN: number;                    // the readiness threshold, carried on the value (v0.44.0)
  harmed: number;                  // decayed count this action left measurably worse (v0.44.0)
  baseHarmed: number;              // the control arm's own worse count (v0.44.0)
  baseN: number;                   // the control arm's decayed n
  baseNodes: number;               // distinct nodes behind the control arm
}
```
...
> construction** — the producer set both from the same `beats` expression — so
> it was two fields encoding one fact, read by nothing but its own tests, and
> one refactor away from disagreeing with itself about whether an action works.
> `expectedEfficacy != null` is now the single source of truth.
...
- **A control arm above ~0.847 makes its action arm permanently uncreditable — documented, not silently absorbed (v0.40.2).** `bump` decays as `n = n·(1−decay) + 1`, so `n` has a fixed point at `1/decay` = 33.3 and is *capped*. `wilsonLower(n, n)` at that cap is 0.8967, while the gate demands `>= base + minEffect`. So for a PERFECT action arm at any n, credit is impossible once the control base exceeds 0.8967 − 0.05 ≈ **0.847**. This is the same structural argument the code already makes for `node-down` (§9.7a), generalized: a kind that mostly self-heals can never have an action credited, however well the action works. Every control closure observed to date is `improved`, so this is a live constraint, not a hypothetical. It is recorded here rather than papered over — changing the gate is a statistics decision, not a bug fix.
...
- **`expectedEfficacy` stays null until the action beats the control arm** by the minimum effect size (`wilsonLower(ok, n) >= base + 0.05` — the bound, not the point estimate `rate`), not merely until minimum-attempts. Note `minEffect` (0.05) does double duty: an *absolute per-command-rate drop* in `computeVerdict`, and a *proportion-of-episodes margin* in `efficacyFor` — different quantities, same conservative constant.

**Planner integration** (`planner.ts`, `planFor`): efficacy is attached only to *executable* candidates — `if (c.action != null) c.efficacy = ctx.efficacyFor(symptom.kind, c.action)`. Physical guidance (no action) gets nothing to score. Critically, this is **purely additive this milestone — recommendation ORDER is unchanged**; efficacy is *shown, not yet used to rank*. A route-rebuild candidate is never a runnable recommendation regardless (it cannot fix a physical link, deletes manual priority routes, and throws on Long-Range nodes), so its note (from v0.35) always renders in the blocked voice — a disagreement or a confirmation, never a green "helped".

**Remedy screen** (`telnet/screens/remedy.ts`, `efficacyNote`) renders the note on **every candidate with an action**, blocked or not (v0.35; before that, runnable only) — a blocked or anti-pattern row reports the measurement in the blocked voice (§8.8) and never carries a green "✓ helped…" that contradicts the advice. The runnable voice:

```
!e                         → (nothing)
!e.ready                   → grey  "≈ still learning this action (n≈2.0 of 4) — 40% self-heal"   // (nothing) only with no control rate (v0.44.0)
e.expectedEfficacy != null → green "✓ helped 82% (n≈6.4 · 7 nodes) vs 40% self-heal"         // yellow when blocked ("⚠ ledger measured 82% here …") or a harm finding is appended
harm gate cleared (ready)  → yellow "⚠ made it WORSE in 60% of n≈6.4 · 3 nodes — leaving it alone: 10% worse"
otherwise (ready)          → grey  "≈ n≈8.0 · 3 nodes: not distinguishable from self-healing"
```
...
2. **Verification probes** at the two moments a verdict depends on: when an episode opens (while the symptom is live, filling the degraded window) and — timed by `confirmBurstDue` — once the confirmation window has aged to `CONFIRM_MS - WINDOW_MS`, which is the moment the after-window opens. That timing is load-bearing and was got wrong first: v0.36.0 probed the instant the symptom cleared, so all three readings were ~8 minutes old when `resolve` cut a trailing 5-minute after-window and every one had aged out of the window it existed to fill. Node 55 closed `unverifiable` with four answered probes sitting just outside the frame (fixed v0.36.3). Widening the after-window instead would have been the wrong repair: the confirm dwell exists so that window settles past the recovery transition, and stretching it back would re-admit exactly the unsettled readings the dwell excludes. `VERIFY_BURST` is 5 at a **60 s** effective spacing (`VERIFY_SPACING_MS` 30 s, below the tick, so the tick is the limiter), and up to `VERIFY_MAX_PER_TICK` = 4 nodes are released per tick. Two arithmetic facts govern all three numbers, and both were missed on the first two attempts:

> **A burst must SPAN less than the window it fills.** 5 probes at the original 120 s effective spacing (70 s requested, rounding up to the next 60 s tick) span 480 s against a 300 s window, so probes 1–2 age out before 4–5 arrive and the window never holds more than two or three readings — adding probes lengthened the stream without ever filling the window. At 60 s the same burst spans 240 s and fits, with two readings spare over the floor of 3. The count was checked against `MIN_OBS` twice; the span against `WINDOW_MS` neither time.
...
- the **coarse noise floor** becomes a multi-row chart that also widens to represent more of the retained series rather than only its newest 24 buckets, keeping the fixed `−110…−80` scale it shares with the fine trend so the two stay directly comparable, and labelling only the two scale ends (an axis tick per row would imply precision the 8-level glyph does not have);
- the **diurnal profile** gains a chart above the existing heat strip on the same 24-hour axis — the strip answers *which* hour is hot, the chart answers *by how much*.

The diurnal chart is bound by the same two honesty rules as the strip beneath it, and for the same reason. It uses the **absolute** `HEAT_MAX` scale with per-hour `heatColorFor`, never normalised-to-peak: a normalised scale is baseline-relative, which is precisely what this heatmap exists to avoid, and an early version of this chart drew every hour of a uniformly *healthy* mesh as a solid warning block sitting directly above an all-clear strip. And an **unrated** hour (`rate === null`, too little traffic to rate) is passed through as `null` so it draws the no-data dot, rather than being substituted with `0` and drawn as a measured floor value one row above the dot that says otherwise.
...
evidenceStore ────┤    (memoized 10 s, zwaveData.ts)       (pure, interference.ts)         │
symptom detector ─┘                                                                          ▼
                              DataProvider.interference()  ◄── createTuiDataProvider() / buildZwaveDataSource(), dataProvider.ts
                                          │
                                          ▼
...
`timeoutResponse` (surfaced as **`reply-tmo`**) is displayed but **deliberately excluded from the band**. This is a direct consequence of the project's load-bearing counter semantics: a `timeoutResponse` is a *per-node* reply timeout — a reply-expecting command (a Get, a supervised Set, or a secure send's nonce Get) whose answer never came while the node stayed Alive — not a host↔stick serial fault. It is the TUI's genuine RF-reliability signal, so it belongs to the node, not to the serial link. (Correspondingly, `commandsDroppedTX` is not on this panel at all: it does not count RF ACK failures, so it would be noise here.)
...
### 10.6 Correlated degradation — the detector owns the ratio, and the scope line
...
**The companion count (v0.35, reworded on review).** `degradedNodes` was computed for every view since M6 and only ever reached the screen through the *inactive* narrative above — that is, it went dark at exactly the moment it mattered. During an ACTIVE event the panel now adds `meanwhile · 7 nodes symptomatic across ALL detectors — not necessarily this event`. Two labelling decisions are load-bearing. First, it is **not called the event's scope**: the count includes nodes whose faults have no relationship to the event (a dead-flap, an S2 storm), so presenting it as the event's reach over-claimed what the detector correlated — the v0.35 review caught exactly that in the first draft. Second, it is labelled **apart** from the detector's ratio, because the two count different populations: "degraded X of Y active" is a share of the nodes that were *transmitting* in the window, while this is every node carrying any symptom at all. What the row buys is the difference between "an RF event, and otherwise healthy" and "an RF event on top of three unrelated fires". Suppressed at zero; the inactive case is untouched — the narrative already states the count there.
...
────────────────────────────────────────────────────────────────────────────
ping            safe         call_service button.press { entity_id }   (one ACK-only NoOp; unanswered ⇒ the driver marks the node Dead)
refreshValues   caution      zwave_js/refresh_node_values { device_id }   (sends nothing to a Dead node)
routedRead      safe         call_service zwave_js.refresh_value { entity_id, refresh_all_values: false } (one Get, read-only; engine-only: ladder read, and the sweep/verification probe since v0.71.0)
...
- **Shortcut keys** (`p`/`i`/`h`/`x`/`R`) are only routed when `this.actions?.enabled` (`feed()` in `session.ts`). In read-only mode they fall through to normal navigation.
...
`ACTION_CATALOG` (`actionsCatalog.ts`) is the single source of truth. It lists ten descriptors (the seven maintenance verbs plus the three mesh-identity answers `identityKeep` / `identityResume` / `identityFresh`, added in v0.64.0; see the Mesh Identity chapter) **in menu order — device actions first (least→most dangerous), then system-wide.** Each `ActionDescriptor` carries a `kind`, a `label`, a `scope` (`'device' | 'system'`), an `impact`, a `desc` ("what it does"), an `impactNote` ("what to expect", shown in the confirm box), and `needsNode`.

The three **impact tiers** drive both the UI colour/badge and the confirm posture:

| Tier | Colour | Meaning | Members |
|---|---|---|---|
| `caution` | yellow | mutating but recoverable | `refreshValues`, `reInterview`, `healNode`, `identityKeep`, `identityResume`, `identityFresh`, `stopRebuild` |
...
`buildMenu(ctx: MenuContext)` produces the ordered, context-aware `MenuItem[]`. `MenuContext` is `{ scope, hasNode, identityResumable, identityPending, cursorScreen?, rebuilding }`. `scope` selects either the device menu or the mesh-wide NETWORK menu, never a mix, and the identity rows appear only while a decision is pending (RESUME only when this controller has an archive):
...
**`renderActionsMenu`** draws the full-frame menu: a header naming the current target (`· target #16 Kitchen`) with a right-aligned `READ-ONLY`/`ARMED` badge (the badge width is reserved *first* so a long target can never truncate the mode flag off a narrow terminal), then rows grouped under `MAINTENANCE` / `DEVICE CONTROLS` / `CONFIGURATION` headings in the `DEVICE ACTIONS` menu (the last two only when a node is selected), or under a single `MESH-WIDE` heading in the `NETWORK ACTIONS` menu, whose header reads `· whole mesh` in place of a target. Each row is `▶ <label,28> [BADGE,13] <what it does>` (`▶` on the cursor row only; a disabled row's reason renders as `— <reason>`). `LABEL_W = MENU_LABEL_W = 28`; the badge occupies a fixed 13-cell field (`[DESTRUCTIVE]`) so the description column aligns. A disabled row's `reason` replaces its `desc`. Dimmed styling (`c.grey`) applies when a row is `disabled` **or** the whole menu is `locked`. The footer shows `↑↓ move · ⏎ locked|select · Esc close` and, on the right, either the action count or `enable write_actions_enabled to unlock`.
...
`CONFIRM_WORD = 'CONFIRM'` (`actionsCatalog.ts`) is the exact string the operator must type. It is exported so the session, the renderer, and tests all agree on it.
...
`beginAction(kind, immediate, node?)` is the **single entry point for both the menu and the shortcut keys** (`session.ts`):
...
- `actionTargetNode()` resolves the target from **the screen's own cursor** (v0.24): the **Log** targets the *highlighted event's* node (matching what Enter would do), **Remedy** targets the *symptom under its cursor* — on that screen the cursor IS the action target — and **Overview**/**Detail** target the selected row. The aggregate screens (Topology, Heatmap, Controller, Interference) have **no node cursor** and return `undefined`, so a node action is REFUSED there with an on-screen reason instead of silently acting on whatever the invisible Overview cursor happened to hold. `p` is the one action that executes with no CONFIRM box, which is what made that fall-through a safety defect rather than a cosmetic one.
...
`handleTypeConfirmKey` (`session.ts`) is the arming box:
...
- **Telnet (`:2324`)** is always constructed with `trusted: false` (`startTelnetServer()` in `server.ts`) — it is direct LAN and never HA-authenticated — so it **always faces the login gate whenever `auth_enabled`.**
- The `/console/ws` transport computes `trusted = isTrusted(req)` per upgrade (`registerWsConsole()` in `wsConsole.ts`).
...
|---|---|---|
| `!o.enabled` check inside `run()` | `run()` in `zwaveActions.ts` | Runner executing even if called with write actions off |
| Buffer capped at `CONFIRM_WORD.length`, `===` match | `handleTypeConfirmKey()` in `session.ts` | Overtyping past `CONFIRM`; partial/"almost" arming |
| `deviceCmd`/`entryCmd`/ping throw on missing id | `deviceCmd` / `entryCmd` / `ping` in `createActionRunner()`, `zwaveActions.ts` | Sending a command with no device/entry/ping target |
...
| `rebuildAll`/`stopRebuild` mutual exclusion | `buildMenu()` in `actionsCatalog.ts` | Offering a no-op action |
| `resetActionState()` on login & re-lock | `resetLogin()` in `session.ts` | A half-armed destructive action surviving the auth boundary |
| Shared per-peer throttle, capped map | `blockedMsFor` / `registerFailure` in `loginPolicy.ts` | Reconnect resetting brute-force budget; map growth flood |
...
> **Unprobeable nodes open no episodes at all (v0.38.1).** The v0.38.0 split counter made the structural churn visible; one audit window later, a single sleeping sensor had closed 16 unverifiable episodes — every one unscoreable by construction, since no lane may wake the device to fill its windows. Episode opening is now gated on `isPingCandidate`, the same rule `node-down` already applied: the SYMPTOM still renders, only the pretence of an experiment is dropped. The structural counter and the resolve-time `unprobeable` flag remain, holding history and covering the one live edge — a node whose `isListening` flips mid-episode.

**Unscoreable-by-design is counted apart from unscoreable-for-now (v0.38).** `OutcomeStore.unverifiableUnprobeable(kind)` tracks episodes that closed `unverifiable` on a node no lane may probe, and REMEDY renders it as its own line (`○ N more on sleeping devices that cannot be probed — unscoreable by design, not a gap`). One counter conflated two different facts: thin evidence is fixable — more probes, a wider window — while a sleeping device can never be asked at all, so no effort changes the outcome. Reported together, the permanent kind silently drained the meaning from a signal built to flag the fixable one. Probeability is passed in at `resolve()` because it is a property of the device that only the data layer knows; the ledger cannot compute it.

**A closure line carries the DECIDING quantity, not only the floors (v0.40.2).** The per-window counts prove the evidence floors were met; they do not show why the verdict came out as it did — the rtt branch decides on medians, flap/s2/route on event counts, timeout on a rate. An audit put it plainly: a line printing only counts is equally consistent with `improved`, `no-change` **and** `worse`. Because a resolved episode is never persisted, that line is the only lasting record of the verdict's inputs, so it now carries the medians, event counts and timeout rate alongside them.

**Every closure shows its arithmetic (v0.38.2).** The episode-closure log line carries per-window evidence counts — `[before fresh=5 rtt=1 rssi=5 rate=y | after fresh=5 rtt=0 rssi=5 rate=y]`, `none` for an absent window — because a bare `unverifiable` names the verdict but not the failing floor. Production showed the need precisely: `rate-fallback` episodes scored six-for-six while `rtt-degraded` episodes on the same probed, answering nodes closed `unverifiable` three-for-three, and nothing in the log could say which of three candidate mechanisms starved the RTT count (the driver omitting the optional `rtt` field from statistics events, the sampling cadence collapsing distinct probes into one reading, or the before-window refinement stopping early). The counts make the next such closure its own diagnosis.

**"We could not sample it" is not "it was brief" (v0.41.2).** The transient split below (v0.39) is right in shape and was over-claiming in practice. For an ECHO-ONLY node — one whose only fresh readings are its 120-minute sweep replies — the before-side floor of `MIN_OBS` is unreachable *whatever the symptom's duration*, so `transient` was **arithmetically guaranteed rather than measured**, and the closure line asserted "degraded state ended before its evidence floor" on evidence that could not distinguish that from "we only ever got one look". A production audit caught it on the first such closure in the wild. The one signal that separates the two is DURATION: an episode whose symptom stayed LIVE past `UNDERSAMPLED_AFTER_MS` after its dwell matured (5 min — the span of a full verification burst, so every opportunity the engine can create to fill the window has passed) and *still* did not reach its floor was limited by the device's reporting rate, not by the symptom's brevity. Those close as `undersampled`, counted apart, tagged apart on the closure line, and rendered as their own REMEDY and ENGINE rows. Genuinely brief episodes keep `transient`, which now means what it says.

**The confirmation window is not live time (v0.64.6).** The v0.41.2 rule measured duration open-to-resolve, and resolution waits out the 10-minute confirmation window (§9.2), so every closure measured at least ten minutes: `transient` could never be assigned, and every brief blink closed as "this node reports too rarely to reach the floor, whatever the duration". In about 50 h of live log, 6 of 6 starved closures were tagged `undersampled`, all on listening nodes; in each, the first absence came within about two minutes of the verification burst starting — three cleared after the first answered probe, three after the second. The split now measures the symptom's live span, from its dwell start (`dwellStartMs`, the first breaching reading) to the first tick it was seen absent (`absentSinceMs`), against `DWELL_MS + UNDERSAMPLED_AFTER_MS` (10 min). Anchoring at the dwell start, not at the episode's open tick, matters for exactly the case `undersampled` exists for: one breaching reading on an echo-only node ages out of rtt-degraded's 10-minute lookback strictly past that boundary whatever the tick timing, whereas an open-tick anchor would let one late or skipped tick relabel it `transient`. A caller that passes neither timestamp keeps the old meaning, so a store-level `resolve()` without them still reads as live until it resolved. A starved `quiet-node` closure (a before-window present but below its floor, an after-window meeting its own) always closes `undersampled`, never `transient`: its breach is silence, so a transmission in its before-window would have ended it, and that window cannot reach the timeout floor at any duration — a live-span label would only record how fast the verification probe landed.

**A transient blink is not an evidence gap (v0.39).** The arithmetic paid for itself on its first unverifiable closure — `[before fresh=1 rtt=1 rssi=1 rate=y | after fresh=5 rtt=5 rssi=5 rate=y]` on a node that answered all ten probes — and settled the three-mechanism question: the driver was delivering `rtt` on every fresh sample (eight windows observed, `rtt` equal to `fresh` in each), and probes were not collapsing. The starving side was the *before*-window, and correctly so: `refineBefore` stops refining the moment the symptom clears, because folding recovered readings into the degraded window would compare the node against itself healthy. A degradation that lasts one sample therefore freezes its before-window below `MIN_OBS` forever — *structurally* unverifiable, exactly the shape the unprobeable split (above) was invented for. `resolve()` now classifies it: an `unverifiable` verdict whose before-window failed its kind's floor while the after-window met its own is counted as `unverifiableTransient`, tagged on the closure line (`(transient — degraded state ended before its evidence floor)`), and rendered on REMEDY as its own `unscoreable by design` row — so the plain `unverifiable` counter again means only what it should: a gap more evidence could have filled. The per-side floors live in one exported `sideFloorMet` (factored out of `scoreRecovery`, which keeps only its comparisons and the joint tx-ratio check), so the verdict and the classification can never disagree about where a floor sits. This also explains the original rate-vs-rtt asymmetry in one stroke: `rate`'s floor is one fresh reading, `rtt`'s is three, so the same one-sample blink scores the first and starves the second.

> Considered and rejected: skipping the confirm burst once the before-window is frozen below its floor. The five probes are what fill the after-window on a quiet node, and without them the classification itself would misfire — the transient would present as an after-side starvation and land back in the fixable counter. Five probes (routed reads since v0.71.0) are a fair price for the taxonomy staying honest.
...
>
> **Those nodes have no liveness coverage from either layer (v0.65.0, live audit).** The driver marks a node Dead only when a transmission it attempted fails, and it never attempts one to a sleeping device; this add-on never sweeps them, and `quiet-node` is gated on `isListening === true`. So a battery sensor that stops reporting altogether is invisible to both. On the reference mesh that is 3 of 39 nodes (two motion sensors reporting every ~71 min, one every ~61 min). The audit measured their cadence over 49 h: **zero missed slots** — no gap on any of the three exceeded 1.01x that node's own median — but the jitter around the period is percent-scale, not a fraction of a percent. Worst gap over median: ~71.19 min +0.39 % (5 gaps outside 0.1 %), ~71.28 min +0.00 % (none), ~61.04 min +0.93 % (4), with the outliers scattered across both days rather than clustered on the driver restarts or the 07:00 UTC blackouts. That is the data the original design note lacked when it worried that watching sleeping nodes would make "every sleeping sensor a standing alert", and it says which watch is safe: one keyed on a MISSED SLOT (say 1.5x the node's own measured period) is cheap and low-false-positive, while one sized on the period's jitter must allow several percent — a tenth of one would have alerted nine times in 48 h on a mesh this audit calls healthy. It remains UNBUILT — recorded here so the gap is a known one rather than an assumption.

> **Measurement probes never reach the ledger (v0.38.1).** The sweep and the verification bursts ride non-learning verbs — `ActionRunner.probe`, the same NoOp ping with `learn=false`, and since v0.71.0 the routed read `routedRead(n, 'probe')`, also `learn=false` — while only the dead-node remediation ladder keeps the learning `ping` verb. (Not being RECORDED as a treatment is not the same as not being one: a read can re-route a node, and v0.71.0 confounds the episodes that could clear by that alone — see *A measurement can change the route it measures* below.) This closes an audited defect: all three lanes shared the learning verb, so every sweep and every verification probe stamped `ping` as the remediation action on any open episode (the open-burst fires within a minute of every episode opening, and first-action-wins), which meant **not one scoreable "(no action)" closure existed in the entire retained log**. The control arm was structurally starved, `baseRate` stayed null, and `expectedEfficacy` was uncomputable — the measurement instrument was the recorded treatment. With the split, a symptom that clears under measurement probes alone now feeds the CONTROL arm, which is what finally makes "helped vs self-healed" answerable for listening-node kinds.
...
> **Self-proven means the node's OWN voice, not our echo (v0.40).** A probe answer advances `lastSeen` too, so the cadence-only definition counted the node's reply to the *previous sweep's probe* as it speaking on its own — the audit's tell was the line "already heard 120m ago on its own — confirming", a full staleness threshold of silence described as confirming. Worse, for quiet-but-answering nodes the confirming/unheard split was decided by sub-minute scheduling timing and was STICKY per node, so physically identical devices accrued opposite persisted `probesSelfProven` counts. Self-proven now additionally requires `lastSeen` to have advanced strictly PAST what our own last answered probe put on the record (`lastProbeSeen`, written at judgment). The sweep line gained a third honest state for the echo case: *"nothing heard past our last probe's answer Xm ago — probing for its own voice"* — worded as exactly what is known, because the attribution value is stamped at judgment time (up to ~2.5 min after the probe) and a node's own report landing inside that ambiguous window is attributed to the probe (conservative: under-credits, never fabricates).

> **Traffic outranks the driver's Dead flag (v0.42.0).** `status === Dead` is the driver's REACTIVE opinion: set when a transmission fails, cleared only when one succeeds. It is not a measurement of reachability, and the engine used to treat it as one. Node 49 ("Garage Workroom") ignored **six consecutive pings over ~12 hours** — the ladder's three, a manual one, and two more after a restart re-armed it — and was declared `node-down`; it then answered an ordinary on/off command immediately and came back reading grade A with +25 dB of margin. The ping button issues a NOP, and that device does not answer NOPs. Every conclusion downstream — the Dead status, the ladder, the give-up, the symptom — was drawn from the one frame it will not reply to. So the dead lane now skips any node **heard from inside the dwell**: no remediation budget spent, no human summoned, and the stale flag announced once per outage (*"reads Dead but was heard from within the dwell — trusting the traffic over the flag"*) rather than silently skipped, which would be worse than the old behaviour. A genuinely silent Dead node is still remediated exactly as before — the guard must not swallow the case the ladder exists for, and a test pins that. The give-up message was corrected in the same release to report what it MEASURED (*"did not answer N pings … that means it ignored N NOP frames, NOT that it is unreachable"*) and to lead with operating the device, which is a stronger reachability test than any number of NOPs.

> **A probe that never left is never judged (v0.40.2).** `zwaveActions.run()` catches its own errors and **returns** `{ ok: false }` — it does not re-throw. Every probe lane guarded its launch with `.catch(unpendProbe)`, which therefore sat on a promise that could not reject: `unpendProbe` never executed in production (zero "could not be probed" lines across the 1206 probes in the retained log; the defect is older than that window — it is simply where it was measured), and every add-on-side failure — HA WS down, Core restarting, no ping button — was judged a moment later as **the node** failing to answer. An audit caught it red-handed: during a Core restart a node logged `did NOT answer` while never going Dead, because the button press never reached HA and the driver never attempted a transmission at all. Three costs, worsening: the log blamed the device while the real transport error went only to the in-memory event ring; `probesAsked` incremented without `probesAnswered`, permanently poisoning a persisted counter the Detail screen colour-codes; and in the dead lane `noteAttempt` fires *before* the call, so the ladder spent a node's 3-attempt budget on packets that were never transmitted. `settleProbe` now treats a resolved `ok:false` exactly like a rejection — the probe is withdrawn from judgment, and the dead lane **refunds** the attempt it booked (captured before `noteAttempt`, or the refund hands back the value the attempt just spent). The backoff clock is deliberately **not** refunded, and launch failures carry their own bounded budget: the first cut of this fix returned both, which put `tries` back to 0, made the ladder index `BACKOFF_MS[-1]` → `undefined`, and turned a persistent launch failure into a once-per-tick ping loop with the give-up unreachable — 190 pings in 200 simulated minutes, caught by pre-release review. Exhausting the launch budget announces itself apart from a node that was genuinely asked and stayed silent, because the fault is ours, not the device's.

> **Only the fixed-cadence sweep feeds the persisted reply rate (v0.40.2).** Verification bursts and dead-remediation probes are symptom-correlated: opening an episode injects up to ten extra probes and going Dead injects near-certain misses, all into the same denominator, so a node under investigation accrued 22 probes against every peer's 13. That destroys the one comparison the counter exists for — the v0.37 rebuild's whole point. Each pending probe now carries its `ProbeLane`, and only `'sweep'` reaches `recordProbe`. **The persisted counters are deliberately NOT migrated**: `probesAsked`/`probesAnswered`/`probesSelfProven` in `/data/evidence.json` keep whatever they accrued under the old all-lanes definition, so a long-lived install's ratio blends two definitions until the old contribution stops mattering. Resetting them would destroy real history to tidy a display ratio; the honest trade is to keep the history and record the discontinuity here. Misses in every lane still log and still move the miss streak: a miss is a miss. **The frame is recorded too (v0.71.0).** An answered NoOp means the controller reached the node on a stored route; an answered routed read means it reached it by any route it could find in one send, including one it computed. So the store keeps the read subset apart (`probesReadAsked` / `probesReadAnswered`, persisted as `ra`/`rk`) beside the unmigrated lifetime pair, and NODE DETAIL splits the ratio by frame on a **Frame** row; a read-era rate approaches 100 % and stops showing weak stored routes, whose signal moves to the **Rerouted** row (`probeReroutes`, `rr`: sweep reads whose stored route failed on the way and were delivered by another).

> **A driver restart is a third party's probe (v0.65.0).** zwave-js pings the whole mesh itself as it comes up, which advances every node's `lastSeen` within seconds — evidence this add-on did not produce and cannot attribute. Read as the node's own voice it becomes a `self-proven` credit, and the 2026-09-15 audit counted 28 of them across two driver restarts in 23 h, into the same persisted, never-decaying counter v0.40.2 was written to protect. A `lastSeen` that lands inside the driver-restart burst (`DRIVER_BURST_MS`, 2 min, plus a 5 s lead) is therefore classed `attribution-unknown` and credits nothing.

**The window has TWO anchors, because one of them is missing exactly when it is needed (v0.65.0 review).** The driver-WS version handshake is the precise signal, but a driver restart is the event most likely to take that link down: in the audit the *second* restart's reconnect ladder stopped after attempt 2 and never handshook again, so anchoring on it alone would have caught 3 of the 28 credits and left the other 25 exactly as they were. The `zwave_js` config-entry reload is the teardown half of the same restart, it is observed on the HA socket (which stayed up), and it lands seconds *before* the driver's ping burst — so `driverReconnectedAt()` reports whichever of the two is later. The window is BOUNDED on purpose, and bounded by construction: it is measured from the anchor, so a stale anchor matches nothing and neither signal can suppress self-proof for the life of the process.

> **Unknown attribution is never credited (v0.40.2).** `lastProbeSeen` is per-process, so the first sweep after a restart cannot tell the node's own traffic from the previous process's probe echoes. v0.40/v0.40.1 resolved that ambiguity as "on its own" and credited it — measured cost: 35 fabricated `confirming` labels and 35 false self-proven credits into a persisted, never-decaying counter, once per boot, fleet-wide. The sweep now says what is actually known (*"heard Xm ago, but this run has no probe attribution yet — not credited"*) and credits nothing while attribution is unknown — a node that genuinely speaks on its own before any probe of this run has been judged is left uncredited too, which under-credits rather than fabricates.

> **The sweep runs in bursts, and a restart re-probes every node once — both on purpose.** The cadence gate (`lastStaleAt`, one probe per node per `staleMs`) is per-process, like attribution. A mesh probed back-to-back once stays synchronised, so the sweep settles into bursts — on the reference mesh 35 probes a minute apart, then about 86 minutes idle — and a restart makes every node due at the end of the boot window, re-probing all of them even if the previous process asked minutes earlier. Neither costs what it appears to. Every node is still asked every 120 minutes to within a second, so per-node detection time and total airtime are unchanged by the bursts; even spacing would buy nothing but roughly 800 extra log lines a day (the trace line fires on every probing tick). The restart burst costs one extra probe per node per restart (a routed read since v0.71.0) — about 0.2 % of a normal week's airtime — and buys the two things a restart most needs: this process's probe attribution re-established for every node within about half an hour, instead of up to 120 minutes during which every sweep would read *not credited*; and a fresh reachability check of the whole mesh at exactly the moment a host reboot or an update may have changed it. Seeding the cadence from the previous run would remove the extra pings and both of those. Comparability, the reason the sweep asks every node (v0.37), is untouched: every node receives the same one extra probe.

The echo label is routed by attribution alone, never by the staleness threshold (v0.40.1): the first production audit of v0.40.0 caught the recency gate splitting one physical situation — a node answering every probe, silent otherwise — into "echo" at 119 minutes and "unheard for 120m" at 121, on sub-minute scheduling jitter, sticky per node. "Unheard" is reserved for a node this run can actually judge silent (v0.66.0): one with nothing on record at all, or one heard on its own past our last probe's answer and then quiet past the threshold. With no probe attribution yet — the first sweep after a boot — an old `lastSeen` is `attribution-unknown`, not `unheard`, because `echo-only` cannot be ruled out: at one boot in the 2026-09-17 log review 24 nodes were booked `unheard`, and all 24 read `echo-only` at their next sweep. Two accepted residuals, documented in the state comment: that swallow window, and the in-memory map resetting on restart (one possible echo-as-self-proven per node per boot). The pre-release review also removed same-tick twin probes: a node owed a verification probe is dropped from that tick's sweep — the verify probe is the same frame (both lanes choose it through one `frameOf`, v0.71.0) and answers the sweep's question, while twins shared one timestamp, let a transport failure withdraw the wrong lane's pending entry, and double-counted one silent instant as two misses.

> **Every probe gets its own judgment (v0.40).** `awaitingAnswer` was a single per-node slot, and burst spacing (60 s) runs under the answer grace (90 s) — so each new probe overwrote the pending judgment before it matured, only the LAST probe of every 5-probe verification burst was ever judged, and a node dying mid-burst had five consecutive misses logged as one "1st consecutive miss". Pending probes are now a per-node LIST; every matured entry is judged oldest-first, so the miss streak and the persisted reply rate see all of the traffic. Each entry also CARRIES its own self-proven flag — the per-node flag slot had the same overwrite disease, a newer sweep rewriting the flag before the older probe was judged. Non-sweep lanes (dead-remediation, verification) carry `self: false`: the flag answers a question only the sweep asks.

> **Verification probes ride this same runner (v0.36).** The outcome ledger requests them (§9.7); they are cleared by `decideAutoPings` alongside the remediation and liveness lanes precisely so they inherit every gate rather than opening a second, less-guarded path to the mesh, and they are logged to **both** destinations at info, like every other autonomous write here. (v0.36.0 put them on the server log at debug only, on an estimate that did not survive contact with the mesh — roughly 60 verification probes per 39 hours against ~635 liveness probes is about a tenth more, not a flood — and the cost showed up immediately: the v0.36.0 deploy could not be verified from the container log at all. Corrected in v0.36.1.) The per-probe **answered** confirmation stays at debug: several hundred lines a day saying "as designed" is the noise that trains an operator to stop reading. The **unanswered** case is the signal, and it reaches both destinations — `info` for a first miss, `warn` from the second consecutive miss (v0.36.5).


> **When a Dead episode starts (v0.50.0, corrected v0.64.3).** The dwell is measured from `deadSince`, and what `deadSince` means depends on whether this run has seen the node alive. A node that is already `Dead` the first time the runner looks is dated from the driver's `lastSeen`: it has been down at least since it was last heard, and dating it from start-up handed it a fresh outage on every deploy (v0.50.0). The value is clamped to `now`, so a future or absent `lastSeen` cannot invent an outage older than the add-on's uptime. A node the runner has observed `Alive` is dated from the tick it is first seen `Dead` (v0.64.3). Its `lastSeen` is only its last transmission — about two hours old for a healthy node, because the liveness sweep runs every two hours. Backdating to it made every observed death skip the dwell entirely: in 50 h of live log, all six ladder probes fired one 60 s tick after the sweep miss that caused the death. Only `Alive` counts as observed, because `trackEpisodes` runs on every tick whether or not the roster is ready, and `Unknown` proves nothing. The mark is dropped when a node leaves the roster, so a re-included device reusing the id is a first sighting again.

> **A sweep kill is not an outage (v0.64.4).** The ping verb is a single-attempt, ACK-only NoOp (transmit options `0x01`: no AutoRoute and no Explore bit — fixed by zwave-js-server, which calls `node.ping()` with no argument), so on a marginal hop the probe itself fails and the driver marks the node `Dead` on that frame. Of the six Dead flags the add-on saw outside a host reboot in 49 h on the reference mesh, five landed 0.26–1.74 s after a liveness-sweep probe, and the next ping reached the node in 30–50 ms; the sixth came 26 minutes after its sweep, from a node heard inside the dwell, and healed itself within two minutes. A quiet mains node that nothing else addresses cannot heal itself, so under v0.64.3 each sweep kill became 11 minutes Dead, a critical `node-down` and six minutes of `degraded`. `trackEpisodes` now settles the case on the tick it first sees a watched death: every pending probe the node has not answered — not heard since the probe's `at`, the judgment's own test applied early — is booked as a miss there and then (logged, added to the miss streak, and for the sweep lane passed to `onProbeResult`). If the newest of them was a **sweep** probe — or, since v0.71.0, a **verification** probe — the node joins `probeDeath` (keyed by that lane), which exempts it from the dwell and stops traffic heard *before* the death from counting as "heard within the dwell" — the failed probe is the newer evidence. Through v0.70.0 a verification burst did not earn the exemption, because it keeps probing: an immediate retry handed it a live node to kill again, and kill–revive–kill is enough crossings for a critical `dead-flap` whose episode requests another burst. The post-kill hold now bounds that loop instead (next note). A manual ping still does not (see the v0.64.5 note below). `deadSince` keeps the observed death, and the mark clears on recovery and on roster departure. Booking at the death instead of at `ANSWER_GRACE_MS` also closes a credit leak present from v0.50.0 to v0.64.2 — the ladder's retry revived the node inside the grace, and the probe that killed it was booked answered — which NODE DETAIL discloses under **Probes** until `creditEpoch` retires the caveat for a node first seen after it. The probe nearest the death decides: an older sweep entry still pending after an ordinary lost reply lends no exemption to a manual kill (through v0.70.0: to a verification-burst kill). The two residuals this note once listed are closed in v0.71.0: a death that heals inside one 60 s tick is seen from the status feed, and a burst can no longer find the revived node inside the hold.

> **Verification kills, the hold, and deaths between ticks (v0.71.0).** A verification kill now gets the immediate retry a sweep kill has had since v0.64.4. The dwell was the only thing stopping a burst from killing its node again, and it cost the availability it guarded: on 2026-09-22 a verification probe killed a working outlet, the burst's own earlier answers — traffic from *before* the kill — filed it as "talking", and it read Dead for 101 minutes until a switch command reached it. Three rules replace the dwell as the brake:
>
> - **The hold.** When a node recovers from a death on our own sweep or verification probe, it gets no sweep and no verification probe for `PROBE_KILL_HOLD_MS` (15 min), timed from the RECOVERY. A kill and its revival are two Dead crossings; `dead-flap` needs three inside 10 minutes; so our own probing cannot produce one. (Timed from the kill, a late revival would leave room for the third.) A held node's verification burst is left **owed**, not drained — `drainVerifyRequests(now, skip)` skips it — so the ledger's budget is not spent on probes that are never sent (the v0.36.2 disease). The dead ladder is never held: an unprovoked death inside the hold still gets its dwell and its pings.
> - **The count.** Each own-probe kill is kept per node for `PROBE_KILL_WINDOW_MS` (24 h, across recoveries). The second inside the window raises ONE warning — *"went Dead on this add-on's own probes 2 times in 24 h … its routes are marginal"* — and ENGINE tracks the node with `N own-probe kills 24h` and, while it lasts, `probes held Nm`. A manual ping's kill is never counted.
> - **Deaths between ticks.** `trackEpisodes` level-samples status once a tick, so a death that clears inside one tick never reached it. Two things revive a node that fast: any frame FROM a Dead node marks it Alive, so a node that reports on its own clears a NoOp kill by itself; and a Get can clear its own — the node takes it, its ACK is lost, the driver marks it Dead on the NoAck, and the node's Report marks it Alive again within a second. The event feed already counted both crossings into `dead-flap` and the confound guard, so zwaveData now also queues every Alive→Dead transition with the node's `lastSeen` at that moment (`drainDeadEvents`, from the status feed or, for a node without one, the roster diff), and hands a death to the runner only once the same source has seen it CLEAR — a Dead node the 2 s roster has not caught up with is left to `trackEpisodes`, which would otherwise book it a second time. The runner reads the queue after `trackEpisodes` and blames a death the tick did not see on our probe only by `settleProbeDeath`'s own test: the newest pending sweep or verification probe sent up to `ANSWER_GRACE_MS` before the death that the node had NOT answered by then. Such a death is counted as an own-probe kill and starts the hold. A routed read revived within `READ_REPORT_REVIVAL_MS` (5 s) of the death — inside the Get's own report timeout — stays pending and is judged ANSWERED: that revival was its Report, and a Report proves the Get arrived (*"went Dead Ns after our sweep routed read and was Alive again before the next tick (revived 0.3s later — a lost ACK, most likely: its Report revives it)"*). Anything else is settled there and then as a MISS: a NoOp gets no reply, and a Get whose NoAck was not followed at once by its Report was not delivered, so whatever revived the node — its own report, another sender's acknowledged frame — was not an answer, and crediting it would reopen the v0.64.4 credit leak. A death on a node whose statistics feed is not live is not blamed at all (`feedLive`): without that feed's `lastSeen` at the death, an answered probe cannot be told from an unanswered one. A read booked this way is marked (`killBooked`), so neither a second death inside its grace nor a later tick that sees the node Dead with it unanswered books it again. The deaths are judged even when the node is Dead AGAIN at the tick on a later death: `trackEpisodes`, reading the `lastSeen` the revival moved, cannot book the first one, and nothing can be booked twice, because `settleProbeDeath` removes every probe it books and an answered probe fails the at-death test.
>
> A read that goes unanswered is a sweep or verification **miss**, never a ladder read: its line ends `— sweep routed read` (every miss line now names its lane and frame), it feeds the reply rate as a missed READ, and it owes no ladder read — the ladder pings first. The immediate retry is the ladder's learning ping, because that ping is the episode's recorded treatment; when it goes unanswered the v0.70.0 routed read follows. A revival by that read in an episode that began on our own measurement probe does not spend `READ_REVIVAL_CAP`: that cap bounds a loop in which a NoOp keeps killing a device the read keeps reviving, and a measurement kill is bounded by the hold and counted apart. (Whether the episode began that way is captured when the read is launched, because recovery clears `probeDeath` before the read is judged.)

> **A measurement can change the route it measures (v0.71.0).** A NoOp can use only the stored routes; a routed read can also be delivered on a route the controller computes, and a successful Auto Route delivery can become the node's new last working route. v0.38.1 kept the measurement lanes from being RECORDED as a treatment; it never stopped a probe from BEING one. So zwaveData attributes a route change to our probe only when all three hold: it arrived with our measurement probe's OWN TX report; that probe was a **routed read**; and the new `lwr` carries `routeFailedBetween`, which zwave-js sets only on a TX report whose route failed on the way (Node.ts `updateRouteStatistics`). zwave-js rewrites `lwr` from EVERY frame's TX report, so a time window alone would hand our read the failover of the ladder's retry, an operator's ping or an automation's command sent inside the same 90 s. Ours is taken to be the first TX report for the node after the stamp (`onMeasurementSent`, taken before the send, with the node's `commandsTX + commandsDroppedTX`), within `ANSWER_GRACE_MS`. zwave-js throttles node statistics to one emit per 250 ms with a trailing call, so on a quiet node that one report usually arrives as TWO events: the counter increment at once, still carrying the OLD route, and the route it carried in the trailing emit about 250 ms later, counters unchanged. The attribution therefore stays open across events with the same counters for `OUR_TX_TRAIL_MS` (2 s), and closes when the counters move again (another frame's report), when the window passes, or once a route change has been judged on it. An event whose counters moved by MORE than one report since the stamp is not attributed at all — the throttle coalesced several, and the route it carries is the last one's. The stamp is withdrawn when the probe's launch was refused (`onMeasurementWithdrawn`), together with any report already taken for it, and dropped when the node is marked Dead (an unacknowledged frame was not delivered by any route). Two residuals: a frame to the node already queued or in flight when our read was stamped — or queued after it but sent first, since the read waits at NodeQuery priority — reports first and is taken for ours, bounded to that one frame; and a read to a node that needs a nonce exchange first (every S0 node, and an S2 node re-establishing its SPAN) produces two or three TX reports, the failover on the first of them, so it attributes nothing and its failover is counted as ordinary route churn. A route change merely first SEEN on our probe — an owner's route rebuild, a priority route, a re-route the controller made earlier — is revealed, not caused: statistics move only on a TX report, and on a quiet node our sweep is often the next frame. An attributed change is logged (*"stored route failed on our sweep routed read and the controller delivered it by another"*), counted into the node's persisted `probeReroutes` when the probe was a SWEEP (a verification burst is symptom-correlated), kept out of `route-churn`'s accumulator — a burst whose reads keep failing over between two routes would otherwise open an episode our probing then asks to verify with more probing — and, at the next engine pass, confounds the node's open `rate-fallback`, `rtt-degraded`, `weak-signal`, `return-path-degraded` and `chronic-return-path` episodes (the kinds whose recovery metric a new route moves by itself) and its open `route-churn` episode: the confirm burst's own reads fill that episode's after-window, and with their failovers kept out of the count it would otherwise score `improved` on a node whose routes never settled. `dead-flap` is left alone, because a flap is its own measured signal (the v0.40 rule); `quiet-node` and `s2-desync` do not depend on the route.

> **What else a read does that a ping does not (v0.71.0).** Each is recorded so it is not mistaken for a defect:
>
> - **It asks for a Report.** A Get whose ACK arrives but whose Report does not still advances `lastSeen` (answered — the node took the frame) and moves `timeoutResponse`, which a NoOp never does. A lost Report is a failed delivery on the return path, so it counts as evidence where timeouts count — `dTimeout`, the timeout baselines, and weak-signal's corroboration on an unmeasured window — as intended evidence, not noise.
> - **It is encrypted on a secure node.** 28 of the 35 mains nodes on the reference mesh are S2; a NoOp is always sent in plain, a Get is S2-encapsulated, and about 6 % of S2 frames needed a nonce exchange first. A resync it causes can reach the `s2-desync` lane, whose threshold (12 in 30 min) our probes alone cannot reach.
> - **Its Report can correct Home Assistant's state.** A Report carrying the value HA already holds is `state_reported` only: no `state_changed`, no automation, and REST `last_reported` does not move. A Report carrying a DIFFERENT value — HA's state was stale — is an ordinary `state_changed` and can fire automations, which is correct (the device's state changed earlier) and is not attributed in the log: a physical press inside the same second cannot be told apart.
> - **It costs more air.** A first-route Get plus its Report is about 2.1× a ping on air (about 2.4× under S2): roughly +32 s of controller transmit time a day for ~500 probes, about 500 more inbound frames, and a Z-Wave JS driver log about 7 % larger. A failing read can hold the send queue for up to the 20 s `SendDataAbort` timeout, against about 90 ms for a failing ping — once per outage onset.
> - **Home Assistant logs a failed one.** A `refresh_value` whose Get fails is logged by HA as `Error while refreshing value …` at ERROR — so it appears at the reference install's WARNING log level — though a race with the node going Dead can hide it. The per-call `Refreshing primary value` line is INFO and does not appear unless the zwave_js logger is raised.
> - **It leaves no `button.*_ping` state change in the recorder.** The 67-day kill attribution above was built from those; from v0.71.0 the add-on log is where each sweep and verification probe's time is recorded.

> **A manual ping is dated from its launch (v0.64.5).** An operator's `p` is judged by the same `judgeProbeAnswers` as an engine probe (v0.47.0), but its pending entry was stamped by `notePending` when the data layer heard the outcome — after Home Assistant's `button.press` call had returned. HA's zwave_js ping button starts the driver's ping as a background task (`hass.async_create_task`) and returns without waiting for it, so that stamp usually precedes the answer: in five dead-ladder probes on the reference mesh, the driver logged the revived node alive 54–197 ms after the add-on logged the probe. Usually is not always. The node's answer and HA's reply race, and when the reply loses — a busy Home Assistant event loop, or a node that answers in a few tens of milliseconds — the stamp post-dated the answer, `lastSeen >= at` failed, and an answered manual ping was logged *did NOT answer* after the 90 s grace, with a miss on the node's consecutive-miss streak; the same test in `settleProbeDeath` could book it as the probe a later, unrelated death happened on. No instance has been observed, because no manual ping has been: over the 50.5 h of log reviewed for this release Home Assistant recorded 1,081 ping-button presses, and every one was the engine's. `zwaveActions.run()` now reads the clock before it awaits the call and passes it as `sentAt` through `onOutcome` → `recordActionOutcome` → the probe hook → `notePending(nodeId, 'manual', at)`. The runner's `now` and auto-ping's both default to `Date.now`, the wall clock `lastSeen` is compared against. The stamp now precedes the send, as every engine lane's does, so a frame the node happens to send between launch and transmission counts as the answer. That interval is the HA round trip and the driver's send queue, plus up to 10 s while the HA WebSocket is re-authenticating (`waitReady`) — the tolerance the sweep has always had, applied to the operator's lane. One ordering consequence: a manual entry registers after any engine probe pended during its call while pre-dating it, so across lanes a node's pending list is in registration order rather than strict send order. For `settleProbeDeath`'s newest-decides rule that selects the manual entry, which keeps the dwell. The persisted reply rate and the ledger's action arm are unchanged. Manual kills still keep the dwell; the loose stamp is no longer a reason, and two remain. No manual kill has been observed (the same 1,081 presses). And `probeDeath` discards traffic heard before the death: the give-up notice tells an operator to operate the device and then ping it, and if that command clears the Dead flag and a tick sees the node Alive before the ping, the ping's NOP makes a fresh watched death — for a device that ignores NOPs (v0.42.0), the exemption would trade its Dead-but-talking notice for an immediate retry of the frame it ignores.
...
**Which frame each lane sends (v0.71.0).** Nothing in this module can remove a node or change a device's configuration, and it holds no privileged channel: every frame is an ordinary `ActionRunner` verb (§11.2) over the authenticated HA WS.

| Lane | Frame | HA call | Options, attempts, priority (at source) | Learned | Feeds reply rate |
|---|---|---|---|---|---|
| Dead ladder | NoOp ping | `button.press` on `button.*_ping` | `0x01` (ACK only), 1, Ping | yes (`ping`) | no |
| …its follow-up (v0.70.0) | routed read | `zwave_js.refresh_value`, `refresh_all_values: false` | `0x25` (ACK, AutoRoute, Explore), 1, NodeQuery | no | no |
| Liveness sweep | routed read; NoOp where the node has no switch/light value, or its read could not be sent in the last 30 min | the same | the same | no | yes, with the read subset apart |
| Verification burst | the same frame as the sweep (`frameOf`) | the same | the same | no | no |
| Manual `p` | NoOp ping | `button.press` | `0x01`, 1, Ping | yes | no |

A ping can use only the stored routes (the last working route, the next-to-last, then direct); when it goes unanswered the driver marks the node Dead. That is why the measurement lanes stopped sending it. Over 67 days of recorder history on the reference mesh, **100 of the 106** times a mains node went from Alive to Dead began 0.11–3.49 s (median 0.33 s) after one of this add-on's own ping presses: 63 on a sweep probe, 35 on a verification probe (28 of them one node's kill–revive loop on 2026-08-22/23), 2 not attributable to one lane; five nodes accounted for 89 of the 100, and 24 of the 35 mains nodes never had one. The per-probe kill rate was 0.46 % over the whole window, 0.39 % since v0.37 and 0.18 % since v0.64.4, when the immediate retry held a sweep kill to about 60 s; the one long episode of that later regime was a verification kill, 101 minutes. In the 39 h driver log reviewed for this release, 5 of 821 NoOps went genuinely unanswered, all to one weak node, and 0 of 170 `0x25` frames did (95 % upper bound ≈1.8 %): 18 of the 170 needed more than one routing attempt, 3 were delivered by Auto Route, none by an explorer frame. A read adds a controller-computed route to the attempts; it can still go unanswered, and then it marks the node Dead exactly as a ping does, which is why the containment below stays. Verification probes move with the sweep rather than keeping the NoOp: they go to symptomatic nodes, killed at 0.59 % per probe against 0.36 % for the sweep, and caused the only long episode; the concern that a read heals what it measures is handled by the re-route confound, not by keeping the killing frame. The dead ladder and `p` keep the ping: a node already Dead cannot be killed by it, the ladder's ping is the outcome ledger's recorded treatment, and an operator who asks for a ping gets one.

> **One routed read after an unanswered ladder ping (v0.70.0).** Because a ping is ACK-only, it can use only the controller's stored routes — the last working route, the next-to-last, then direct. Every ordinary command goes out with the driver's default options (`0x25`: ACK, AutoRoute, Explore), so the controller may also compute a route from its routing table. When every stored route is marginal, no number of pings reaches a node that any real command reaches at once. On the reference mesh, 2026-09-22, the ladder spent three pings on a plug-in outlet and summoned a human; a switch command then revived it in 340 ms through a repeater the stored routes did not use, and its next ping succeeded over that new route. So each ladder ping **judged** unanswered earns one `routedRead`: `zwave_js.refresh_value` with `refresh_all_values: false` on the node's own Binary/Multilevel Switch `currentValue` entity (chosen by the value id in its `unique_id`; config-parameter switches, node-level entities and `switch_as_x` wrappers never qualify). At source that is `Node.pollValue`: one Get, one attempt, default routing, no state change, and — unlike `refresh_node_values`, whose refresh task returns before querying anything when the node is Dead — it is sent regardless of status. It is judged exactly like a ping, by `lastSeen` advancing past the moment it went out, because HA returns before the Get is on the air. Rules: at most one read per rung, sent on the tick after the ping's judgment and before the next rung or the give-up (the give-up waits for its judgment, about four minutes later than before); it spends neither the attempt budget nor `launchFailures`; a read that never leaves is un-counted and not re-sent; a read the RF blackout swallows is owed again; it obeys every gate the ladder does; battery/FLiRS nodes and nodes with no readable value never get one (their ladder is exactly v0.69.0's). **Bounded:** a device whose stored routes keep failing is revived by the read and killed again by the next ping, and recovery resets the ladder, so after `READ_REVIVAL_CAP` (2) read revivals of one node inside `READ_REVIVAL_WINDOW_MS` (24 h) the read is withheld, a WARN names the loop, and the next death ends in a summons that says why. (Since v0.71.0 the next ping is the ladder's or an operator's — the measurement lanes read — and a revival in an episode that began on our own measurement probe does not count against the cap; see the v0.71.0 note above.) In v0.70.0 the sweep and verification lanes were left unchanged; v0.71.0 changed their frame (see *Which frame each lane sends*). A revival by the read is credited in the outcome ledger to the episode's first action (the ping), as a human's command is today; the log line `answered our routed read` is the attribution. The v0.70.0 text cited "81 of 85 mains Dead episodes ended on one of this add-on's pings" as evidence that pings revive nodes; the same episodes had BEGUN on this add-on's own pings, and the count missed two nodes whose entities carry a `_2` suffix — the corrected figure is 100 of 106, and it is the reason for v0.71.0.

**Three probe kinds** (`server/src/zwave/autoPing.ts`) — the two below, plus the ledger-requested verification probes of §9.7, which ride the same runner (v0.36, see above):

1. **Remediation probe** — a mains node reported `Dead` for longer than the dwell is pinged, up to a per-episode budget with escalating backoff, and each ping judged unanswered is followed by **one routed read** on the next tick (v0.70.0; see the note below). Either the node answers (and the driver clears `Dead`) or the attempt budget exhausts and the node is left to the human remediation path with its evidence intact.
2. **Liveness probe** — Z-Wave JS sets `Dead` *reactively*, only when a transmission fails; a node nobody addresses can be unplugged for hours while still reading Alive (measured on the reference mesh: 10 of 38 nodes silent 35.7 h, all "Alive"). So every listening, non-Dead node is asked once per sweep cadence (v0.37; through v0.36 only nodes silent past the threshold) — by one routed read since v0.71.0, a NoOp ping where it has no switch/light value — to convert silence into evidence: an answer refreshes `lastSeen` (and route/RSSI statistics with it); a failure flips the node to `Dead`, where the remediation path owns it.
...
| Gate | Value | Why |
|---|---|---|
| Attempt budget `auto_ping_max_attempts` | 3 per dead episode (1–10) | Then stop and leave it to a human; recovery clears the episode so a later failure gets a fresh budget. Each unanswered ping is followed by one routed read, which spends none of this budget (v0.70.0) |
| Backoff between attempts | 10 / 30 / 60 m; at the default 3 attempts only the first two waits are reached (10 m dwell + 10 m + 30 m ⇒ ~50 m to the give-up), and the 60 m rung governs the third wait onward | A node that did not answer seconds ago will not answer now; avoid avoidable RF |
| Controller RF off | The controller reported `Turning RF off` on the driver log stream and has not reported it back on (v0.65.0). The reading expires 2 min (`RF_OFF_MAX_MS`) after the controller LAST reported RF off — each `off` line refreshes it — and also on a dark log lane and on any driver-WS reconnect | The nightly NVM backup takes the radio down for ~10 s and soft-resets the controller (observed 07:00 UTC, both nights of the 2026-09-15 audit). Seen firing live on 2026-09-21 at 07:00:08 UTC against zwave-js 15.29.0, whose log still emits both `Turning RF off` and `Turning RF on`; because the gate is read on the 60 s tick, a given night shows a `controller-rf-off` suppression line only when a tick lands inside the ~10 s window, so a night without one is not a failure. A frame sent across that window cannot be acknowledged, and ONE unacknowledged sweep frame is enough for the driver to mark a node Dead — a false death this engine would then report, remediate and count. Probes already in flight when the radio goes down are dropped **unjudged** rather than booked as misses: an unanswered frame there measures the radio, not the node. Two limits, stated so the guarantee is not read wider than it is: the drop can only act on a tick that sees the reading, so for a blackout shorter than the 60 s tick it covers the ticks that land inside the window and not the probe launched just before it; and a probe whose node was already seen to go Dead on it is settled first and stays booked — that death was observed with the radio on, and keeps its v0.64.4 dwell exemption |
| Boot window | 5 m after start (or roster not ready) for the sweep and verification lanes; the dead ladder waits only for a ready roster, and the gates it passes still report `boot-window` (v0.64.4) | Right after start every node can read Dead; without this the engine would ping the whole mesh on every restart. A ready roster is that hazard's own measure — holding the ladder to the wall clock kept a node's Home Assistant entities unavailable ~3 min longer after a host reboot. The trade: a restart loop no longer rate-limits the ladder, which can probe each quiet Dead node once per start, no sooner than the first tick |
...
| Dead dwell `auto_ping_after_min` | 10 m (2–120); not applied to a node that went Dead with a sweep (v0.64.4) or verification (v0.71.0) probe of ours to it unanswered | Measured on the reference mesh: self-healing episodes resolve inside ~5 m, stuck ones run 6–9 h — 10 m sits in the gap. A node our own probe knocked Dead is not healing, so the dwell would only hold it down |
| Post-kill hold `PROBE_KILL_HOLD_MS` | 15 m from the recovery of a death on our own sweep or verification probe (v0.71.0) | No sweep and no verification probe to that node (its burst stays owed); longer than `dead-flap`'s 10-minute window, so our probing cannot make three crossings in one. The ladder is not held |
| Repeat-kill warning | `PROBE_KILL_WARN_AT` = 2 own-probe kills of one node inside `PROBE_KILL_WINDOW_MS` (24 h), once per crossing (v0.71.0) | Once is a lost frame; twice a day is a marginal route worth a human's look |
| Read fallback `READ_LAUNCH_FALLBACK_MS` | 30 m of NoOp measurement probes for a node whose routed read could not be SENT (v0.71.0) | The v0.40.2 refund keeps a refused sweep at the head of the one-per-tick queue; without a fallback one refusing entity would stall the sweep for every node |
| Answer grace `ANSWER_GRACE_MS` | 90 s | How long a probe waits before it is judged; also the window in which a route change can be attributed to our read, and in which a between-tick death can be put on our probe (v0.71.0) |
| Report revival `READ_REPORT_REVIVAL_MS` | 5 s (v0.71.0) | A routed read killed between ticks counts as answered only if the node was revived this soon after the death — its own Report; later, the read is a miss |
...
| Liveness threshold `auto_ping_stale_min` | 120 m, `0` = off (0–1440) | Every listening non-Dead node is asked, chatty or not, once this long has passed since this run's last sweep probe of it (`lastStaleAt`), not since its own `lastSeen` (v0.37): a reply rate is comparable between devices only when each was asked the same question at the same interval. Since v0.71.0 the question is a routed read wherever the node has a switch/light value |
...
opt-in mesh-mutating tool safe to run. Five files form the spine of this chapter
...
A value is only "wired" when all five agree on it. The header of `config.ts`
...
| `auto_ping_max_attempts` | `3` | `int(1,10)` | `AUTO_PING_MAX_ATTEMPTS` | `config.autoPing.maxAttempts` | Probes per dead episode, then the node is left to the human path. Each unanswered ping is followed by one routed read (v0.70.0). Recovery resets the budget. |
| `auto_ping_stale_min` | `120` | `int(0,1440)` | `AUTO_PING_STALE_MS` (run script ×60000) | `config.autoPing.staleMs` | Liveness sweep cadence: each listening non-Dead node is probed once per this interval, chatty or not — by one routed read since v0.71.0, a NoOp ping where it has no switch/light value; `0` disables the liveness lane. One probe per tick, stalest first. |
...
| `ha_ws_url` | `ws://supervisor/core/websocket` | `str?` | `HA_WS_URL` | `config.haWsUrl` | Override to point at a different HA Core WS. Never the driver socket (that is `driver_ws_url`): the data layer speaks HA's `zwave_js/*` commands, which zwave-js-server does not serve. |
...
trusted LAN). Change the LAN port here, in Network."**
...
export HOST=::
export TELNET_HOST=::
export TELNET_PORT=2324                # fixed; remap the LAN side in Network
...
fine ring that is display + recent-window evidence only.
...
stack with no listener and get a TCP RST. The Dockerfile's image-level `ENV`
sets `HOST=::` as well, and the run script exports `::` again at start, so the
two agree.
...
- **Add-on runtime:** the run script always exports `HA_WS_URL` (from the option,
  default `ws://supervisor/core/websocket`), so the first branch wins. The
  fallback is `||`, not `??`: `ha_ws_url` is a clearable `str?`, bashio exports a
  cleared field as the empty string, and `??` would accept `''` and connect to
  nothing; `||` falls through to the default instead.
...
- `build.yaml` pins `build_from` to `ghcr.io/home-assistant/{arch}-base:3.21`
  (Alpine 3.21 ships Node 22 in `main`). Release images are built per arch on
  native runners by `publish-release.yml` (QEMU is set up only as a fallback), and a
  local-folder deploy is built by HA Supervisor on the target arch, so neither path
  normally cross-compiles.
...
`repository.yaml` advertises the store entry, and `config.yaml`'s `image:` key
(`ghcr.io/<owner>/{arch}-zwave-tui`) points Supervisor at the prebuilt
multi-arch images `publish-release.yml` pushes for every release (§12.7). A store
install and the Update button **pull** the image for `version:`; nothing is built
on the user's host. That is the path documented in `README.md` for anyone
installing it. `configContract.test.ts` pins the image name in `image:` (the
`{arch}-zwave-tui` part) to a package name the workflow pushes; it does not check
the `ghcr.io/<owner>/` prefix.
...
repo into the local add-on folder** and strips `image:` from the deployed
`config.yaml` (step 3), so Supervisor builds that local copy from the
`Dockerfile` instead of pulling the release image. The operational recipe (with
its hard-won gotchas):
...
Only the local-folder path builds from source, which suits intra-version
iteration; the store path pulls the published GHCR image and involves no
on-device build.

### 12.7 Release pipeline

Five workflows guard and publish the project. Two gate every change:

1. **`ci.yml`** (pushes to and PRs into `main` — a **required** check: `main` is
   protected and these contexts must pass to merge, with admin bypass left on
   for urgent fixes) — type-checks and runs the
   full server test suite (`tsc --noEmit -p tsconfig.test.json`, which covers the
   test tree too), smoke-builds the add-on container for amd64 and aarch64 (each
   on a native runner), and builds the printable manual, failing the PR if
   `DOCS.md` stops converting cleanly.
2. **`codeql.yml`** (pushes to and PRs into `main`, and weekly; also a required
   check) — CodeQL `security-extended` analysis of the server. Results are
   uploaded to the
   Security tab, and `.github/scripts/check-sarif.mjs` fails the job on
   actionable findings, because an upload alone does not fail a build.

Three form the release relay:

3. **`release.yml`** (manual `workflow_dispatch`) — the one-click way to open a
   release: computes the next version, refuses one whose tag already exists,
   bumps `version:` in `config.yaml` together with `server/package.json` and the
   two version fields of `server/package-lock.json`, prepends the CHANGELOG
   section, and opens a `release/vX.Y.Z` PR titled `Release vX.Y.Z` with
   auto-merge enabled.
4. **`tag-release.yml`** (a push to `main` that touches `config.yaml`, acting only
   when the commit subject starts with `Release v`) — reads the version from
   `config.yaml`, refuses to tag when `server/package.json` disagrees, pushes the
   annotated `vX.Y.Z` tag, and dispatches `publish-release.yml` for that version:
   a tag pushed with the default `GITHUB_TOKEN` cannot trigger another workflow
   by itself. A tag that already exists makes it a no-op.
5. **`publish-release.yml`** (a `vX.Y.Z` tag push, or `workflow_dispatch` with a
   `version` input) — checks the version against `config.yaml`, re-runs the
   tests, builds and pushes the aarch64 + amd64 images to GHCR
   (`ghcr.io/<owner>/{arch}-zwave-tui`, tagged `X.Y.Z` and `latest`, each arch on
   a native runner), builds the manual (`.docx` via pandoc + `.pdf` via
   LibreOffice), and cuts a GitHub Release using that version's `CHANGELOG.md`
   section as the notes, with the manual attached. A missing CHANGELOG section
   fails the release; a Release that already exists is not re-created.

Cutting a release is therefore: bump `version:` in `config.yaml`,
`server/package.json` and `server/package-lock.json` together
(`configContract.test.ts` pins them to each other), add the `## X.Y.Z — DATE`
CHANGELOG section, and squash-merge with a subject that starts `Release vX.Y.Z`
— or run `release.yml`, which opens that PR. Tagging is automatic; there is no
manual tag push. A merge whose subject does not start `Release v` releases
nothing. The dispatch path remains for running the release of an existing tag:

```bash
gh workflow run publish-release.yml -f version=X.Y.Z
```

The GHCR images pushed here are what store installs and updates pull (§12.6).
Green tests plus adversarial multi-agent review are the merge gate.
...
`stop_listening_logs` — four, frozen), and is **never proxied or
...
tagged with the controller's `homeId`, and a mismatch (a stick swap / different
NVM) never aliases one network's data onto another. The evidence store
**purges** its rings (and `reset()` immediately rewrites the on-disk file so a
crash cannot resurrect the old rings); the learned stores — outcome ledger,
baselines, history — are wiped from memory and **parked** with saves latched off,
nothing archived or deleted, until the operator answers the identity decision
(v0.64.0; see the Mesh Identity chapter).
...
`logger.ts` writes to stdout, which is what `ha apps logs` and
...
[zwave-tui] ERROR: driver-ws: handshake never completed — terminating socket for reconnect
```
...
in per call with `(log.error ?? log)(…)` — the asset check with `warn`. A
seventh, the driver-WS client, claims `error` for its handshake timeout alone;
every other site there stays `info` deliberately, because the state machine
already reports the link on screen. The Home Assistant states publisher
(§12.11) types its sink the same way with only `warn` (v0.66.1): a publish
failure is the one line there that must survive `log_level: warning`.
...
- **Home Assistant states**, re-published every 30 s:
  ...
  **`sensor.zwave_tui_recommendation` is escalation, not execution** (v0.72.0;
  ...
  The route-failures sensor reads `unknown`, never `0`, while the statistics
  feed is blind (v0.68.0) and until a node other than the controller is on the
  roster (v0.68.1): the first publish after a restart runs before the first
  roster poll, and a zero there would be a false all-clear. It does not feed
  `degraded`.
...
"structurally unable": a `zwave_js` config-entry reload orphans the feeds with
the socket still up, and until v0.65.0 the sensor read `off` right through a
22 h 48 m evidence blackout while every detector starved. The reason names the
measured silence, so the operator is told how long rather than merely that.
...
**A refused publish is logged, and never mutes the rest.** Every entity is
attempted on every tick, so one payload Home Assistant rejects cannot stop the
others from publishing (v0.68.0); each request is abandoned after 7 s, so a
Core that hangs rather than refuses cannot hold a tick open indefinitely; and
because five such timeouts outlast the 30 s interval, a publish requested while
one is in flight joins it rather than doubling it, so ticks never pile up. A
failure is logged at `warn`, so it survives `log_level: warning` (v0.66.1), and
only when the failure message changes — a steady outage logs once, not once per
tick. The line carries the count of entities affected and the reason the
refusal gave: a JSON `message` is preferred, the text is bounded, and an
